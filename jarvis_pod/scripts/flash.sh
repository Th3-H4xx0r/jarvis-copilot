#!/usr/bin/env bash
# Build and flash the Jarvis Pod over USB.
#
#   scripts/flash.sh            back up (first time per pod), build, flash
...
#
# The first flash of each pod saves its whole 16 MB flash to
# ~/.jarvis_pod/backups/<mac>.bin — restore it with scripts/restore.sh.
set -euo pipefail
...
if [ -z "$port" ]; then
    echo "No pod on USB. Plug it in (USB-C), switch it on, and if it still doesn't show up hold BOOT while plugging in." >&2
    exit 1
fi
echo "pod on $port"
...
[ -n "$mac" ] || { echo "Couldn't read the pod's MAC over $port" >&2; exit 1; }
backups="$HOME/.jarvis_pod/backups"
...
if ! grep -a -q -E "sp-esp32-s3-1\.28-box|JarvisPod/" "$backup" && [ "$force" -eq 0 ]; then
...
# write it when the frames changed for this pod.
