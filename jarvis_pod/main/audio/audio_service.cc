...
// RBJ biquad high-pass, 180 Hz, Q 0.707, at 16 kHz; filters the first (mic) channel in place.
void AudioService::HighPass(std::vector<int16_t>& data, int channels) {
    if (hp_b0_ == 0) {
        const double w0 = 2 * M_PI * 180.0 / 16000.0, c = cos(w0), alpha = sin(w0) / (2 * 0.7071);
        const double a0 = 1 + alpha;
        hp_b0_ = static_cast<float>((1 + c) / 2 / a0);
        hp_b1_ = static_cast<float>(-(1 + c) / a0);
        hp_b2_ = hp_b0_;
        hp_a1_ = static_cast<float>(-2 * c / a0);
        hp_a2_ = static_cast<float>((1 - alpha) / a0);
    }
    const int step = channels > 0 ? channels : 1;
    for (size_t i = 0; i < data.size(); i += step) {
        const float x = data[i];
        const float y = hp_b0_ * x + hp_b1_ * hp_x1_ + hp_b2_ * hp_x2_ - hp_a1_ * hp_y1_ - hp_a2_ * hp_y2_;
        hp_x2_ = hp_x1_;
        hp_x1_ = x;
        hp_y2_ = hp_y1_;
        hp_y1_ = y;
        data[i] = static_cast<int16_t>(std::max(-32768.0f, std::min(32767.0f, y)));
    }
}
