// JARVIS: the pod's application. Owns the main task, wires audio → voice, buttons and
...
void Application::Initialize() {
    ...
    ApplyTimezone(ui);
    audio_service_.SetInputHighPass(ui.noise_cancel);
    jarvis::RegisterPodTools();
    jarvis::Link::Get().on_state = [](jarvis::LinkState s) {
        if (s == jarvis::LinkState::Unpaired) {
            Ui::Get().ShowStatus("Unpaired", "Jarvis unpaired this pod. Hold BOOT 5 s to set up again.");
        }
    };
