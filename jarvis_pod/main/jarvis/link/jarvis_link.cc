...
std::string UserAgent() { return std::string("JarvisPod/") + esp_app_get_description()->version; }
...
bool Link::Claim(const std::string& server, const std::string& code, const std::string& cf_id,
    ...
    cJSON_AddStringToObject(body, "name", ("Jarvis Pod " + logic::ApSsid(mac).substr(7)).c_str());
    ...
    if (status < 0 || status >= 500) {
        ...
        message = "The pod reached Wi-Fi but not Jarvis";
        return false;
    }
        ...
        if (NowMs() - opened < 5000) {
            if (++quick_closes >= 3) {
                ...
                if (probe.status == 401 || probe.status == 403) {
                    ESP_LOGW(TAG, "server revoked this pod");
                    SetState(LinkState::Unpaired);
                    vTaskDelay(portMAX_DELAY);
                }
            }
