// The pod_* bridge skills. The app's pod page and the agent both use these — one path.
#include <cJSON.h>
...
void RegisterPodTools() {
    auto& t = Tools();

    t.Add("pod_status",
          "Jarvis Pod status: battery, Wi-Fi, bridge link, current home and shown page, touch, wake word, firmware.",
    ...
    t.Add("pod_system_info", "Jarvis Pod firmware, ESP-IDF version, free memory, uptime, MAC and IP.",
          R"({"type":"object","properties":{}})", [](const cJSON*, cJSON* out) -> std::string {
              cJSON_AddStringToObject(out, "fw", esp_app_get_description()->version);
    ...
    t.Add("pod_snapshot", "A JPEG of exactly what the Jarvis Pod's screen shows right now (data URI).",
          R"({"type":"object","properties":{}})", [](const cJSON*, cJSON* out) -> std::string {
              std::string jpeg;
    ...
    t.Add("pod_reboot", "Restart the Jarvis Pod (it comes back online in about 15 seconds).",
          R"({"type":"object","properties":{}})", [](const cJSON*, cJSON* out) -> std::string {
    ...
    t.Add("pod_show",
          "Show a page on the Jarvis Pod's round 240 px screen right now (anything the user asks to see: "
          "a chart, weather, a list). Temporary: pressing Back discards it. Read the jarvis-pod skill for the page format.",
    ...
    t.Add("pod_home_save",
          "Save a page as a Jarvis Pod home screen (kept across reboots) and, by default, switch to it. "
          "Reusing an id replaces that home. Read the jarvis-pod skill for the page format.",
    ...
    t.Add("pod_home_list", "Jarvis Pod home screens: the built-in orb and clock plus saved custom homes, and which is current.",
          R"({"type":"object","properties":{}})", [](const cJSON*, cJSON* out) -> std::string {
    ...
    t.Add("pod_home_get", "The JSON of a saved Jarvis Pod home page.",
    ...
    t.Add("pod_home_delete", "Delete a saved Jarvis Pod home page. The built-in orb and clock can't be deleted.",
          R"({"type":"object","required":["id"],"properties":{"id":{"type":"string"}}})",
          [](const cJSON* args, cJSON* out) -> std::string {
              std::string id = cJSON_GetObjectItemCaseSensitive(args, "id")->valuestring;
...
std::map<std::string, std::string> FetchPageImages(const cJSON* page) {
    ...
    for (auto& url : urls) {
        ...
        // Web pictures go through Jarvis (/api/devices/pod/image): it re-encodes them as a small
        // baseline JPEG, which is all the pod's decoder handles (no progressive JPEG, WebP, GIF).
        ...
        if (!own_server && !pairing.server.empty()) {
            ...
            fetch = logic::JoinUrl(pairing.server, "/api/devices/pod/image?size=240&url=" + enc);
        }
