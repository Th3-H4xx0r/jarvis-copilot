...
- ``jarviscopilot_cli.providers``        -- canonical provider identity + overlays
- ``jarviscopilot_cli.model_normalize``  -- per-provider name formatting
...
from jarviscopilot_cli.providers import (
...
_HERMES_MODEL_WARNING = (
    "Nous Research JarvisCopilot 3 & 4 models are NOT agentic and are not designed "
    "for use with JarvisCopilot. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research JarvisCopilot 3 / JarvisCopilot 4 chat families.
# The previous substring check (`"jarviscopilot" in name.lower()`) false-positived on
...
_NOUS_HERMES_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])jarviscopilot[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous JarvisCopilot 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous JarvisCopilot 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``jarviscopilot config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from jarviscopilot_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'jarviscopilot model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'jarviscopilot doctor' — config issues detected:"
    ...
    # Mirrors the same logic in jarviscopilot_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from jarviscopilot_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from jarviscopilot_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by jarviscopilot provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://jarviscopilot.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a JarvisCopilot release. Falls back to the in-repo
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # show up in the picker without requiring a JarvisCopilot release.
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve JarvisCopilot slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # Use curated list — look up by JarvisCopilot slug, fall back to overlay key
            model_ids = curated.get(hermes_slug, []) or curated.get(pid, [])
    ...
    # with `jarviscopilot model`).
    try:
        from jarviscopilot_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is JarvisCopilot's canonical write key (matches
            ...
            # JarvisCopilot writes ``models:`` as a dict keyed by model id
            # (see jarviscopilot_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # JarvisCopilot's own writer uses; a hyphen variant is accepted
            ...
            # active model. JarvisCopilot's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, jarviscopilot_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`jarviscopilot_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from jarviscopilot_cli.models import fetch_openrouter_models
