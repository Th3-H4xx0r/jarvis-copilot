"""Migrate JarvisCopilot' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads JarvisCopilot' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  JarvisCopilot mcp_servers.<n>.command/args/env  → codex stdio transport
  JarvisCopilot mcp_servers.<n>.url/headers       → codex streamable_http transport
  JarvisCopilot mcp_servers.<n>.timeout           → codex tool_timeout_sec
  JarvisCopilot mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec
...
What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. JarvisCopilot' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in JarvisCopilot config.")
...
# JarvisCopilot keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # JarvisCopilot' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one JarvisCopilot MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # JarvisCopilot' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown JarvisCopilot key)")
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by JarvisCopilot)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert JarvisCopilot' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before JarvisCopilot' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior JarvisCopilot
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    ``~/.jarviscopilot`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches JarvisCopilot' own
    ...
    so a jarviscopilot installed under /opt/, /usr/local/, or a venv all work.
    ...
    # PYTHONPATH passes through so a worktree-launched jarviscopilot finds the
...
def migrate(
    ...
    *,
    ...
    """Translate JarvisCopilot mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.jarviscopilot/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register JarvisCopilot' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into JarvisCopilot for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in JarvisCopilot config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject JarvisCopilot' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into JarvisCopilot for the tools codex
