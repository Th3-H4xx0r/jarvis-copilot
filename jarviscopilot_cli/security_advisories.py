"""
Security advisory checker for JarvisCopilot.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``jarviscopilot doctor --ack <id>``; the ack is persisted to
...
1. ``jarviscopilot doctor`` (and ``jarviscopilot doctor --ack <id>``)
...
run in environments where the rest of JarvisCopilot failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including jarviscopilot when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.jarviscopilot/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: jarviscopilot doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — JarvisCopilot requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'jarviscopilot doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.jarviscopilot/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``jarviscopilot doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `jarviscopilot doctor` on the gateway host for details.")
