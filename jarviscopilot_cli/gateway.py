...
# when jarviscopilot_constants is cached from a pre-update version during `hermes update`.
...
SERVICE_DESCRIPTION = "JarvisCopilot Agent Gateway - Messaging Platform Integration"
...
def get_service_name() -> str:
    ...
    Default ``~/.jarviscopilot`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.jarviscopilot/profiles/coder`` returns ``hermes-gateway-coder``.
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.jarviscopilot`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.jarviscopilot/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    ...
    },
    {
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address JarvisCopilot will use (e.g., hermes@gmail.com)."},
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("  nohup hermes gateway run > ~/.jarviscopilot/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup hermes gateway run > ~/.jarviscopilot/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'hermes gateway run'         # persistent via tmux")
                    print("  nohup hermes gateway run > ~/.jarviscopilot/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
