"""CLI for the JarvisCopilot Kanban board — ``jarviscopilot kanban …`` subcommand.
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    Used by ``jarviscopilot kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`jarviscopilot kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across JarvisCopilot profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://jarviscopilot.nousresearch.com/docs/user-guide/features/kanban "
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `jarviscopilot kanban boards switch <slug>` or the "
            "HERMES_KANBAN_BOARD env var). Use `jarviscopilot kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `jarviscopilot gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.jarviscopilot/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``jarviscopilot kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: jarviscopilot kanban <action> [options]\n"
                "Run 'jarviscopilot kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `jarviscopilot kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # `jarviscopilot kanban boards create …`.
    if action == "boards":
        try:
...
# ---------------------------------------------------------------------------
# Boards management (jarviscopilot kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``jarviscopilot kanban boards <action>``.
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `jarviscopilot kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `jarviscopilot kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `jarviscopilot kanban boards switch {meta['slug']}` to make it current.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # through to `jarviscopilot -p <name>`.
    try:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.jarviscopilot/profiles/.")
        print("Create one with `jarviscopilot -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  jarviscopilot gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `jarviscopilot -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`jarviscopilot kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from jarviscopilot_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``jarviscopilot kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "jarviscopilot kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    jarviscopilot gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`jarviscopilot kanban list --status ready` / "
                    f"`jarviscopilot kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real JarvisCopilot profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from jarviscopilot_cli import kanban_specify as spec
