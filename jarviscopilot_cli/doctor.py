...
from jarviscopilot_cli.config import get_project_root, get_hermes_home, get_env_path
...
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.jarviscopilot or ~/.jarviscopilot/profiles/coder)

# Load environment variables from ~/.jarviscopilot/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from jarviscopilot_cli.colors import Colors, color
from jarviscopilot_cli.models import _HERMES_USER_AGENT
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.jarviscopilot/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from jarviscopilot_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from jarviscopilot_cli.auth import get_minimax_oauth_auth_status
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from jarviscopilot_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.jarviscopilot/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 JarvisCopilot Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.jarviscopilot/.env (primary location for user config)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from jarviscopilot_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from jarviscopilot_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from jarviscopilot_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from jarviscopilot_cli.config import check_config_version, migrate_config
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give JarvisCopilot a custom personality)")
        if should_fix:
    ...
    try:
        from jarviscopilot_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
