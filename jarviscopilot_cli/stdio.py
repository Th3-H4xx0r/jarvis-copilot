...
on Japanese locales, etc.).  JarvisCopilot's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``jarviscopilot_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. JarvisCopilot is running
...
def configure_windows_stdio() -> bool:
    ...
    # already-running shells don't see the change — which means jarviscopilot
    ...
    # stdin is re-configured for completeness; JarvisCopilot's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and JarvisCopilot's
    ``jarviscopilot config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching JarvisCopilot (User env var in Windows Settings, or
    export in a PowerShell profile) and JarvisCopilot picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\jarviscopilot\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``jarviscopilot`` from right after install) retain their old PATH.

    Any subprocess JarvisCopilot spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "jarviscopilot", "git", "cmd"),
        os.path.join(local_appdata, "jarviscopilot", "git", "bin"),
        os.path.join(local_appdata, "jarviscopilot", "git", "usr", "bin"),
        # JarvisCopilot venv Scripts directory — host of the jarviscopilot.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes jarviscopilot, but harmless to include.
        os.path.join(local_appdata, "jarviscopilot", "jarviscopilot", "venv", "Scripts"),
