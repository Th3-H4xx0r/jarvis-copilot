...
Pure logic: walks a JarvisCopilot config dict, returns issues for any reference
...
class RetirementIssue:
    """A reference to a retired xAI model found in a JarvisCopilot config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a JarvisCopilot config and return retirement issues.
