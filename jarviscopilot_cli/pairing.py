...
Usage:
    jarviscopilot pairing list              # Show all pending + approved users
    jarviscopilot pairing approve <platform> <code>  # Approve a pairing code
    jarviscopilot pairing revoke <platform> <user_id> # Revoke user access
    jarviscopilot pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle jarviscopilot pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: jarviscopilot pairing {list|approve|revoke|clear-pending}")
        print("Run 'jarviscopilot pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.jarviscopilot/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'jarviscopilot pairing list' to see pending codes.\n")
