...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or jarviscopilot -c.",
    ...
    "Ctrl+Z suspends JarvisCopilot to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "jarviscopilot -c resumes your most recent CLI session. jarviscopilot -c \"project name\" resumes by title.",
    "jarviscopilot -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "jarviscopilot -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "jarviscopilot chat -t web,terminal enables only specific toolsets for a focused session.",
    "jarviscopilot chat -s github-pr-workflow preloads a skill at launch.",
    "jarviscopilot chat -q \"query\" runs a single non-interactive query and exits.",
    "jarviscopilot chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "jarviscopilot chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "jarviscopilot --yolo bypasses all dangerous command approval prompts for the entire session.",
    "jarviscopilot chat --source telegram tags the session for filtering in jarviscopilot sessions list.",
    "jarviscopilot -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "jarviscopilot doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "jarviscopilot dump outputs a compact setup summary — great for bug reports.",
    "jarviscopilot config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "jarviscopilot config edit opens config.yaml in your default editor.",
    "jarviscopilot config check scans for missing or stale configuration options.",
    "jarviscopilot sessions browse opens an interactive session picker with search.",
    "jarviscopilot sessions stats shows session counts by platform and database size.",
    "jarviscopilot sessions prune --older-than 30 cleans up old sessions.",
    "jarviscopilot skills search react --source skills-sh searches the skills.sh public directory.",
    "jarviscopilot skills check scans installed hub skills for upstream updates.",
    "jarviscopilot skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "jarviscopilot skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "jarviscopilot mcp add github --command npx adds MCP servers from the command line.",
    "jarviscopilot mcp serve runs JarvisCopilot itself as an MCP server for other agents.",
    "jarviscopilot auth add lets you add multiple API keys for credential pool rotation.",
    "jarviscopilot completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "jarviscopilot logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "jarviscopilot backup creates a zip backup of your entire JarvisCopilot home directory.",
    "jarviscopilot profile create coder creates an isolated profile that becomes its own command.",
    "jarviscopilot profile create work --clone copies your current config and keys to a new profile.",
    "jarviscopilot update syncs new bundled skills to ALL profiles automatically.",
    "jarviscopilot gateway install sets up JarvisCopilot as a system service (systemd/launchd).",
    "jarviscopilot memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "jarviscopilot webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: jarviscopilot tools disables unused tools, jarviscopilot skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "jarviscopilot models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as jarviscopilot -w).",
    ...
    "jarviscopilot_timeZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call JarvisCopilot tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'jarviscopilot profile create coder' creates the 'coder' command.",
    "jarviscopilot profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, JarvisCopilot prints a resume command with session ID and stats.",
    "jarviscopilot sessions export backup.jsonl exports all sessions for backup or analysis.",
    "jarviscopilot -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "jarviscopilot skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .jarviscopilot/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: jarviscopilot cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "JarvisCopilot runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "jarviscopilot gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to JarvisCopilot via a local macOS server.",
    ...
    "Context files (.jarviscopilot.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.jarviscopilot/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "JarvisCopilot loads project context from .jarviscopilot.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — jarviscopilot tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.jarviscopilot/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "jarviscopilot chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "jarviscopilot plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "jarviscopilot acp runs JarvisCopilot as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "jarviscopilot login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "jarviscopilot status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.jarviscopilot/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make JarvisCopilot your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.jarviscopilot/SOUL.md to customize.",
    ...
    "Before a gateway session reset, JarvisCopilot auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.jarviscopilot/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.jarviscopilot/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'jarviscopilot profile create ls' would be rejected.",
    "jarviscopilot profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "jarviscopilot claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via jarviscopilot tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.jarviscopilot/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.jarviscopilot/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — JarvisCopilot auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of JarvisCopilot config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while JarvisCopilot is working.',
    ...
    '/reload reloads ~/.jarviscopilot/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.jarviscopilot/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.jarviscopilot/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'jarviscopilot curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "jarviscopilot curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'jarviscopilot curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'jarviscopilot auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'jarviscopilot dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'jarviscopilot dashboard --tui embeds the full JarvisCopilot TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.jarviscopilot/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.jarviscopilot/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.jarviscopilot/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.jarviscopilot/skills/ so drop-in skills appear without restarting the session.',
    ...
    'jarviscopilot -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'jarviscopilot chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'jarviscopilot chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'jarviscopilot chat --ignore-user-config skips ~/.jarviscopilot/config.yaml — reproducible bug reports and CI runs.',
    "jarviscopilot chat --source tool tags programmatic chats so they don't clutter jarviscopilot sessions list.",
    'jarviscopilot dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'jarviscopilot sessions rename <ID> "new title" renames any past session; jarviscopilot sessions delete <ID> removes one.',
    'jarviscopilot import restores a session export or profile archive produced by sessions export or profile export.',
    'jarviscopilot fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'jarviscopilot pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'jarviscopilot setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'jarviscopilot status --deep runs the full health sweep across every component; plain jarviscopilot status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.jarviscopilot/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes JarvisCopilot block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from jarviscopilot sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.jarviscopilot/dashboard-plugins/.',
]
