"""
jarviscopilot fallback — manage the fallback provider chain.
...
Subcommands:
  jarviscopilot fallback [list]   Show the current fallback chain (default when no subcommand)
  jarviscopilot fallback add      Pick provider + model via the same picker as `jarviscopilot model`,
                           then append the selection to the chain
  jarviscopilot fallback remove   Pick an entry to delete from the chain
  jarviscopilot fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.jarviscopilot/config.yaml`` (top-level, list of
...
from jarviscopilot_cli.fallback_config import get_fallback_chain
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from jarviscopilot_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `jarviscopilot model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  jarviscopilot fallback add")
        print()
        return
    ...
    print("  Docs: https://jarviscopilot.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `jarviscopilot model`, then append the selection to the chain."""
    from jarviscopilot_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `jarviscopilot model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `jarviscopilot fallback list` to view, or `jarviscopilot fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``jarviscopilot fallback [subcommand]``."""
