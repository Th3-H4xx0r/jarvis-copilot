"""`jarviscopilot checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.jarviscopilot/checkpoints/``.  Actions:

    jarviscopilot checkpoints               # same as `status`
    jarviscopilot checkpoints status        # total size, project count, breakdown
    jarviscopilot checkpoints list          # per-project checkpoint counts + workdir
    jarviscopilot checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    jarviscopilot checkpoints clear [-f]    # nuke the entire base (asks first)
    jarviscopilot checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    jarviscopilot checkpoints
    jarviscopilot checkpoints prune --retention-days 3 --max-size-mb 200
    jarviscopilot checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: jarviscopilot checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``jarviscopilot checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `jarviscopilot checkpoints` → status
