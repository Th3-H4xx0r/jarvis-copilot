"""Shell completion script generation for jarviscopilot CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# JarvisCopilot bash completion
# Add to ~/.bashrc:
#   eval "$(jarviscopilot completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.jarviscopilot/profiles"
...
complete -F _hermes_completion jarviscopilot
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef jarviscopilot
# JarvisCopilot zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.jarviscopilot/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.jarviscopilot/profiles 2>/dev/null)}}")
    fi
...
{top_cmds_str}
            ...
            _describe 'jarviscopilot command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# JarvisCopilot fish completion",
        "# Add to your config:",
        "#   jarviscopilot completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.jarviscopilot/profiles",
        "        ls $HOME/.jarviscopilot/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c jarviscopilot -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c jarviscopilot -f -s p -l profile"
