"""
Unified tool configuration for JarvisCopilot.

`jarviscopilot tools` and `jarviscopilot setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.jarviscopilot/config.yaml under
...
from jarviscopilot_cli.nous_subscription import (
...
from jarviscopilot_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `jarviscopilot tools` → Video Generation, which walks
...
# `jarviscopilot tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "coding_sessions"}
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from jarviscopilot_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `jarviscopilot tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``jarviscopilot tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from jarviscopilot_cli.plugins import discover_plugins, get_plugin_toolsets
...
from jarviscopilot_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "JarvisCopilot routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``jarviscopilot update`` and
      by ``jarviscopilot computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `jarviscopilot update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / JarvisCopilot process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/jarviscopilot && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in jarviscopilot directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/jarviscopilot:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `jarviscopilot update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.jarviscopilot/config.yaml")
        ...
        # Run the full `jarviscopilot auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.jarviscopilot/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from jarviscopilot_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: jarviscopilot auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `jarviscopilot auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: jarviscopilot auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: jarviscopilot auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `jarviscopilot auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: jarviscopilot auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: jarviscopilot auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # ``[hermes-cli, spotify]`` after enabling Spotify via ``jarviscopilot
        ...
        # through ``jarviscopilot tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `jarviscopilot tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `jarviscopilot tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `jarviscopilot tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by jarviscopilot tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `jarviscopilot tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `jarviscopilot tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `jarviscopilot tools` and `jarviscopilot setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ JarvisCopilot Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://jarviscopilot.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color("  Changes take effect on next 'jarviscopilot' or gateway restart.", Colors.DIM))
    print()
