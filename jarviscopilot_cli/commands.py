"""Slash command definitions and autocomplete for the JarvisCopilot CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal JarvisCopilot works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while JarvisCopilot is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.jarviscopilot/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update JarvisCopilot to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`jarviscopilot_cli.plugins.PluginContext.register_command`. They behave
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``jarviscopilot skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``jarviscopilot skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``jarviscopilot skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("jarviscopilot", "Talk to JarvisCopilot or run a subcommand", "[subcommand] [args]"))
    seen.add("jarviscopilot")
...
def slack_app_manifest(request_url: str = "https://jarviscopilot.local/slack/commands") -> dict[str, Any]:
