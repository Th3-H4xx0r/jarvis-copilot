...
Usage:
    python -m jarviscopilot_cli.main web          # Start on http://127.0.0.1:9119
    python -m jarviscopilot_cli.main web --port 8080
"""
...
# streamed to a per-action log file under ``~/.jarviscopilot/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", "jarviscopilot_cli.main", *subcommand]
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    JarvisCopilot supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in jarviscopilot_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.jarviscopilot/config.yaml`` — applies to **new** sessions only.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"JarvisCopilot PKCE ({_HERMES_OAUTH_FILE})",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from jarviscopilot_cli import auth as hauth
        if provider_id == "nous":
...
#          → persists to ~/.jarviscopilot/.anthropic_oauth.json AND credential pool
...
async def get_logs(
    ...
    from jarviscopilot_cli.logs import _read_tail, LOG_FILES
...
async def get_skills():
    ...
    from jarviscopilot_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from jarviscopilot_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from jarviscopilot_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    matching what ``jarviscopilot_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from jarviscopilot_cli.main import PROJECT_ROOT, _make_tui_argv
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall JarvisCopilot inside WSL2 to use the dashboard's /chat "
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the JarvisCopilot backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "JarvisCopilot Teal",         "description": "Classic dark teal — the canonical JarvisCopilot look"},
    {"name": "default-large", "label": "JarvisCopilot Teal (Large)", "description": "JarvisCopilot Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.jarviscopilot/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.jarviscopilot/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.jarviscopilot/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    from jarviscopilot_cli.plugins import get_bundled_plugins_dir
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from jarviscopilot_cli.plugins_cmd import (
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from jarviscopilot_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from jarviscopilot_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from jarviscopilot_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from jarviscopilot_cli.plugins_cmd import dashboard_remove_user_plugin
