...
_TASK_DESCRIPTION = "JarvisCopilot Gateway - Messaging Platform Integration"
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\jarviscopilot\\gateway-service\\<task_name>.cmd``
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m jarviscopilot_cli.main [--profile X] gateway run --replace``
    ...
    prog_args = [python_path, "-m", "jarviscopilot_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m jarviscopilot_cli.main gateway run --replace``
