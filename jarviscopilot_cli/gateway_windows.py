...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m jarviscopilot_cli.main gateway run --replace`` invocation) and
...
_TASK_DESCRIPTION = "JarvisCopilot Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current JarvisCopilot profile."""
    from jarviscopilot_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "jarviscopilot_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during jarviscopilot_cli boot.
    from jarviscopilot_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    JarvisCopilot installs stay self-contained).
    """
    _assert_windows()
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m jarviscopilot_cli.main [--profile X] gateway run``
    ...
    # if someone imports jarviscopilot_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "jarviscopilot_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older JarvisCopilot builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "jarviscopilot_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m jarviscopilot_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    from jarviscopilot_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from jarviscopilot_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated JarvisCopilot gateway install prompt.")
                if start_now:
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from jarviscopilot_cli.gateway import find_gateway_pids
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated JarvisCopilot gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: hermes gateway status")
                    return
...
def stop() -> None:
    ...
    _assert_windows()
    from jarviscopilot_cli.gateway import kill_gateway_processes
