...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.jarviscopilot/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.jarviscopilot/skills/<name>/)[/]"
        )
    try:
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via JarvisCopilot Skills Hub.\n\n"
                        f"This skill was scanned by the JarvisCopilot Skills Guard before submission.",
