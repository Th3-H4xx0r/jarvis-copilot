"""Profile distributions — shareable, packaged JarvisCopilot profiles via git.

A distribution is a JarvisCopilot profile published as a git repository (or
...
* ``jarviscopilot profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``jarviscopilot skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``jarviscopilot profile``, not a parallel tree):

    jarviscopilot profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "jarviscopilot_state.db", "response_store.db",
    ...
    "backups", "cache",
    # Infrastructure
    "jarviscopilot", ".worktrees", "profiles", "bin", "node_modules",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires JarvisCopilot {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``jarviscopilot profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a JarvisCopilot profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a JarvisCopilot distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.jarviscopilot).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `jarviscopilot profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`jarviscopilot profile install <source> --name {canon} --force`."
        )
