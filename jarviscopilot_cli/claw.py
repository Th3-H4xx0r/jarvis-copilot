"""jarviscopilot claw — OpenClaw migration commands.

Usage:
    jarviscopilot claw migrate              # Preview then migrate (always shows preview first)
    jarviscopilot claw migrate --dry-run    # Preview only, no changes
    jarviscopilot claw migrate --yes        # Skip confirmation prompt
    jarviscopilot claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    jarviscopilot claw migrate --no-backup  # Skip pre-migration snapshot
    jarviscopilot claw cleanup              # Archive leftover OpenClaw directories
    jarviscopilot claw cleanup --dry-run    # Preview what would be archived
"""
...
from jarviscopilot_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "JarvisCopilot may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a JarvisCopilot gateway is running with connected platforms.
    ...
    print()
    print_error(
        "JarvisCopilot gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'jarviscopilot stop'.")
    print()
...
def claw_command(args):
    """Route jarviscopilot claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: jarviscopilot claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to JarvisCopilot")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'jarviscopilot claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → JarvisCopilot migration."""
    ...
    print(
        color(
            "│          ⚕ JarvisCopilot — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: jarviscopilot claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.jarviscopilot/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: jarviscopilot claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the JarvisCopilot home ─────────
    # Delegates to jarviscopilot_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `jarviscopilot import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from jarviscopilot_cli.backup import create_pre_migration_backup, _format_size
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: jarviscopilot import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the JarvisCopilot home."
            )
    ...
    # 'jarviscopilot claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    print(
        color(
            "│          ⚕ JarvisCopilot — OpenClaw Cleanup                   │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: jarviscopilot claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: jarviscopilot claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  jarviscopilot claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  jarviscopilot claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  jarviscopilot config set OPENROUTER_API_KEY sk-or-v1-...")
