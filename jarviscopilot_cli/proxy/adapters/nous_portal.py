...
Reads the user's Nous OAuth state from ``~/.jarviscopilot/auth.json`` through the
...
:func:`jarviscopilot_cli.auth.resolve_nous_runtime_credentials`.
"""
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `jarviscopilot login nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `jarviscopilot login nous` to re-authenticate."
                )
    ...
    # to jarviscopilot_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
