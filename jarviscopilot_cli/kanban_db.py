...
``<root>`` is the **shared JarvisCopilot root** (the parent of any active
...
``jarviscopilot -p <profile>`` joins the same board as the dispatcher that
...
the profile's JarvisCopilot home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``jarviscopilot kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.jarviscopilot``. In Docker / custom
deployments where ``HERMES_HOME`` points outside ``~/.jarviscopilot`` (e.g.
...
# enough that kebab-case names like ``atm10-server`` or ``jarviscopilot``
...
def kanban_home() -> Path:
    """Return the shared JarvisCopilot root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``jarviscopilot kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``jarviscopilot kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``jarviscopilot kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``jarviscopilot kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # See jarviscopilot_state._WAL_INCOMPAT_MARKERS for detection logic.
    from jarviscopilot_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``jarviscopilot kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from jarviscopilot_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``jarviscopilot --skills ...`` alongside the built-in
    ...
    # `jarviscopilot --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `jarviscopilot kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``jarviscopilot kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``jarviscopilot kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``jarviscopilot kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a JarvisCopilot
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``jarviscopilot kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real JarvisCopilot profile.
    ...
    if not rows:
        return False
    try:
        from jarviscopilot_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real JarvisCopilot profile.
        # `_default_spawn` invokes ``jarviscopilot -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a JarvisCopilot
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``jarviscopilot`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``shutil.which("jarviscopilot")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m jarviscopilot_cli.main`` — fallback for setups where
       JarvisCopilot is launched from a venv and the ``jarviscopilot`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hermes_bin`` for the same reason. Kept
    local (not imported from gateway) because ``jarviscopilot_cli`` sits below
    ...
    if hermes_bin:
        return [hermes_bin]
    # Fallback to the module form. ``jarviscopilot_cli.main`` is the actual
    # console-script target declared in pyproject.toml, NOT a top-level
    # ``jarviscopilot`` package — there is no ``jarviscopilot`` package to import.
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``jarviscopilot -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `jarviscopilot -p <name>` the
    # _apply_profile_override() runs *before* jarviscopilot_constants is imported.
    # If HERMES_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".jarviscopilot" (the DEFAULT profile root), ignoring the
    ...
    # that even when the worker activates a profile (`jarviscopilot -p <name>`
    ...
    # `jarviscopilot -p <assignee>` activates the profile, but the env var is
    ...
    # `jarviscopilot kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`jarviscopilot` executable not found on PATH. "
            "Install JarvisCopilot or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``jarviscopilot kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default JarvisCopilot root exists

    Reads profile paths directly so this module has no import dependency on
    ``jarviscopilot_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from jarviscopilot_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``jarviscopilot kanban assignees`` for the terminal.
