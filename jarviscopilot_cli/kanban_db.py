...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources JarvisCopilot treats as user-controlled-but-trusted on the
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound JarvisCopilot CLI invocation."""
    # ``jarviscopilot_cli.main`` is the console-script target declared in
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved JarvisCopilot shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved JarvisCopilot executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m jarviscopilot_cli.main`` — fallback for setups where
       JarvisCopilot is launched from a venv and the ``hermes`` shim is not on
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    # home (``~/.jarviscopilot``), which ships the bundled skill.
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install JarvisCopilot Agent or activate its venv before running the kanban dispatcher."
        )
