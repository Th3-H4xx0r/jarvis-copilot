"""CLI subcommand: ``jarviscopilot send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"jarviscopilot send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"jarviscopilot send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"jarviscopilot send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"jarviscopilot send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"jarviscopilot send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `jarviscopilot gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.jarviscopilot/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.jarviscopilot/.env`` AND bridge top-level
    ...
    startup that ``jarviscopilot send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.jarviscopilot/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.jarviscopilot/config.yaml`` into
       ...
       via ``jarviscopilot config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``jarviscopilot send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.jarviscopilot/.env and ~/.jarviscopilot/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "jarviscopilot send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  jarviscopilot send --to telegram \"hello\"\n"
            "  jarviscopilot send --to discord:#ops --file report.md\n"
            "  jarviscopilot send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "jarviscopilot send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `jarviscopilot send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform JarvisCopilot "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.jarviscopilot/.env + ~/.jarviscopilot/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  jarviscopilot send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  jarviscopilot send --list                  # all platforms\n"
            "  jarviscopilot send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `jarviscopilot send --list telegram`.",
    )
