"""``jarviscopilot plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.jarviscopilot/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other JarvisCopilot subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.jarviscopilot/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from jarviscopilot_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.jarviscopilot/plugins``.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update JarvisCopilot.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `jarviscopilot plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid JarvisCopilot plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `jarviscopilot plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  jarviscopilot gateway restart[/dim]")
    console.print()
...
def _discover_all_plugins() -> list:
    ...
    user types into ``jarviscopilot plugins enable <key>``. For category-namespaced
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] jarviscopilot plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] jarviscopilot plugins")
    console.print("[dim]Enable/disable:[/dim] jarviscopilot plugins enable/disable <name>")
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.jarviscopilot/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.jarviscopilot/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.jarviscopilot/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch jarviscopilot plugins subcommands."""
