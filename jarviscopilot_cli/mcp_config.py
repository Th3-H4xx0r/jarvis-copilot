"""
MCP Server Management CLI — ``jarviscopilot mcp`` subcommand.

Implements ``jarviscopilot mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.jarviscopilot/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from jarviscopilot_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # jarviscopilot_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  jarviscopilot mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  jarviscopilot mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  jarviscopilot mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: jarviscopilot mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `jarviscopilot mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  jarviscopilot mcp add <name> --url <endpoint>')
        _info('  jarviscopilot mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `jarviscopilot mcp remove` + `jarviscopilot mcp add` to reconfigure auth.")
        return
...
# ─── jarviscopilot mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'jarviscopilot mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``jarviscopilot mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("jarviscopilot mcp serve                              Run as MCP server")
        _info("jarviscopilot mcp add <name> --url <endpoint>        Add an MCP server")
        _info("jarviscopilot mcp add <name> --command <cmd>         Add a stdio server")
        _info("jarviscopilot mcp add <name> --preset <preset>       Add from a known preset")
        _info("jarviscopilot mcp remove <name>                      Remove a server")
        _info("jarviscopilot mcp list                               List servers")
        _info("jarviscopilot mcp test <name>                        Test connection")
        _info("jarviscopilot mcp configure <name>                   Toggle tools")
        _info("jarviscopilot mcp login <name>                       Re-authenticate OAuth")
        print()
