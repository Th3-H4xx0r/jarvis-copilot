"""
Backup and import commands for jarviscopilot CLI.

`jarviscopilot backup` creates a zip archive of the entire ~/.jarviscopilot/ directory
(excluding the jarviscopilot repo and transient files).

`jarviscopilot import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
from jarviscopilot_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
...
_EXCLUDED_DIRS = {
    "jarviscopilot",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to jarviscopilot root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the JarvisCopilot home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: JarvisCopilot home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside jarviscopilot root
            try:
    ...
    print(f"\nRestore with: jarviscopilot import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a JarvisCopilot backup.
    ...
    # Look for telltale files that a jarviscopilot home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a JarvisCopilot backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.jarviscopilot/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a jarviscopilot dir name
        if prefix in {".jarviscopilot", "jarviscopilot"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has JarvisCopilot configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # jarviscopilot_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: jarviscopilot profile list  (after installing jarviscopilot)")

        # Guidance
        print()
        if not (hermes_root / "jarviscopilot").is_dir():
            print("Note: The jarviscopilot codebase was not included in the backup.")
            print("  If this is a fresh install, run: jarviscopilot update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  jarviscopilot -p {pname} gateway install")

        print("Done. Your JarvisCopilot configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and jarviscopilot backup --quick)
# ---------------------------------------------------------------------------
...
# — `jarviscopilot update` snapshots this set before pulling so approved-user lists
...
def run_quick_backup(args) -> None:
    """CLI entry point for jarviscopilot backup --quick."""
...
def create_pre_update_backup(
    ...
    (``jarviscopilot update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `jarviscopilot claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `jarviscopilot import` and the
