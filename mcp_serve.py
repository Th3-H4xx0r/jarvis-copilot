"""
JarvisCopilot MCP Server — expose messaging conversations as MCP tools.
...
Usage:
    ...
    jarviscopilot mcp serve --verbose
...
logger = logging.getLogger("jarviscopilot.mcp_serve")
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return Path(os.environ.get("HERMES_HOME", Path.home() / ".jarviscopilot")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("HERMES_HOME", Path.home() / ".jarviscopilot")
...
class EventBridge:
    ...
    This is the JarvisCopilot equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("HERMES_HOME", Path.home() / ".jarviscopilot")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the JarvisCopilot MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "jarviscopilot",
        instructions=(
            "JarvisCopilot messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the JarvisCopilot MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
