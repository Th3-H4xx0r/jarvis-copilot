...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/jarviscopilot/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("jarviscopilot") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `jarviscopilot skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with JarvisCopilot"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "JarvisCopilot ships with a large built-in skill library copied into `~/.jarviscopilot/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "JarvisCopilot also syncs bundled skills on `jarviscopilot update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.jarviscopilot/skills/` tree, it is still shipped with JarvisCopilot; restore it with `jarviscopilot skills reset <name> --restore`.",
        "",
