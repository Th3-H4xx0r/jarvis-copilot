...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to JarvisCopilot — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`jarviscopilot claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into JarvisCopilot. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

## Quick start

```bash
# Preview then migrate (always shows a preview first, then asks to confirm)
jarviscopilot claw migrate

# Preview only, no changes
jarviscopilot claw migrate --dry-run

# Full migration including API keys, skip confirmation
jarviscopilot claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.jarviscopilot/` (by default a single restore-point archive is written before apply, under `~/.jarviscopilot/backups/pre-migration-*.zip`; restorable with `jarviscopilot import`). |
...
| What | OpenClaw source | JarvisCopilot destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.jarviscopilot/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.jarviscopilot/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.jarviscopilot/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.jarviscopilot/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | JarvisCopilot destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.jarviscopilot/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.jarviscopilot/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.jarviscopilot/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.jarviscopilot/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing JarvisCopilot skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | JarvisCopilot destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.jarviscopilot/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | JarvisCopilot config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | JarvisCopilot config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | JarvisCopilot field | Notes |
|----------------|-------------|-------|
...
| What | JarvisCopilot destination |
|------|-------------------|
...
| TTS assets | `~/.jarviscopilot/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | JarvisCopilot `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | JarvisCopilot path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct JarvisCopilot equivalent)

These are saved to `~/.jarviscopilot/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in JarvisCopilot |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | JarvisCopilot has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `jarviscopilot cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/docs/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `jarviscopilot webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `jarviscopilot honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `jarviscopilot skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use JarvisCopilot profiles |
...
7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `jarviscopilot whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `jarviscopilot claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `jarviscopilot config set`.

### Skills not appearing after migration

Imported skills land in `~/.jarviscopilot/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `jarviscopilot config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
