...
description: "Diagnose and fix common JarvisCopilot cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`jarviscopilot gateway` for foreground, or `jarviscopilot gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `jarviscopilot cron tick`.
...
```bash
date
jarviscopilot cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.jarviscopilot/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.jarviscopilot/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.jarviscopilot/.env` |
...
| `local` | Write access to `~/.jarviscopilot/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `jarviscopilot cron list` for updated `last_error` field (if available).
...
```bash
jarviscopilot skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `jarviscopilot skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `jarviscopilot skills list`.
...
2. `~/.jarviscopilot/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `jarviscopilot cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the JarvisCopilot config directory). Verify:
```bash
ls ~/.jarviscopilot/scripts/your-script.py   # Must exist
jarviscopilot cron edit <job_id> --script ~/.jarviscopilot/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `jarviscopilot skills install <skill-name>`.
...
```bash
ps aux | grep jarviscopilot
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.jarviscopilot/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.jarviscopilot/cron/jobs.json
chmod 600 ~/.jarviscopilot/cron/jobs.json   # Your user should own it
```
...
```bash
jarviscopilot cron list                    # Show all jobs, states, next_run times
jarviscopilot cron run <job_id>            # Schedule for next tick (for testing)
jarviscopilot cron edit <job_id>           # Fix configuration issues
jarviscopilot logs                         # View recent JarvisCopilot logs
jarviscopilot skills list                  # Verify installed skills
```
...
1. Run the job with `jarviscopilot cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.jarviscopilot/logs/agent.log` for scheduler messages and `~/.jarviscopilot/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/jarviscopilot](https://github.com/NousResearch/jarviscopilot) with:
