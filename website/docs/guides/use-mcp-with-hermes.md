---
sidebar_position: 6
title: "Use MCP with JarvisCopilot"
description: "A practical guide to connecting MCP servers to JarvisCopilot, filtering their tools, and using them safely in real workflows"
---

# Use MCP with JarvisCopilot

This guide shows how to actually use MCP with JarvisCopilot in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native JarvisCopilot tool
- you want JarvisCopilot to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect JarvisCopilot to internal APIs, databases, or company systems without modifying JarvisCopilot core

Do not use MCP when:
- a built-in JarvisCopilot tool already solves the job well
...
- JarvisCopilot remains the agent
- MCP servers contribute tools
- JarvisCopilot discovers those tools at startup or reload time
...
If you installed JarvisCopilot with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- JarvisCopilot banner/status should show MCP integration when configured
- ask JarvisCopilot what tools it has available
...
## WSL2: bridge JarvisCopilot in WSL to Windows Chrome

This is the practical setup when:

- JarvisCopilot runs inside WSL2
...
In this setup, JarvisCopilot does **not** connect to Chrome directly. Instead:

- JarvisCopilot runs in WSL
- JarvisCopilot starts a local stdio MCP server
...
```text
JarvisCopilot (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- JarvisCopilot stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on JarvisCopilot core browser transport
...
```bash
jarviscopilot mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
jarviscopilot mcp test chrome-devtools-win
```

Then start a fresh JarvisCopilot session or run:
...
Once loaded, JarvisCopilot can use the MCP-prefixed browser tools directly. For example:
...
If JarvisCopilot runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start JarvisCopilot from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start JarvisCopilot from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in JarvisCopilot:
...
So JarvisCopilot will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want JarvisCopilot to reason over a bounded workspace.
...
Now JarvisCopilot can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing JarvisCopilot core.
...
Because JarvisCopilot now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
