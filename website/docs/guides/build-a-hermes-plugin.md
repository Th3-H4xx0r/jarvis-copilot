...
title: "Build a JarvisCopilot Plugin"
description: "Step-by-step guide to building a complete JarvisCopilot plugin with tools, hooks, data files, and skills"
---

# Build a JarvisCopilot Plugin

This guide walks through building a complete JarvisCopilot plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
JarvisCopilot has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.jarviscopilot/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/docs/user-guide/features/skills) — `jarviscopilot skills tap add <repo>` · [Publishing a tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.jarviscopilot/plugins/calculator
cd ~/.jarviscopilot/plugins/calculator
```
...
This tells JarvisCopilot: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** JarvisCopilot may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `jarviscopilot my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but JarvisCopilot continues fine
...
```bash
HERMES_PLUGINS_DEBUG=1 jarviscopilot plugins list
```
...
The same logs are always written to `~/.jarviscopilot/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
jarviscopilot logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `jarviscopilot plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.jarviscopilot/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.jarviscopilot/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
```
~/.jarviscopilot/plugins/calculator/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.jarviscopilot/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.jarviscopilot/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `jarviscopilot plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
auditable in `~/.jarviscopilot/logs/agent.log`. Plugins load after built-in
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), JarvisCopilot injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: jarviscopilot"}
...
- **The system prompt is JarvisCopilot's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `jarviscopilot <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for jarviscopilot my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: jarviscopilot my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for jarviscopilot my-plugin."""
...
After registration, users can run `jarviscopilot my-plugin status`, `jarviscopilot my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `jarviscopilot name` in a terminal |
...
JarvisCopilot has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.jarviscopilot/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `jarviscopilot gateway status` without SDK instantiation.
...
JarvisCopilot also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/docs/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into JarvisCopilot without any Python plugin. Declare them in `~/.jarviscopilot/config.yaml`:
...
JarvisCopilot connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/docs/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.jarviscopilot/hooks/<name>/`:

```yaml
# ~/.jarviscopilot/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.jarviscopilot/hooks/long-task-alert/handler.py
...
jarviscopilot skills search my-workflow --source myorg/skills-repo
jarviscopilot skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install jarviscopilot-plugin-calculator
# Plugin auto-discovered on next jarviscopilot startup
```
...
```python
# Wrong — will break if JarvisCopilot passes extra context
def handler(args):
    ...
