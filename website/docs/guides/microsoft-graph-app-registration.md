...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.jarviscopilot/.env` at the end.
...
4. Fill in:
   - **Name:** `JarvisCopilot Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `jarviscopilot-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the JarvisCopilot app
New-CsApplicationAccessPolicy `
  -Identity "Hermes-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict JarvisCopilot meeting pipeline to allow-listed users"
...
Put the three values you collected into `~/.jarviscopilot/.env`:
...
JarvisCopilot ships a Graph auth smoke-test. From your JarvisCopilot install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.jarviscopilot/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `jarviscopilot gateway restart`.
