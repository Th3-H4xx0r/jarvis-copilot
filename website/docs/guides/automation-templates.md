...
Copy-paste recipes for common automation patterns. Each template uses JarvisCopilot's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`jarviscopilot webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `jarviscopilot webhook subscribe`) |
...
```bash
jarviscopilot cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/jarviscopilot GitHub repo.

1. Run: gh issue list --repo NousResearch/jarviscopilot --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
jarviscopilot webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
jarviscopilot cron create "0 9 * * 1" \
  "Scan the NousResearch/jarviscopilot repo for documentation drift.

1. Run: gh pr list --repo NousResearch/jarviscopilot --state merged --json number,title,files,mergedAt --limit 30
...
```bash
jarviscopilot cron create "0 6 * * *" \
  "Run a dependency security audit on the jarviscopilot project.

1. cd ~/.jarviscopilot/jarviscopilot && source .venv/bin/activate
...
```bash
jarviscopilot webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
jarviscopilot webhook subscribe alert-triage \
...
```python title="~/.jarviscopilot/scripts/check-uptime.py"
...
```bash
jarviscopilot cron create "every 30m" \
  ...
  --script ~/.jarviscopilot/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
jarviscopilot cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to JarvisCopilot development." \
...
```bash
jarviscopilot webhook subscribe github-issues \
  --events "issues" \
...
```bash
jarviscopilot webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
jarviscopilot webhook subscribe stripe-payments \
...
```bash
jarviscopilot cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the jarviscopilot codebase.
...
```bash
jarviscopilot cron create "0 10 * * 3" \
