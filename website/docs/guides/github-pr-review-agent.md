...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  JarvisCopilot  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to JarvisCopilot instantly when PRs are opened or updated.
:::
...
- **JarvisCopilot installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  jarviscopilot gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.jarviscopilot/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure JarvisCopilot can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/jarviscopilot --state open --limit 3
```
...
Still in the chat, ask JarvisCopilot to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/jarviscopilot
```
...
A skill gives JarvisCopilot consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.jarviscopilot/skills/code-review
```

Create `~/.jarviscopilot/skills/code-review/SKILL.md`:
...
Verify it loaded — start `jarviscopilot` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach JarvisCopilot your team's standards:
...
```bash
jarviscopilot cron run pr-review
```
...
2. Teach JarvisCopilot your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
jarviscopilot gateway status    # Is the gateway running?
jarviscopilot cron list         # Is the job enabled?
```
