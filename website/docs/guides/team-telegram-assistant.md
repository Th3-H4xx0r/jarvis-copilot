...
This tutorial walks you through setting up a Telegram bot powered by JarvisCopilot that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **JarvisCopilot installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.jarviscopilot/.env`

:::tip
A $5/month VPS is plenty for running the gateway. JarvisCopilot itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team JarvisCopilot Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by JarvisCopilot. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.jarviscopilot/.env`:
...
```
[Gateway] Starting JarvisCopilot Gateway...
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `jarviscopilot gateway install` to update the plist.
:::
...
```bash
jarviscopilot gateway stop && jarviscopilot gateway start
```
...
jarviscopilot pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.jarviscopilot/.env`:
...
Control how much detail the bot shows when using tools. In `~/.jarviscopilot/config.yaml`:
...
Customize how the bot communicates by editing `~/.jarviscopilot/SOUL.md`:

For a full guide, see [Use SOUL.md with JarvisCopilot](/docs/guides/use-soul-with-hermes).
...
```bash
# From the CLI
jarviscopilot cron list          # View all scheduled jobs
jarviscopilot cron status        # Check if scheduler is running
...
| Session history | `~/.jarviscopilot/sessions/` |
