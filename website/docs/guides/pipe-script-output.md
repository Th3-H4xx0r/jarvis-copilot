...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `jarviscopilot send`."
---

# Pipe Script Output to Messaging Platforms

`jarviscopilot send` is a small, scriptable CLI that pushes a message to any
messaging platform JarvisCopilot is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | jarviscopilot send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `jarviscopilot
...
jarviscopilot send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | jarviscopilot send --to telegram:-1001234567890

# Send a file
jarviscopilot send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
jarviscopilot send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
jarviscopilot send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
jarviscopilot send --list

# Filter by platform
jarviscopilot send --list telegram
```
...
Any platform JarvisCopilot ships adapters for works as a target:
...
`jarviscopilot send` resolves the message body in this order:

1. **Positional argument** — `jarviscopilot send --to telegram "hi"`
2. **`--file PATH`** — `jarviscopilot send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | jarviscopilot send --to telegram`

When stdin is a TTY (no pipe), JarvisCopilot does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  jarviscopilot send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `jarviscopilot send` reuses your JarvisCopilot config, the same script works on
any host where JarvisCopilot is installed — no need to export bot tokens into
...
`jarviscopilot send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  jarviscopilot send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | jarviscopilot send \
...
```bash
./train.py --epochs 200 && \
  jarviscopilot send --to telegram "training done" || \
  jarviscopilot send --to telegram "training failed (exit $?)"
```
...
jarviscopilot send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(jarviscopilot send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `jarviscopilot send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `jarviscopilot send` calls
the platform's REST endpoint directly using credentials from
`~/.jarviscopilot/.env` and `~/.jarviscopilot/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `jarviscopilot gateway start` and retry.
...
# Machine-readable
jarviscopilot send --list --json
```

The listing is built from `~/.jarviscopilot/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`jarviscopilot gateway
...
| Approach | Multi-platform | Reuses JarvisCopilot creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `jarviscopilot send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`jarviscopilot send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `jarviscopilot send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `jarviscopilot send` shares with cron delivery.
