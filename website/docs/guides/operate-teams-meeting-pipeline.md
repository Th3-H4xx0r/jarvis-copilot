...
```bash
jarviscopilot teams-pipeline validate
```
...
```bash
jarviscopilot teams-pipeline token-health
jarviscopilot teams-pipeline token-health --force-refresh
```
...
```bash
jarviscopilot teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
jarviscopilot teams-pipeline maintain-subscriptions
jarviscopilot teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: JarvisCopilot cron (recommended if you already run the JarvisCopilot gateway)

JarvisCopilot ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.jarviscopilot/scripts/`. First create the script:

```bash
mkdir -p ~/.jarviscopilot/scripts
cat > ~/.jarviscopilot/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec jarviscopilot teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.jarviscopilot/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
jarviscopilot cron create "0 */12 * * *" \
...
```ini
[Unit]
Description=JarvisCopilot Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run JarvisCopilot Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now jarviscopilot-teams-pipeline-maintain.timer
systemctl list-timers jarviscopilot-teams-pipeline-maintain.timer
```
...
Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.jarviscopilot/.env` at the top of a wrapper script that crontab calls.
...
```bash
jarviscopilot teams-pipeline subscriptions   # should show expirationDateTime advanced
jarviscopilot teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
jarviscopilot teams-pipeline list
jarviscopilot teams-pipeline list --status failed
jarviscopilot teams-pipeline show <job-id>
```

### Replay a stored job

```bash
jarviscopilot teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
jarviscopilot teams-pipeline fetch --meeting-id <meeting-id>
jarviscopilot teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `jarviscopilot teams-pipeline maintain-subscriptions --dry-run`
- inspect `jarviscopilot teams-pipeline list --status failed`
...
- run `jarviscopilot teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `jarviscopilot teams-pipeline run`
...
- [ ] `jarviscopilot teams-pipeline validate` returns an OK snapshot
- [ ] `jarviscopilot teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (JarvisCopilot cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
