...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept JarvisCopilot already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — JarvisCopilot provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different JarvisCopilot provider
        # (must be one JarvisCopilot already knows about — same names as
        # `jarviscopilot model` and config.yaml model.provider).
        ...
        # plugin sends — JarvisCopilot does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.jarviscopilot/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through JarvisCopilot' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing JarvisCopilot subsystem:
