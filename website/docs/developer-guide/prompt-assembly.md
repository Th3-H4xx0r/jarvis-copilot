...
description: "How JarvisCopilot builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

JarvisCopilot deliberately separates:
...
```
# Layer 1: Agent Identity (from ~/.jarviscopilot/SOUL.md)
You are JarvisCopilot, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.jarviscopilot/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .jarviscopilot.md / HERMES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.jarviscopilot.md`, `HERMES.md` | CWD up to git root | Hermes-native project config |
...
- **YAML frontmatter stripped** — `.jarviscopilot.md` frontmatter is removed (reserved for future config overrides)
...
1. `.jarviscopilot.md` / `HERMES.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs JarvisCopilot already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.jarviscopilot/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.jarviscopilot/MEMORY.md` and `~/.jarviscopilot/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.jarviscopilot.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking JarvisCopilot.
...
- if you want to change how JarvisCopilot assembles prompts for everyone, change Python and treat it as a code contribution
