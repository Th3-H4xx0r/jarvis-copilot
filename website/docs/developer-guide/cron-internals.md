...
description: "How JarvisCopilot stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `jarviscopilot cron` subcommands |
...
Jobs are stored in `~/.jarviscopilot/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `jarviscopilot cron` commands are run or during active CLI sessions.
...
```python
# ~/.jarviscopilot/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.jarviscopilot/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `jarviscopilot cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `jarviscopilot cron` CLI provides direct job management:

```bash
jarviscopilot cron list                    # Show all jobs
jarviscopilot cron create                  # Interactive job creation (alias: add)
jarviscopilot cron edit <job_id>           # Edit job configuration
jarviscopilot cron pause <job_id>          # Pause a running job
jarviscopilot cron resume <job_id>         # Resume a paused job
jarviscopilot cron run <job_id>            # Trigger immediate execution
jarviscopilot cron remove <job_id>         # Delete a job
```
