...
The ACP adapter wraps JarvisCopilot' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
jarviscopilot acp / jarviscopilot-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.jarviscopilot/.env
  -> configure stderr logging
  -> construct HermesACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'jarviscopilot[acp]==<version>' jarviscopilot-acp`, pointed at the `jarviscopilot` PyPI release.
...
- `allow_once` -> JarvisCopilot `once`
- `allow_always` -> JarvisCopilot `always`
- reject options -> JarvisCopilot `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps JarvisCopilot tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["jarviscopilot-acp"])
...
Instead it reuses JarvisCopilot' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured JarvisCopilot provider/credentials. It also always advertises a terminal setup auth method (`jarviscopilot-setup`, args `--setup`) so first-run registry clients can open JarvisCopilot' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.jarviscopilot/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `jarviscopilot-acp` toolset definition
- `hermes_cli/main.py` — `jarviscopilot acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `jarviscopilot-acp` script
