...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with JarvisCopilot
...
The only required file is `__init__.py`. `plugin.yaml` is used by `jarviscopilot plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
| `jarviscopilot model` picker | `hermes_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `jarviscopilot doctor` | `hermes_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `jarviscopilot setup` | `hermes_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| `display_name` | str | Human label shown in `jarviscopilot model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.jarviscopilot/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. JarvisCopilot picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but JarvisCopilot' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
jarviscopilot -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `jarviscopilot plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$HERMES_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any JarvisCopilot plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."jarviscopilot.plugins"]
...
See [Building a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin) — general plugin authoring
