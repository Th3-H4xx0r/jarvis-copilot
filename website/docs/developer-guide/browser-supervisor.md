...
[PR #12550](https://github.com/NousResearch/jarviscopilot/pull/12550) proposed a
...
(`jarviscopilot-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per JarvisCopilot `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `jarviscopilot-acp`, `jarviscopilot-api-server`, core toolsets (gated on CDP reachability)
