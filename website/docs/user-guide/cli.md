...
sidebar_position: 1
title: "CLI Interface"
description: "Master the JarvisCopilot terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

JarvisCopilot's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
JarvisCopilot also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `jarviscopilot --tui` — see the [TUI](tui.md) guide.
:::
...
jarviscopilot chat -q "Hello"

# With a specific model
jarviscopilot chat --model "anthropic/claude-sonnet-4"

# With a specific provider
jarviscopilot chat --provider nous        # Use Nous Portal
jarviscopilot chat --provider openrouter  # Force OpenRouter
...
jarviscopilot --continue             # Resume the most recent CLI session (-c)
jarviscopilot --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
jarviscopilot chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
jarviscopilot -w                         # Interactive mode in worktree
jarviscopilot -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the JarvisCopilot CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The JarvisCopilot CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`jarviscopilot -c` or `jarviscopilot --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend JarvisCopilot to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. JarvisCopilot supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for JarvisCopilot replies |
...
JarvisCopilot loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.jarviscopilot/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.jarviscopilot/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain JarvisCopilot..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches JarvisCopilot — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while JarvisCopilot is working, JarvisCopilot prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend JarvisCopilot to the background — just like any terminal process. The shell prints a confirmation:

```
JarvisCopilot has been suspended. Run `fg` to bring JarvisCopilot back.
```
...
Resume this session with:
  jarviscopilot --resume 20260225_143052_a1b2c3
...
```bash
jarviscopilot --continue                          # Resume the most recent CLI session
jarviscopilot -c                                  # Short form
jarviscopilot -c "my project"                     # Resume a named session (latest in lineage)
jarviscopilot --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
jarviscopilot --resume "refactoring auth"         # Resume by title
jarviscopilot -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `jarviscopilot sessions rename <id> <title>` from the command line. Use `jarviscopilot sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in JarvisCopilot's SQLite state database under `~/.jarviscopilot/state.db`. The database keeps:
...
JarvisCopilot immediately confirms the task and gives you back the prompt:
