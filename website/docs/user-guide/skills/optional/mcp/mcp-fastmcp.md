...
| | |
|---|---|
| Source | Optional — install with `jarviscopilot skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into JarvisCopilot or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to JarvisCopilot. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.jarviscopilot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.jarviscopilot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is JarvisCopilot integration, either:

- configure the server in `~/.jarviscopilot/config.yaml` using the `native-mcp` skill, or
...
### JarvisCopilot cannot see the deployed server

The server-building part may be correct while the JarvisCopilot config is not. Load the `native-mcp` skill and configure the server in `~/.jarviscopilot/config.yaml`, then restart JarvisCopilot.
