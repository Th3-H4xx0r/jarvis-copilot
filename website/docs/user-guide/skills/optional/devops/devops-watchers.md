...
| | |
|---|---|
| Source | Optional — install with `jarviscopilot skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.jarviscopilot/.env` to avoid the 60 req/hr anonymous rate limit):
