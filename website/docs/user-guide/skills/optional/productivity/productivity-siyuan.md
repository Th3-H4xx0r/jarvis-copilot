...
| | |
|---|---|
| Source | Optional — install with `jarviscopilot skills install official/productivity/siyuan` |
...
3. Store it in `~/.jarviscopilot/.env`:
   ```
...
```yaml
# In ~/.jarviscopilot/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
