...
| | |
|---|---|
| Source | Optional — install with `jarviscopilot skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client jarviscopilot
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client jarviscopilot
```
...
| `--client {name}`      | Agent name for attribution (e.g. `jarviscopilot`)    |
