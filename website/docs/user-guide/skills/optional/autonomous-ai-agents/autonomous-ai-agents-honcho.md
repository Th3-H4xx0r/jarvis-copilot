...
description: "Configure and use Honcho memory with JarvisCopilot -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with JarvisCopilot -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `jarviscopilot skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for JarvisCopilot

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every JarvisCopilot profile its own peer identity while sharing a unified view of the user.
...
```bash
jarviscopilot honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this JarvisCopilot profile's AI peer
...
Honcho models conversations as interactions between **peers**. JarvisCopilot creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this JarvisCopilot instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each JarvisCopilot run |
| `global` | Single session across all directories |

Manual override: `jarviscopilot honcho map my-project-name`
...
Each JarvisCopilot profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block jarviscopilot.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `jarviscopilot.coder` host block in `honcho.json`
...
```bash
jarviscopilot honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for JarvisCopilot when Honcho memory is active.
...
Run `jarviscopilot honcho setup`. Ensure `memory.provider: honcho` is in `~/.jarviscopilot/config.yaml`.

### Memory not persisting across sessions
Check `jarviscopilot honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `jarviscopilot profile create <name> --clone`. For existing profiles: `jarviscopilot honcho sync`.
...
| Command | Description |
|---------|-------------|
| `jarviscopilot honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `jarviscopilot honcho status` | Show resolved config, connection test, peer info for active profile |
| `jarviscopilot honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `jarviscopilot honcho disable` | Disable Honcho for the active profile |
| `jarviscopilot honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `jarviscopilot honcho peers` | Show peer identities across all profiles |
| `jarviscopilot honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `jarviscopilot honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `jarviscopilot honcho sessions` | List known directory-to-session-name mappings |
| `jarviscopilot honcho map <name>` | Map current working directory to a Honcho session name |
| `jarviscopilot honcho identity` | Seed AI peer identity or show both peer representations |
| `jarviscopilot honcho sync` | Create host blocks for all JarvisCopilot profiles that don't have one yet |
| `jarviscopilot honcho migrate` | Step-by-step migration guide from OpenClaw native memory to JarvisCopilot + Honcho |
| `jarviscopilot memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `jarviscopilot memory status` | Show active memory provider and config |
| `jarviscopilot memory off` | Disable external memory provider |
