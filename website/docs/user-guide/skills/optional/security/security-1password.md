...
| | |
|---|---|
| Source | Optional — install with `jarviscopilot skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by JarvisCopilot |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during JarvisCopilot terminal calls (desktop app flow only)
...
### Service Account (recommended for JarvisCopilot)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.jarviscopilot/.env` (the skill will prompt for this on first load).
...
## JarvisCopilot Execution Pattern (desktop app flow)

JarvisCopilot terminal commands are non-interactive by default and can lose auth context between calls.
