---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into JarvisCopilot"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into JarvisCopilot"
---
...
Migrate a user's OpenClaw customization footprint into JarvisCopilot. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `jarviscopilot skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | JarvisCopilot (Nous Research) |
| License | MIT |
...
# OpenClaw -> JarvisCopilot Migration

Use this skill when a user wants to move their OpenClaw setup into JarvisCopilot with minimal manual cleanup.
...
```bash
jarviscopilot claw migrate              # Full interactive migration
jarviscopilot claw migrate --dry-run    # Preview what would be migrated
jarviscopilot claw migrate --preset user-data   # Migrate without secrets
jarviscopilot claw migrate --overwrite  # Overwrite existing conflicts
jarviscopilot claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `jarviscopilot setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the JarvisCopilot home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into JarvisCopilot memory entries
- merge OpenClaw command approval patterns into JarvisCopilot `command_allowlist`
- migrate Hermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.jarviscopilot/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen JarvisCopilot workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.jarviscopilot/tts/`
- archive non-secret docs that do not have a direct JarvisCopilot destination
...
- `~/.jarviscopilot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.jarviscopilot/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.jarviscopilot/skills/migration/openclaw-migration/`.
...
JarvisCopilot CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in JarvisCopilot. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a JarvisCopilot workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing JarvisCopilot skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.jarviscopilot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.jarviscopilot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.jarviscopilot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.jarviscopilot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.jarviscopilot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.jarviscopilot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of JarvisCopilot unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty JarvisCopilot targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean JarvisCopilot destination. Unsupported auth blobs must still be reported as skipped.
...
- JarvisCopilot persona state imported
- JarvisCopilot memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.jarviscopilot/skills/openclaw-imports/`
