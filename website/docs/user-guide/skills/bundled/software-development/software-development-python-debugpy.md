...
- A long-running process (jarviscopilot gateway, tui_gateway) misbehaves and you can't restart it
...
The jarviscopilot test runner and pytest both support this:
...
For long-lived processes: JarvisCopilot gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside JarvisCopilot you have two practical options:
...
Easiest: add `breakpoint()` near the suspect line, then run `jarviscopilot` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `jarviscopilot --tui`)
...
debugpy.wait_for_client()
```
Start `jarviscopilot --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For JarvisCopilot subagents, debug one process at a time.
