---
title: "Debugging JarvisCopilot Tui Commands — Debug JarvisCopilot TUI slash commands: Python, gateway, Ink UI"
sidebar_label: "Debugging JarvisCopilot Tui Commands"
description: "Debug JarvisCopilot TUI slash commands: Python, gateway, Ink UI"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Debugging JarvisCopilot Tui Commands

Debug JarvisCopilot TUI slash commands: Python, gateway, Ink UI.
...
| Tags | `debugging`, `jarviscopilot`, `tui`, `slash-commands`, `typescript`, `python` |
...
# Debugging JarvisCopilot TUI Slash Commands

## Overview

JarvisCopilot slash commands span three layers — Python command registry, tui_gateway JSON-RPC bridge, and the Ink/TypeScript frontend. When a command misbehaves (missing from autocomplete, works in CLI but not TUI, config persists but UI doesn't update), the bug is almost always one layer being out of sync with another.

Use this skill when you encounter issues with slash commands in the JarvisCopilot TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/jarviscopilot && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.jarviscopilot/config.yaml`)
