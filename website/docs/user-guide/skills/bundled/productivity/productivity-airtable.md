...
4. Store the token in `~/.jarviscopilot/.env` (or via `jarviscopilot setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for JarvisCopilot. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Typical JarvisCopilot Workflow
...
## Important Notes for JarvisCopilot
...
- **`AIRTABLE_API_KEY` flows from `~/.jarviscopilot/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
