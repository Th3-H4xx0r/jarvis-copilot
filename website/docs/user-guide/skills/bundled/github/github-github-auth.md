...
- Give it a name like "jarviscopilot"
- Select scopes:
...
- Give it a title like "jarviscopilot-&lt;machine-name>"
...
elif [ -f ~/.jarviscopilot/.env ] && grep -q "^GITHUB_TOKEN=" ~/.jarviscopilot/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.jarviscopilot/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
