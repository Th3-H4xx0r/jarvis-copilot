...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, JarvisCopilot automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, JarvisCopilot auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
jarviscopilot auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
jarviscopilot auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
jarviscopilot auth add anthropic --type oauth
...
Run `jarviscopilot auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `jarviscopilot auth` | Interactive pool management wizard |
| `jarviscopilot auth list` | Show all pools and credentials |
| `jarviscopilot auth list <provider>` | Show a specific provider's pool |
| `jarviscopilot auth add <provider>` | Add a credential (prompts for type and key) |
| `jarviscopilot auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `jarviscopilot auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `jarviscopilot auth remove <provider> <index>` | Remove credential by 1-based index |
| `jarviscopilot auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `jarviscopilot auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `jarviscopilot model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via jarviscopilot model:
jarviscopilot auth list
# Shows:
...
jarviscopilot auth add Together.ai --api-key sk-together-second-key
```
...
JarvisCopilot automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| JarvisCopilot PKCE OAuth | `~/.jarviscopilot/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `jarviscopilot auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `jarviscopilot auth add`) are never auto-pruned.
...
Pool state is stored in `~/.jarviscopilot/auth.json` under the `credential_pool` key:
