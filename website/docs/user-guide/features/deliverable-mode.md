...
When JarvisCopilot runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.jarviscopilot/config.yaml` under `agent.custom_instructions`.
...
If you use JarvisCopilot' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.jarviscopilot/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. JarvisCopilot's deliverable
