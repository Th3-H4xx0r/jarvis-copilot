...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with JarvisCopilot — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

JarvisCopilot supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with JarvisCopilot](/docs/guides/use-voice-mode-with-hermes).
...
:::tip
The `~/.jarviscopilot/` directory and default `config.yaml` are created automatically the first time you run `jarviscopilot`. You only need to create `~/.jarviscopilot/.env` manually for API keys.
:::
...
Voice mode is available in both the **classic CLI** (`jarviscopilot chat`) and the **TUI** (`jarviscopilot --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.jarviscopilot/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
jarviscopilot                # Start the interactive CLI
```
...
1. Start the CLI with `jarviscopilot` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.jarviscopilot/config.yaml` (default: `ctrl+b`).
:::
...
```bash
jarviscopilot gateway        # Start the gateway (connects to configured platforms)
jarviscopilot gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.jarviscopilot/.env`:
```bash
...
```bash
jarviscopilot gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.jarviscopilot/.env`
...
- Review gateway logs: `tail -f ~/.jarviscopilot/logs/gateway.log`
