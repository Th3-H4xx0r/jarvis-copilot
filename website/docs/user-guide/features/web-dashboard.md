...
The web dashboard is a browser-based UI for managing your JarvisCopilot installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `jarviscopilot --tui` via PTY/WebSocket). Alternatively set `HERMES_DASHBOARD_TUI=1`. |

```bash
# Custom port
jarviscopilot dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
jarviscopilot dashboard --host 0.0.0.0

# Start without opening browser
jarviscopilot dashboard --no-open
...
The default `jarviscopilot` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'jarviscopilot[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install jarviscopilot[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `jarviscopilot dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is intentionally off for a plain `jarviscopilot dashboard` launch. Start the dashboard with `jarviscopilot dashboard --tui` or set `HERMES_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
- Node.js (same requirement as `jarviscopilot --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'jarviscopilot[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `jarviscopilot config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.jarviscopilot/skills/` and grouped by category.
...
This re-reads `~/.jarviscopilot/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
When you run `jarviscopilot update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `jarviscopilot dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **JarvisCopilot Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **JarvisCopilot Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
