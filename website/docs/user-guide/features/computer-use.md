...
JarvisCopilot can drive your Mac's desktop — clicking, typing, scrolling,
...
```
jarviscopilot computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `jarviscopilot computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `jarviscopilot tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
3. Grant macOS permissions when prompted:
   - **System Settings → Privacy & Security → Accessibility** → allow the
     terminal (or JarvisCopilot app).
   - **System Settings → Privacy & Security → Screen Recording** → allow
     the same.
4. Start a session with the toolset enabled:
   ```
   jarviscopilot -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.jarviscopilot/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). JarvisCopilot refreshes the binary in two
places so you don't get stuck on a stale release:

- **`jarviscopilot update`** — when you update JarvisCopilot itself, if `cua-driver` is
...
- **`jarviscopilot computer-use install --upgrade`** — manual force-refresh.
...
`jarviscopilot computer-use status` shows the installed version next to the
binary path.
...
JarvisCopilot applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.jarviscopilot/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. JarvisCopilot applies four layers of optimisation:
...
`jarviscopilot computer-use install` to fetch the cua-driver binary, or run
`jarviscopilot tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/jarviscopilot/blob/main/skills/apple/macos-computer-use/SKILL.md)
