...
description: "Plugins shipped with JarvisCopilot that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

JarvisCopilot ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.jarviscopilot/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/docs/user-guide/features/plugins) page for the general plugin system, and [Build a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin) to write your own.
...
2. **User** — `~/.jarviscopilot/plugins/<name>/`
3. **Project** — `./.jarviscopilot/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `jarviscopilot memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `jarviscopilot plugins list` and the interactive `jarviscopilot plugins` UI), but none load until you explicitly enable them:

```bash
jarviscopilot plugins enable disk-cleanup
```

Or via `~/.jarviscopilot/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer JarvisCopilot. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
jarviscopilot plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `jarviscopilot plugins enable <name>`.
...
| `jarviscopilot-achievements` | dashboard tab | Steam-style collectible badges generated from your real JarvisCopilot session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `jarviscopilot memory` and `jarviscopilot plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**Enabling:** `jarviscopilot plugins enable disk-cleanup` (or check the box in `jarviscopilot plugins`).

**Disabling again:** `jarviscopilot plugins disable disk-cleanup`.

### observability/langfuse

Traces JarvisCopilot turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of JarvisCopilot' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `jarviscopilot logs`.
...
jarviscopilot plugins enable observability/langfuse
```

Or check the box in the interactive `jarviscopilot plugins` UI. Then put the credentials in `~/.jarviscopilot/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "JarvisCopilot turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the JarvisCopilot session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `jarviscopilot chat` session lives under one Langfuse session.

**Verify:**

```bash
jarviscopilot plugins list                 # observability/langfuse should show "enabled"
jarviscopilot chat -q "hello"              # check the Langfuse UI for a "JarvisCopilot turn" trace
```
...
**Disabling:** `jarviscopilot plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.jarviscopilot/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
jarviscopilot plugins enable google_meet
...
**Disabling:** `jarviscopilot plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.jarviscopilot/cache/google_meet/` until you remove them.

### jarviscopilot-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real JarvisCopilot session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with JarvisCopilot feature changes.

**How it works:**

- Scans your entire `~/.jarviscopilot/state.db` session history on the dashboard backend
...
| Secret | Hidden until JarvisCopilot detects the first related signal in your history |
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across JarvisCopilot updates. |
...
**Enabling:** Nothing to enable — `jarviscopilot-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `jarviscopilot dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.jarviscopilot/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$HERMES_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other JarvisCopilot plugin — see [Build a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.jarviscopilot/plugins/<name>/`
- Manifest source is reported as `bundled` in `jarviscopilot plugins list`
