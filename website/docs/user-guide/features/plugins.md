...
title: "Plugins"
description: "Extend JarvisCopilot with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

JarvisCopilot has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in JarvisCopilot
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.jarviscopilot/plugins/` with a `plugin.yaml` and Python code:

```
~/.jarviscopilot/plugins/my-plugin/
...
Start JarvisCopilot — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.jarviscopilot/plugins/hello-world/plugin.yaml`**
...
**`~/.jarviscopilot/plugins/hello-world/__init__.py`**

```python
"""Minimal JarvisCopilot plugin — registers a tool and a hook."""
...
Drop both files into `~/.jarviscopilot/plugins/hello-world/`, restart JarvisCopilot, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.jarviscopilot/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting JarvisCopilot.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `jarviscopilot <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `jarviscopilot plugins install` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with JarvisCopilot — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
...
Within each source, JarvisCopilot also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.jarviscopilot/plugins/model-providers/<name>/` and `~/.jarviscopilot/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

Sub-category plugins surface in `jarviscopilot plugins list` and the interactive `jarviscopilot plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `jarviscopilot plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `jarviscopilot plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.jarviscopilot/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
jarviscopilot plugins                    # interactive toggle (space to check/uncheck)
jarviscopilot plugins enable <name>      # add to allow-list
jarviscopilot plugins disable <name>     # remove from allow-list + add to disabled
```

After `jarviscopilot plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of JarvisCopilot' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.jarviscopilot/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.jarviscopilot/plugins/`.

### Migration for existing users

When you upgrade to a version of JarvisCopilot that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.jarviscopilot/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
JarvisCopilot has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.jarviscopilot/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and JarvisCopilot also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a JarvisCopilot Plugin](/docs/guides/build-a-hermes-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `jarviscopilot <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. JarvisCopilot auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `jarviscopilot skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.jarviscopilot/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
```bash
jarviscopilot plugins                                       # unified interactive UI
jarviscopilot plugins list                                  # table: enabled / disabled / not enabled
jarviscopilot plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
jarviscopilot plugins install user/repo --enable            # install AND enable (no prompt)
jarviscopilot plugins install user/repo --no-enable         # install but leave disabled (no prompt)
jarviscopilot plugins update my-plugin                      # pull latest
jarviscopilot plugins remove my-plugin                      # uninstall
jarviscopilot plugins enable my-plugin                      # add to allow-list (flat plugin)
jarviscopilot plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
jarviscopilot plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `jarviscopilot plugins list` shows in the **Name** column.

### Interactive UI

Running `jarviscopilot plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `jarviscopilot plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
