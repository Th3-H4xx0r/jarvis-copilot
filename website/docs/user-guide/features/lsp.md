...
JarvisCopilot runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. JarvisCopilot
...
1. JarvisCopilot captures a baseline of current diagnostics for the file.
...
…). JarvisCopilot auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — JarvisCopilot installs both packages together when you
run `jarviscopilot lsp install typescript` or auto-install fires on first
use.

## CLI

```
jarviscopilot lsp status          # service state + per-server install status
jarviscopilot lsp list            # registry, optionally --installed-only
jarviscopilot lsp install <id>    # eagerly install one server
jarviscopilot lsp install-all     # try every server with a known recipe
jarviscopilot lsp restart         # tear down running clients
jarviscopilot lsp which <id>      # print resolved binary path
```

`jarviscopilot lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, JarvisCopilot installs binaries into
...
Servers are kept alive for the life of the JarvisCopilot process. There's
...
**`jarviscopilot lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`jarviscopilot lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `jarviscopilot lsp status`**
...
When `jarviscopilot lsp status` shows a `Backend warnings` section, install
...
`~/.jarviscopilot/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.jarviscopilot/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `jarviscopilot lsp restart` to clear the set;
