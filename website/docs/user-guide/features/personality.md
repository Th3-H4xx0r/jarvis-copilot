...
description: "Customize JarvisCopilot's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

JarvisCopilot's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who JarvisCopilot is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

JarvisCopilot now seeds a default `SOUL.md` automatically in:

```text
~/.jarviscopilot/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run JarvisCopilot with a custom home directory, it will use:
...
- JarvisCopilot creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- JarvisCopilot loads `SOUL.md` only from `HERMES_HOME`
- JarvisCopilot does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, JarvisCopilot falls back to a built-in default identity
...
If JarvisCopilot loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the JarvisCopilot instance itself.

That also makes it easier to teach users:
- "Edit `~/.jarviscopilot/SOUL.md` to change JarvisCopilot' default personality."
...
- how JarvisCopilot should handle uncertainty, disagreement, or ambiguity
...
## What JarvisCopilot injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, JarvisCopilot falls back to a built-in default identity ("You are JarvisCopilot, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
JarvisCopilot ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain JarvisCopilot, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives JarvisCopilot its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.jarviscopilot/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.jarviscopilot/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how JarvisCopilot speaks
- `display.skin` and `/skin` affect how JarvisCopilot looks in the terminal
