...
description: "Durable SQLite-backed task board for coordinating multiple JarvisCopilot profiles"
---
...
JarvisCopilot Kanban is a durable task board, shared across all your JarvisCopilot profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.jarviscopilot/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.jarviscopilot/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `jarviscopilot kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `jarviscopilot kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.jarviscopilot/kanban/workspaces/<id>/` (or `~/.jarviscopilot/kanban/boards/<slug>/workspaces/<id>/` on non-default boards).
...
called `default` (DB at `~/.jarviscopilot/kanban.db` for back-compat). Users who
only want one stream of work never need to know about boards; the feature
is opt-in.

Per-board isolation is absolute:

- Separate SQLite DB per board (`~/.jarviscopilot/kanban/boards/<slug>/kanban.db`).
...
jarviscopilot kanban boards list

# Create a new board.
jarviscopilot kanban boards create atm10-server \
    --name "ATM10 Server" \
...
jarviscopilot kanban --board atm10-server list
jarviscopilot kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
jarviscopilot kanban boards switch atm10-server
jarviscopilot kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
jarviscopilot kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.jarviscopilot/kanban/current` — the slug persisted by `jarviscopilot kanban
   boards switch`.
4. `default`.
...
`jarviscopilot dashboard` → Kanban tab shows a board switcher at the top as soon
...
jarviscopilot kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
jarviscopilot kanban watch

# 5. See the board (you)
jarviscopilot kanban list
jarviscopilot kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `jarviscopilot kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `jarviscopilot gateway
...
until one comes up — `jarviscopilot kanban create` warns about this at creation
time.

Running `jarviscopilot kanban daemon` as a separate process is **deprecated**;
...
jarviscopilot kanban create "nightly ops review" \
    --assignee ops \
...
```bash
jarviscopilot kanban complete t_abc t_def t_hij --result "batch wrap"
jarviscopilot kanban archive  t_abc t_def t_hij
jarviscopilot kanban unblock  t_abc t_def
jarviscopilot kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `jarviscopilot kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `jarviscopilot kanban` CLI.
...
### Why tools instead of shelling to `jarviscopilot kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `jarviscopilot kanban complete` *inside* the container, where `jarviscopilot` isn't installed and `~/.jarviscopilot/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.jarviscopilot/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `jarviscopilot chat` session has zero `kanban_*` tools in its schema. The `check_fn` on each tool only returns True when `HERMES_KANBAN_TASK` is set, which only happens when the dispatcher spawned this process. No tool bloat for users who never touch kanban.
...
```bash
jarviscopilot -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
jarviscopilot -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
jarviscopilot kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

jarviscopilot kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
jarviscopilot -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
jarviscopilot -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `jarviscopilot kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `jarviscopilot kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `jarviscopilot profile create --description "..."`, `jarviscopilot profile describe <name> --text "..."`, `jarviscopilot profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).

Config knobs (all under `kanban:` in `~/.jarviscopilot/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `jarviscopilot profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.jarviscopilot/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard JarvisCopilot dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
jarviscopilot kanban init                                     # create kanban.db + print daemon hint
jarviscopilot kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
jarviscopilot kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
jarviscopilot kanban show <id> [--json]
jarviscopilot kanban assign <id> <profile>                    # or 'none' to unassign
jarviscopilot kanban link <parent_id> <child_id>
jarviscopilot kanban unlink <parent_id> <child_id>
jarviscopilot kanban claim <id> [--ttl SECONDS]
jarviscopilot kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
jarviscopilot kanban complete <id>... [--result "..."]
jarviscopilot kanban block <id> "<reason>" [--ids <id>...]
jarviscopilot kanban unblock <id>...
jarviscopilot kanban archive <id>...

jarviscopilot kanban tail <id>                                # follow a single task's event stream
jarviscopilot kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
jarviscopilot kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
jarviscopilot kanban runs <id> [--json]                       # attempt history (one row per run)
jarviscopilot kanban assignees [--json]                       # profiles on disk + per-assignee task counts
jarviscopilot kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
jarviscopilot kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `jarviscopilot gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
jarviscopilot kanban stats [--json]                           # per-status + per-assignee counts
jarviscopilot kanban log <id> [--tail BYTES]                  # worker log from ~/.jarviscopilot/kanban/logs/
jarviscopilot kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
jarviscopilot kanban notify-list [<id>] [--json]
jarviscopilot kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
jarviscopilot kanban context <id>                             # what a worker sees
jarviscopilot kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
jarviscopilot kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `jarviscopilot kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `jarviscopilot chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `jarviscopilot kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `jarviscopilot kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.jarviscopilot/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`jarviscopilot kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `jarviscopilot kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
```bash
jarviscopilot kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
jarviscopilot kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
jarviscopilot kanban notify-list
jarviscopilot kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
jarviscopilot kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
jarviscopilot kanban runs t_abcd
...
**Bulk close caveat.** `jarviscopilot kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`jarviscopilot kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `jarviscopilot kanban heartbeat $TASK` to signal liveness during long operations. |
...
`jarviscopilot kanban tail <id>` shows these for a single task. `jarviscopilot kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.jarviscopilot/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
