...
A **profile distribution** packages a complete JarvisCopilot agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a JarvisCopilot agent meant sending someone:
...
```bash
jarviscopilot profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `jarviscopilot profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running JarvisCopilot in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`jarviscopilot profile export` / `import`](../reference/profile-commands.md#jarviscopilot-profile-export) — that's what those are for.
...
```bash
jarviscopilot profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.jarviscopilot/profiles/research-bot/SOUL.md
...
Create `~/.jarviscopilot/profiles/research-bot/distribution.yaml`:
...
```bash
cd ~/.jarviscopilot/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `jarviscopilot profile update research-bot` will pull the latest.
...
```bash
jarviscopilot profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.jarviscopilot/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
# GitHub shorthand
jarviscopilot profile install github.com/you/research-bot

# Full HTTPS
jarviscopilot profile install https://github.com/you/research-bot.git

# SSH
jarviscopilot profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
jarviscopilot profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
jarviscopilot profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
jarviscopilot profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
jarviscopilot profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
jarviscopilot profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this JarvisCopilot distribution.
...
```bash
cp ~/.jarviscopilot/profiles/research-bot/.env.EXAMPLE ~/.jarviscopilot/profiles/research-bot/.env
...
```bash
jarviscopilot profile info research-bot
```
...
`jarviscopilot profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
jarviscopilot profile update research-bot
```
...
```bash
jarviscopilot profile delete research-bot
```
...
Path:    ~/.jarviscopilot/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `jarviscopilot profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
```bash
# Engineering lead
cd ~/.jarviscopilot/profiles/pr-reviewer
# ... build and tune ...
...
# Each engineer
jarviscopilot profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `jarviscopilot profile update pr-reviewer` and everyone's on the new version within minutes.
...
```bash
# You
cd ~/.jarviscopilot/profiles/polymarket-trader
...
# Anyone
jarviscopilot profile install github.com/you/hermes-polymarket-trader --alias
```
...
# Each on-call
jarviscopilot profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
jarviscopilot profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `jarviscopilot profile info <name>` and hold off on updates until you're ready.
:::
...
jarviscopilot profile info research-bot | grep Version
...
```yaml
# ~/.jarviscopilot/profiles/research-bot/local/my-overrides.yaml
...
jarviscopilot profile delete research-bot --yes
jarviscopilot profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
jarviscopilot profile update research-bot --force-config --yes
```
...
jarviscopilot profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.jarviscopilot/profiles/forked-research-bot/
...
jarviscopilot profile install ~/.jarviscopilot/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
jarviscopilot profile delete research-bot-test --yes
jarviscopilot profile install ~/.jarviscopilot/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `jarviscopilot -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `jarviscopilot profile` — not a parallel command tree.
...
- Reserved profile names (`jarviscopilot`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`jarviscopilot profile export` / `import`](../reference/profile-commands.md#jarviscopilot-profile-export) — local backup / restore (not distribution)
- [Using SOUL with JarvisCopilot](../guides/use-soul-with-hermes.md) — authoring personalities
