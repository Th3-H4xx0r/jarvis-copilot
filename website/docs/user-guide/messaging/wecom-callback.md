...
Connect JarvisCopilot to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
JarvisCopilot supports two WeCom integration modes:
...
3. JarvisCopilot decrypts the message, queues it for the agent
...
(Use `jarviscopilot gateway start` only after `jarviscopilot gateway install` has registered the systemd/launchd service.)
