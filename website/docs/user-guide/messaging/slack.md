...
sidebar_position: 4
title: "Slack"
description: "Set up JarvisCopilot as a Slack bot using Socket Mode"
---

# Slack Setup

Connect JarvisCopilot to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your JarvisCopilot instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. JarvisCopilot uses the modern
...
The fastest path is to paste a manifest JarvisCopilot generates for you. It
...
1. Generate the manifest:
   ...
   ```
   This writes `~/.jarviscopilot/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "JarvisCopilot") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, JarvisCopilot can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `jarviscopilot-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a JarvisCopilot configuration issue.
:::
...
JarvisCopilot uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
jarviscopilot gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
jarviscopilot gateway              # Foreground
jarviscopilot gateway install      # Install as a user service
sudo jarviscopilot gateway install --system   # Linux only: boot-time system service
```
...
Every JarvisCopilot command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
JarvisCopilot command with its description.

Under the hood: JarvisCopilot ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/jarviscopilot/blob/main/hermes_cli/commands.py)
...
When JarvisCopilot adds new commands (e.g. after `jarviscopilot update`), regenerate
...
1. Open [https://api.slack.com/apps](https://api.slack.com/apps) →
   your JarvisCopilot app
2. **Features → App Manifest → Edit**
3. Paste the new contents of `~/.jarviscopilot/slack-manifest.json`
...
`/hermes btw run the tests` — JarvisCopilot routes it the same way as `/btw
...
Slack never delivers them to JarvisCopilot.

As a workaround, JarvisCopilot recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
JarvisCopilot treats it identically to the slash form and replies in the same
thread.
...
```bash
jarviscopilot slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how JarvisCopilot behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@JarvisCopilot what time is it?`). In channels, JarvisCopilot replies in a thread attached to that message. |
| **Threads** | If you @mention JarvisCopilot inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.jarviscopilot/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to JarvisCopilot in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before JarvisCopilot will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where JarvisCopilot will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @JarvisCopilot`).

---

## Multi-Workspace Support

JarvisCopilot can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, JarvisCopilot also loads tokens from an **OAuth token file** at:

```
~/.jarviscopilot/slack_tokens.json
```
...
- When a message arrives, JarvisCopilot uses the correct workspace-specific client to respond.
...
JarvisCopilot supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @JarvisCopilot` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @JarvisCopilot` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. JarvisCopilot now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @JarvisCopilot`)
...
- Tokens should be stored in `~/.jarviscopilot/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your JarvisCopilot config directory
