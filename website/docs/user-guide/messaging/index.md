...
description: "Chat with JarvisCopilot from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with JarvisCopilot from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with JarvisCopilot](/docs/guides/use-voice-mode-with-hermes).
...
```mermaid
flowchart TB
    subgraph Gateway["JarvisCopilot Gateway"]
...
```bash
jarviscopilot gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
jarviscopilot gateway              # Run in foreground
jarviscopilot gateway setup        # Configure messaging platforms interactively
jarviscopilot gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo jarviscopilot gateway install --system   # Linux only: install a boot-time system service
jarviscopilot gateway start        # Start the default service
jarviscopilot gateway stop         # Stop the default service
jarviscopilot gateway status       # Check default service status
jarviscopilot gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update JarvisCopilot to the latest version |
...
Configure per-platform overrides in `~/.jarviscopilot/gateway.json`:
...
jarviscopilot pairing approve telegram XKGH5N7P

# Other pairing commands:
jarviscopilot pairing list          # View pending + approved users
jarviscopilot pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, JarvisCopilot appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.jarviscopilot/config.yaml`:
...
JarvisCopilot confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.jarviscopilot/config.yaml`:
...
```bash
jarviscopilot gateway install               # Install as user service
...
```bash
jarviscopilot gateway install               # Install as launchd agent
jarviscopilot gateway start                 # Start the service
jarviscopilot gateway stop                  # Stop the service
jarviscopilot gateway status                # Check status
tail -f ~/.jarviscopilot/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.jarviscopilot.gateway.plist`. It includes three environment variables:
...
- **HERMES_HOME** — scopes the gateway to your JarvisCopilot installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `jarviscopilot gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HERMES_HOME` directory gets its own launchd label. The default `~/.jarviscopilot` uses `ai.jarviscopilot.gateway`; other installations use `ai.jarviscopilot.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `jarviscopilot-cli` | Full access |
| Telegram | `jarviscopilot-telegram` | Full tools including terminal |
| Discord | `jarviscopilot-discord` | Full tools including terminal |
| WhatsApp | `jarviscopilot-whatsapp` | Full tools including terminal |
| Slack | `jarviscopilot-slack` | Full tools including terminal |
| Google Chat | `jarviscopilot-google_chat` | Full tools including terminal |
| Signal | `jarviscopilot-signal` | Full tools including terminal |
| SMS | `jarviscopilot-sms` | Full tools including terminal |
| Email | `jarviscopilot-email` | Full tools including terminal |
| Home Assistant | `jarviscopilot-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `jarviscopilot-mattermost` | Full tools including terminal |
| Matrix | `jarviscopilot-matrix` | Full tools including terminal |
| DingTalk | `jarviscopilot-dingtalk` | Full tools including terminal |
| Feishu/Lark | `jarviscopilot-feishu` | Full tools including terminal |
| WeCom | `jarviscopilot-wecom` | Full tools including terminal |
| WeCom Callback | `jarviscopilot-wecom-callback` | Full tools including terminal |
| Weixin | `jarviscopilot-weixin` | Full tools including terminal |
| BlueBubbles | `jarviscopilot-bluebubbles` | Full tools including terminal |
| QQBot | `jarviscopilot-qqbot` | Full tools including terminal |
| Yuanbao | `jarviscopilot-yuanbao` | Full tools including terminal |
| Microsoft Teams | `jarviscopilot-teams` | Full tools including terminal |
| API Server | `jarviscopilot-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `jarviscopilot-webhook` | Full tools including terminal |
