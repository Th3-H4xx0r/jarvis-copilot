...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up JarvisCopilot as a Feishu or Lark bot"
---

# Feishu / Lark Setup

JarvisCopilot integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; JarvisCopilot opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | JarvisCopilot responds to every message. |
| Group chats | JarvisCopilot responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. JarvisCopilot will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `jarviscopilot gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when JarvisCopilot runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run JarvisCopilot behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, JarvisCopilot starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
## Step 3: Configure JarvisCopilot
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how JarvisCopilot responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | JarvisCopilot responds to @mentions from any user in any group. |
| `allowlist` | JarvisCopilot only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | JarvisCopilot ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let JarvisCopilot read all group traffic without requiring an @mention:
...
JarvisCopilot auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default JarvisCopilot ignores messages sent by other bots. Enable bot-to-bot messaging when you want JarvisCopilot to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions JarvisCopilot. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `jarviscopilot update --gateway` needs confirmation, the adapter records the selected answer in JarvisCopilot's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, JarvisCopilot reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.jarviscopilot/feishu_comment_rules.json` (pairing grants in `~/.jarviscopilot/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.jarviscopilot/feishu_seen_message_ids.json`.
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `jarviscopilot gateway setup` |
| `Another local JarvisCopilot gateway is already using this Feishu app_id` | Only one JarvisCopilot instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | JarvisCopilot can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `jarviscopilot-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
