...
sidebar_position: 13
title: "Webhooks"
description: "Receive events from GitHub, GitLab, and other services to trigger JarvisCopilot agent runs"
---

# Webhooks

Receive events from external services (GitHub, GitLab, JIRA, Stripe, etc.) and trigger JarvisCopilot agent runs automatically. The webhook adapter runs an HTTP server that accepts POST requests, validates HMAC signatures, transforms payloads into agent prompts, and routes responses back to the source or to another configured platform.
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="JarvisCopilot — Webhooks Tutorial"
...
1. Enable via `jarviscopilot gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `jarviscopilot webhook subscribe`
...
Add the `github-pr` route to your `~/.jarviscopilot/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, JarvisCopilot processes the event, and posts a review comment on the PR.
...
```bash
jarviscopilot webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `jarviscopilot webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
jarviscopilot webhook list
```

### Remove a subscription

```bash
jarviscopilot webhook remove github-issues
```

### Test a subscription

```bash
jarviscopilot webhook test github-issues
jarviscopilot webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.jarviscopilot/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `jarviscopilot webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `jarviscopilot gateway run`
