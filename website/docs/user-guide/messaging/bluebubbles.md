...
Connect JarvisCopilot to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between JarvisCopilot and the BlueBubbles server
...
Or set environment variables directly in `~/.jarviscopilot/.env`:
...
When someone messages your iMessage, JarvisCopilot automatically sends them a pairing code. Approve it with:
```bash
jarviscopilot pairing approve bluebubbles <CODE>
```
Use `jarviscopilot pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.jarviscopilot/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.jarviscopilot/.env`):
```bash
...
JarvisCopilot will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → JarvisCopilot
JarvisCopilot → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** JarvisCopilot sends messages via the BlueBubbles REST API.
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.jarviscopilot/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — JarvisCopilot resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `jarviscopilot logs gateway` for webhook errors (or `jarviscopilot logs -f` to follow in real-time)
