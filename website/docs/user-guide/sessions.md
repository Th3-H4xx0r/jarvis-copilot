...
JarvisCopilot automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.jarviscopilot/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.jarviscopilot/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
JarvisCopilot stores session history so it can resume conversations, but it does not
...
JarvisCopilot explicitly injects for that turn.
...
For example, if a user sends an image and asks JarvisCopilot to make a meme from it,
JarvisCopilot may inspect that image once with vision and run an image-processing
...
`jarviscopilot sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`jarviscopilot` or `jarviscopilot chat`) |
...
jarviscopilot --continue
jarviscopilot -c

# Or with the chat subcommand
jarviscopilot chat --continue
jarviscopilot chat -c
```
...
jarviscopilot -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
jarviscopilot -c "my project"   # → resumes "my project #3"
```
...
jarviscopilot --resume 20250305_091523_a1b2c3d4
jarviscopilot -r 20250305_091523_a1b2c3d4

# Resume by title
jarviscopilot --resume "refactoring auth"

# Or with the chat subcommand
jarviscopilot chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `jarviscopilot sessions list`.

### Conversation Recap on Resume

When you resume a session, JarvisCopilot displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a JarvisCopilot session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.jarviscopilot/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `jarviscopilot -r "<title>"` from the shell) and pick up where the platform left off.
...
JarvisCopilot automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `jarviscopilot sessions list` or `jarviscopilot sessions browse`.
...
```bash
jarviscopilot sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), JarvisCopilot creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`jarviscopilot -c "my project"`), it automatically picks the most recent session in the lineage.
...
JarvisCopilot provides a full set of session management commands via `jarviscopilot sessions`:
...
# Filter by platform
jarviscopilot sessions list --source telegram

# Show more sessions
jarviscopilot sessions list --limit 50
```
...
jarviscopilot sessions export backup.jsonl

# Export sessions from a specific platform
jarviscopilot sessions export telegram-history.jsonl --source telegram

# Export a single session
jarviscopilot sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
jarviscopilot sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
jarviscopilot sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
jarviscopilot sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
jarviscopilot sessions prune

# Custom age threshold
jarviscopilot sessions prune --older-than 30

# Only prune sessions from a specific platform
jarviscopilot sessions prune --source telegram --older-than 60

# Skip confirmation
jarviscopilot sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`jarviscopilot insights`](/docs/reference/cli-commands#jarviscopilot-insights).
...
When JarvisCopilot cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, JarvisCopilot uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to JarvisCopilot in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.jarviscopilot/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.jarviscopilot/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.jarviscopilot/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every JarvisCopilot process in the same `HERMES_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.jarviscopilot/config.yaml`:
...
jarviscopilot sessions delete <session_id>
