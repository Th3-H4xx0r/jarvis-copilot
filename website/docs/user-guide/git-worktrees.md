...
title: "Git Worktrees"
description: "Run multiple JarvisCopilot agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

JarvisCopilot is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with JarvisCopilot so each session has a clean, isolated working directory.

## Why Use Worktrees with JarvisCopilot?

JarvisCopilot treats the **current working directory** as the project root:

- CLI: the directory where you run `jarviscopilot` or `jarviscopilot chat`
...
Now you can `cd` into the new worktree and run JarvisCopilot there:

```bash
cd ../repo-feature

# Start JarvisCopilot in the worktree
jarviscopilot
```
...
- JarvisCopilot checkpoint data under `~/.jarviscopilot/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per JarvisCopilot experiment**
...
- **Avoid running JarvisCopilot from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `jarviscopilot -w` (Automatic Worktree Mode)

JarvisCopilot has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `jarviscopilot/hermes-<hash>`).
...
```bash
jarviscopilot -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `jarviscopilot -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each JarvisCopilot session its own clean checkout.
