...
sidebar_position: 2
title: "Installation"
description: "Install JarvisCopilot on Linux, macOS, WSL2, native Windows (early beta), or Android via Termux"
---

# Installation

Get JarvisCopilot up and running in under two minutes with the one-line installer.
...
2. Otherwise it downloads portable **PortableGit** (~50MB, from the official `git-for-windows` GitHub release) and unpacks it to `%LOCALAPPDATA%\jarviscopilot\git`.  No admin rights required.  Completely isolated — it won't interfere with any system Git install, broken or otherwise.  (On 32-bit Windows it falls back to MinGit because PortableGit ships only 64-bit and ARM64 assets; bash-dependent JarvisCopilot features won't work on 32-bit hosts.)

**Why not use winget?**  Earlier designs auto-installed Git via `winget install Git.Git`, but winget fails badly when a system Git install is in a partial or broken state (exactly when users need the installer to just work).  The portable Git approach sidesteps winget, the Windows installer registry, and any existing system Git entirely.  If the JarvisCopilot Git install itself ever breaks, `Remove-Item %LOCALAPPDATA%\jarviscopilot\git` and re-run the installer — no system impact, no uninstall drama.

The installer also sets `HERMES_GIT_BASH_PATH` to the located `bash.exe` so JarvisCopilot resolves it deterministically in fresh shells.

If you prefer WSL2, the Linux installer above works inside it; both native and WSL installs can coexist without conflict (native data lives under `%LOCALAPPDATA%\jarviscopilot`, WSL data lives under `~/.jarviscopilot`).
...
- **CLI (`jarviscopilot chat`, `jarviscopilot setup`, `jarviscopilot gateway`, …)** — native, uses your default terminal
...
The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `jarviscopilot` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
jarviscopilot tools          # Configure which tools are enabled
jarviscopilot gateway setup  # Set up messaging platforms
jarviscopilot config set     # Set individual config values
jarviscopilot setup          # Or run the full setup wizard to configure everything at once
```
...
| Problem | Solution |
|---------|----------|
| `jarviscopilot: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `jarviscopilot model` to configure your provider, or `jarviscopilot config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `jarviscopilot config check` then `jarviscopilot config migrate` |

For more diagnostics, run `jarviscopilot doctor` — it will tell you exactly what's missing and how to fix it.
