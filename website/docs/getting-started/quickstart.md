...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with JarvisCopilot — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working JarvisCopilot setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [JarvisCopilot Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="JarvisCopilot Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up JarvisCopilot for a team, bot, or always-on workflow
...
|---|---|---|
| I just want JarvisCopilot working on my machine | `jarviscopilot setup` | Run a real chat and verify it responds |
| I already know my provider | `jarviscopilot model` | Save the config, then start chatting |
| I want a bot or always-on setup | `jarviscopilot gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `jarviscopilot model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `jarviscopilot model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if JarvisCopilot cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install JarvisCopilot
...
The single most important setup step. Use `jarviscopilot model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `jarviscopilot model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `jarviscopilot model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `jarviscopilot model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `jarviscopilot model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `jarviscopilot model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `jarviscopilot model` (requires `copilot` CLI + `copilot login`) |
...
JarvisCopilot requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `jarviscopilot model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

JarvisCopilot separates secrets from normal config:

- **Secrets and tokens** → `~/.jarviscopilot/.env`
- **Non-secret settings** → `~/.jarviscopilot/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
jarviscopilot config set model anthropic/claude-opus-4.6
jarviscopilot config set terminal.backend docker
jarviscopilot config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
jarviscopilot            # classic CLI
jarviscopilot --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
JarvisCopilot ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `jarviscopilot` vs `jarviscopilot --tui`.
:::
...
- JarvisCopilot replies without error
...
```bash
jarviscopilot --continue    # Resume the most recent session
jarviscopilot -c            # Short form
```
...
```bash
jarviscopilot gateway setup    # Interactive platform configuration
```
...
- `jarviscopilot tools` — tune tool access per platform
- `jarviscopilot skills` — browse and install reusable workflows
...
```bash
jarviscopilot config set terminal.backend docker    # Docker isolation
jarviscopilot config set terminal.backend ssh       # Remote server
```
...
```bash
jarviscopilot skills search kubernetes
...
```yaml
# Add to ~/.jarviscopilot/config.yaml
mcp_servers:
  github:
    command: npx
...
|---|---|---|
| JarvisCopilot opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `jarviscopilot model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `jarviscopilot gateway setup` and check `jarviscopilot gateway status` |
| `jarviscopilot --continue` can't find old session | Switched profiles or session never saved | Check `jarviscopilot sessions list` and confirm you're in the right profile |
...
| `jarviscopilot doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |

## Recovery Toolkit

When something feels off, use this order:

1. `jarviscopilot doctor`
2. `jarviscopilot model`
3. `jarviscopilot setup`
4. `jarviscopilot sessions list`
5. `jarviscopilot --continue`
6. `jarviscopilot gateway status`
...
| Command | Description |
|---------|-------------|
| `jarviscopilot` | Start chatting |
| `jarviscopilot model` | Choose your LLM provider and model |
| `jarviscopilot tools` | Configure which tools are enabled per platform |
| `jarviscopilot setup` | Full setup wizard (configures everything at once) |
| `jarviscopilot doctor` | Diagnose issues |
| `jarviscopilot update` | Update to latest version |
| `jarviscopilot gateway` | Start the messaging gateway |
| `jarviscopilot --continue` | Resume last session |
