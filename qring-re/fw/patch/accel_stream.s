@ RT12_3.10.06 patch: expose accelerometer samples over BLE (new command 0xB2).
...
@ are replaced with "b.w cave". The cave intercepts cmd 0xB2 and otherwise replays
@ the two displaced instructions and returns into the dispatcher at 0x82b62a.
@
@ Protocol added:  phone -> ring   16-byte frame, byte[0]=0xB2  (poll)
@                  ring  -> phone   16-byte frame, byte[0]=0xB2, [1..6]=x,y,z int16 LE
@ The phone polls 0xB2 as fast as it likes; each reply is the most recent FIFO sample.
@
...
.Lnowrap:
        ...
        movs    r0, #0xB2                   @ reply command id
        movs    r2, #6                      @ payload length
        bl      SEND_REPLY                  @ send_reply_payload(0xB2, &sample, 6)
