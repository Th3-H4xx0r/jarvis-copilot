# nix/nixosModules.nix — NixOS module for jarviscopilot
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: jarviscopilot runs from /nix/store bind-mounted read-only into a
...
# and read by jarviscopilot at startup — no container recreation needed for env changes.
#
# Tool resolution: the jarviscopilot wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.jarviscopilot;
    effectivePackage =
    ...
    jarviscopilot = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-jarviscopilot)
    deepConfigType = lib.types.mkOptionType {
      name = "hermes-config-attrs";
      description = "JarvisCopilot YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    # jarviscopilot user + sudo on first boot (writable layer persists), then
  ...
  in {
    options.services.jarviscopilot = with lib; {
      enable = mkEnableOption "JarvisCopilot Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = jarviscopilot;
        description = "The jarviscopilot package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .jarviscopilot/ subdir (HERMES_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative JarvisCopilot config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          JarvisCopilot reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `jarviscopilot gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the jarviscopilot user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the jarviscopilot plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. JarvisCopilot discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as jarviscopilot (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in jarviscopilot's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the jarviscopilot CLI to environment.systemPackages and export
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.jarviscopilot symlink to the service
            stateDir. These users are automatically added to the jarviscopilot group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.jarviscopilot.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the jarviscopilot CLI to system PATH and export HERMES_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.jarviscopilot/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.HERMES_HOME = "${cfg.stateDir}/.jarviscopilot";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.jarviscopilot.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the jarviscopilot user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.jarviscopilot: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed jarviscopilot
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure jarviscopilot is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.jarviscopilot        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarviscopilot/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarviscopilot/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarviscopilot/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarviscopilot/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarviscopilot/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."jarviscopilot-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.jarviscopilot
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.jarviscopilot ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.jarviscopilot ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.jarviscopilot -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.jarviscopilot/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.jarviscopilot/$_subdir"
            chmod 2770 "${cfg.stateDir}/.jarviscopilot/$_subdir"
            find "${cfg.stateDir}/.jarviscopilot/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.jarviscopilot/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.jarviscopilot/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarviscopilot/config.yaml
            chmod 0640 ${cfg.stateDir}/.jarviscopilot/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.jarviscopilot/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarviscopilot/.managed
          chmod 0644 ${cfg.stateDir}/.jarviscopilot/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.jarviscopilot/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    ...
    HERMES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarviscopilot/.container-mode
            chmod 0644 ${cfg.stateDir}/.jarviscopilot/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.jarviscopilot/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jarviscopilot";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.jarviscopilot" ]; then
                  rm -f "${symlinkPath}"
                  echo "jarviscopilot: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.jarviscopilot -> stateDir/.jarviscopilot for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jarviscopilot";
                target = "${cfg.stateDir}/.jarviscopilot";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "jarviscopilot: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jarviscopilot/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.jarviscopilot/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jarviscopilot/auth.json
              fi
            ''}
          ''}
          ...
          # JarvisCopilot reads $HERMES_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.jarviscopilot/.env"
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.jarviscopilot/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.jarviscopilot/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarviscopilot/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.jarviscopilot = {
          description = "JarvisCopilot Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            HERMES_HOME = "${cfg.stateDir}/.jarviscopilot";
            HERMES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # so interactive users in the jarviscopilot group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.jarviscopilot = {
          description = "JarvisCopilot Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env HERMES_HOME=${containerDataDir}/.jarviscopilot \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/jarviscopilot gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
