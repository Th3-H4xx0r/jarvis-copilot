...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      jarviscopilot = self'.packages.default;
      hermesVenv = jarviscopilot.hermesVenv;
    ...
    in {
      ...
      checks = {
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${jarviscopilot}/bin/hermes || (echo "FAIL: jarviscopilot binary missing"; exit 1)
          test -x ${jarviscopilot}/bin/jarviscopilot || (echo "FAIL: jarviscopilot binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${jarviscopilot}/bin/jarviscopilot version 2>&1 | grep -qi "jarviscopilot" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in jarviscopilot jarviscopilot jarviscopilot-acp; do
            test -x ${jarviscopilot}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hermes-cli-commands" { } ''
          ...
          echo "=== Checking jarviscopilot --help ==="
          ${jarviscopilot}/bin/jarviscopilot --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${jarviscopilot}/bin/jarviscopilot --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          test -d ${jarviscopilot}/share/jarviscopilot/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${jarviscopilot}/share/jarviscopilot/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${jarviscopilot}/bin/hermes || \
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          test -d ${jarviscopilot}/share/jarviscopilot/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${jarviscopilot}/share/jarviscopilot/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${jarviscopilot}/bin/hermes || \
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          test -d ${jarviscopilot}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${jarviscopilot}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${jarviscopilot}/bin/hermes || \
        ...
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          ...
          grep -q "HERMES_NODE" ${jarviscopilot}/bin/hermes || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${jarviscopilot}/bin/hermes)
        ...
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked "config set" ${jarviscopilot}/bin/jarviscopilot config set model foo
          check_blocked "config edit" ${jarviscopilot}/bin/jarviscopilot config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = jarviscopilot.override {
        ...
        in pkgs.runCommand "hermes-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${jarviscopilot}/bin/hermes; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = jarviscopilot.override {
