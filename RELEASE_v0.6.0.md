...
- **Profiles — Multi-Instance JarvisCopilot** — Run multiple isolated JarvisCopilot instances from the same installation. Each profile gets its own config, memory, sessions, skills, and gateway service. Create with `jarviscopilot profile create`, switch with `jarviscopilot -p <name>`, export/import for sharing. Full token-lock isolation prevents two profiles from using the same bot credential. ([#3681](https://github.com/NousResearch/hermes-agent/pull/3681))
...
- **Docker Container** — Official Dockerfile for running JarvisCopilot in a container. Supports both CLI and gateway modes with volume-mounted config. ([#3668](https://github.com/NousResearch/hermes-agent/pull/3668), closes [#850](https://github.com/NousResearch/hermes-agent/issues/850))
...
- **Slack Multi-Workspace OAuth** — Connect a single JarvisCopilot gateway to multiple Slack workspaces via OAuth token file. Each workspace gets its own bot token, resolved dynamically per incoming event. ([#3903](https://github.com/NousResearch/hermes-agent/pull/3903))
...
- **Stop silent OpenRouter fallback** — when no provider is configured, JarvisCopilot now raises a clear error instead of silently routing to OpenRouter ([#3807](https://github.com/NousResearch/hermes-agent/pull/3807), [#3862](https://github.com/NousResearch/hermes-agent/pull/3862))
...
- **Profiles system** — `jarviscopilot profile create/list/switch/delete/export/import/rename`. Each profile gets isolated HERMES_HOME, gateway service, CLI wrapper. Token locks prevent credential collisions. Tab completion for profile names. ([#3681](https://github.com/NousResearch/hermes-agent/pull/3681))
- **Profile-aware display paths** — all user-facing `~/.jarviscopilot` paths replaced with `display_hermes_home()` to show the correct profile directory ([#3623](https://github.com/NousResearch/hermes-agent/pull/3623))
- **Lazy display_hermes_home imports** — prevents `ImportError` during `jarviscopilot update` when modules cache stale bytecode ([#3776](https://github.com/NousResearch/hermes-agent/pull/3776))
- **HERMES_HOME for protected paths** — `.env` write-deny path now respects HERMES_HOME instead of hardcoded `~/.jarviscopilot` ([#3840](https://github.com/NousResearch/hermes-agent/pull/3840))
...
- **DISCORD_IGNORE_NO_MENTION** — skip messages that @mention other users/bots but not JarvisCopilot ([#3640](https://github.com/NousResearch/hermes-agent/pull/3640))
...
- **Tool token context display** — `jarviscopilot tools` checklist now shows estimated token cost per toolset ([#3805](https://github.com/NousResearch/hermes-agent/pull/3805))
...
### Setup & Configuration
- **Auto-install matrix-nio** during `jarviscopilot setup` when Matrix is selected ([#3802](https://github.com/NousResearch/hermes-agent/pull/3802), [#3873](https://github.com/NousResearch/hermes-agent/pull/3873))
...
- **Clear __pycache__ during update** — prevents stale bytecode ImportError after `jarviscopilot update` ([#3819](https://github.com/NousResearch/hermes-agent/pull/3819))
...
### MCP
- **MCP Server Mode** — `jarviscopilot mcp serve` exposes conversations, sessions, and attachments to MCP clients via stdio or Streamable HTTP ([#3795](https://github.com/NousResearch/hermes-agent/pull/3795))
...
### Plugin System
- **Plugin enable/disable commands** — `jarviscopilot plugins enable/disable <name>` for managing plugin state without removing them ([#3747](https://github.com/NousResearch/hermes-agent/pull/3747))
...
- **Lazy imports for update safety** — prevent ImportError chains during `jarviscopilot update` when modules reference new functions ([#3776](https://github.com/NousResearch/hermes-agent/pull/3776))
...
- **api_mode stale on provider switch** — correctly clear when switching providers via `jarviscopilot model` ([#3857](https://github.com/NousResearch/hermes-agent/pull/3857))
...
- **Comprehensive OpenClaw migration guide** — step-by-step guide for migrating from OpenClaw/Claw3D to JarvisCopilot ([#3864](https://github.com/NousResearch/hermes-agent/pull/3864), [#3900](https://github.com/NousResearch/hermes-agent/pull/3900))
