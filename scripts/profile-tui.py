#!/usr/bin/env python3
"""Drive the JarvisCopilot TUI under HERMES_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.jarviscopilot/perf.log on exit.
...
bypasses the jarviscopilot_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HERMES_PERF_LOG     (default ~/.jarviscopilot/perf.log)
