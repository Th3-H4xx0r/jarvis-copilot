...
# Python-driven tool session (e.g. JarvisCopilot terminal tool). A pre-set PYTHONPATH
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --postinstall)
            ...
            ;;
        -h|--help)
            echo "JarvisCopilot Agent Installer"
            echo ""
...
install_node() {
    ...
    log_info "Extracting to ~/.jarviscopilot/node/..."
    ...
    # Place into ~/.jarviscopilot/node/ and symlink binaries to ~/.local/bin/
    ...
    log_success "Node.js $installed_ver installed to ~/.jarviscopilot/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. JarvisCopilot install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "JarvisCopilot Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if JarvisCopilot behaves unexpectedly."
                    else
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# JarvisCopilot Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# JarvisCopilot Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # Create ~/.jarviscopilot directory structure (config at top level, code in subdir)
    mkdir -p "$HERMES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.jarviscopilot/.env (top level, easy to find)
    if [ ! -f "$HERMES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HERMES_HOME/.env"
            log_success "Created ~/.jarviscopilot/.env from template"
        else
            touch "$HERMES_HOME/.env"
            log_success "Created ~/.jarviscopilot/.env"
        fi
    else
        log_info "~/.jarviscopilot/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.jarviscopilot/config.yaml (top level, easy to find)
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HERMES_HOME/config.yaml"
            log_success "Created ~/.jarviscopilot/config.yaml from template"
        fi
    else
        log_info "~/.jarviscopilot/config.yaml already exists, keeping it"
    fi
...
Edit this to customize how JarvisCopilot communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.jarviscopilot/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.jarviscopilot/"

    # Seed bundled skills into ~/.jarviscopilot/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.jarviscopilot/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.jarviscopilot/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HERMES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$HERMES_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.jarviscopilot/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# JarvisCopilot Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; JarvisCopilot will use the system browser."
        else
            case "$DISTRO" in
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        python -m jarviscopilot_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for JarvisCopilot to send/receive messages."
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up JarvisCopilot for pip install"
