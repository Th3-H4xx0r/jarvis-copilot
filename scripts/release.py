...
VERSION_FILE = REPO_ROOT / "jarviscopilot_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+jarviscopilot-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "jarviscopilot@nousresearch.com": "NousResearch",
    ...
    "jarviscopilot@noushq.ai": "benbarclay",
    ...
    "jarviscopilot@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "agent@jarviscopilot.local": "jacdevos",
    ...
    "teknium@jarviscopilot": "teknium1",
    ...
    "vigo@jarviscopilot": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"jarviscopilot[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "jarviscopilot@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/jarviscopilot",
    ...
    lines.append(f"# JarvisCopilot v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for JarvisCopilot. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="JarvisCopilot Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  JarvisCopilot Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"JarvisCopilot v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"JarvisCopilot v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'JarvisCopilot v{new_version} ({calver_date})' "
