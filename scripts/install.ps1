# ============================================================================
# JarvisCopilot Agent Installer for Windows
# ============================================================================
...
function Write-Banner {
    ...
    Write-Host "|             * JarvisCopilot Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Git {
    ...
    which JarvisCopilot uses to run shell commands.
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    JarvisCopilot needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    ``HERMES_GIT_BASH_PATH`` (User scope) so JarvisCopilot can find it in a fresh
    ...
    try {
        ...
        Write-Info "then re-run this installer.  JarvisCopilot needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so JarvisCopilot can find it even before
    ...
    Write-Warn "Could not locate bash.exe -- JarvisCopilot may not find Git Bash."
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.jarviscopilot directory structure
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.jarviscopilot/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.jarviscopilot/.env"
        }
    } else {
        Write-Info "~/.jarviscopilot/.env already exists, keeping it"
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.jarviscopilot/config.yaml from template"
        }
    } else {
        Write-Info "~/.jarviscopilot/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and JarvisCopilot's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.jarviscopilot/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.jarviscopilot/"
    
    # Seed bundled skills into ~/.jarviscopilot/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.jarviscopilot/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.jarviscopilot/skills/"
        } catch {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to ~/.jarviscopilot/skills/"
            }
        }
    }
}
...
function Install-PlatformSdks {
    ...
    # ~/.jarviscopilot/.env are importable.  Two problems this solves:
    #
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning JarvisCopilot repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding JarvisCopilot to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
