...
# Bootstrap Open WebUI against JarvisCopilot Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.jarviscopilot/.env has API server settings
...
#   OPEN_WEBUI_NAME='Johnny JarvisCopilot'
...
#   HERMES_API_MODEL_NAME='JarvisCopilot Agent'
...
HERMES_ENV_FILE="${HERMES_ENV_FILE:-$HOME/.jarviscopilot/.env}"
...
LOG_DIR="$HOME/.jarviscopilot/logs"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.jarviscopilot.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.jarviscopilot</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.jarviscopilot"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.jarviscopilot"
}
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to JarvisCopilot Agent
...
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.jarviscopilot/logs/openwebui.log
StandardError=append:%h/.jarviscopilot/logs/openwebui.error.log
...
main() {
  ...
  log 'Ensuring JarvisCopilot API server is configured...'
  ...
  log 'Restarting JarvisCopilot gateway so API server settings take effect...'
  jarviscopilot gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'JarvisCopilot API server did not answer on the first check. Trying to start gateway in the background...'
    nohup jarviscopilot gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "JarvisCopilot API endpoint: ${HERMES_API_BASE_URL}"
