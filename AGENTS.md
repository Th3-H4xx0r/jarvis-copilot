...
│   ├── jarviscopilot-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `jarviscopilot --tui`
...
**User config:** `~/.jarviscopilot/config.yaml` (settings), `~/.jarviscopilot/.env` (API keys only).
**Logs:** `~/.jarviscopilot/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_hermes_home()`.
Browse with `jarviscopilot logs [--follow] [--level ...] [--session ...]`.
...
- Skill slash commands: `agent/skill_commands.py` scans `~/.jarviscopilot/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `jarviscopilot --tui` or `HERMES_TUI=1`.
...
npm run dev       # watch mode (rebuilds jarviscopilot-ink + tsx --watch)
npm start         # production
npm run build     # full build (jarviscopilot-ink + tsc)
...
### TUI in the Dashboard (`jarviscopilot dashboard` → `/chat`)

The dashboard embeds the real `jarviscopilot --tui` — **not** a rewrite.  See `hermes_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `hermes_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `jarviscopilot --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `jarviscopilot --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.
...
For most custom or local-only tools, do **not** edit JarvisCopilot core. Use the plugin
route instead: create `~/.jarviscopilot/plugins/<name>/plugin.yaml` and
`~/.jarviscopilot/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core JarvisCopilot tool that should ship in the base system.
...
**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_hermes_home()` for the base directory — never `Path.home() / ".jarviscopilot"`. This ensures each profile gets its own state.
...
| `load_config()` | `jarviscopilot tools`, `jarviscopilot setup`, most CLI subcommands | `hermes_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
~/.jarviscopilot/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic JarvisCopilot gold/kawaii (the current look)
...
Users create `~/.jarviscopilot/skins/<name>.yaml`:
...
JarvisCopilot has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.jarviscopilot/plugins/` and pip-installed entry points.

### General plugins (`hermes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.jarviscopilot/plugins/`, `./.jarviscopilot/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `jarviscopilot` at startup so
  `jarviscopilot <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `jarviscopilot <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `jarviscopilot --help`.
...
into `~/.jarviscopilot/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `jarviscopilot memory setup` / `post_setup()` without
...
[`jarviscopilot-example-plugins`](https://github.com/NousResearch/hermes-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `jarviscopilot skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.jarviscopilot.tags`, `metadata.jarviscopilot.category`,
`metadata.jarviscopilot.related_skills`, `metadata.jarviscopilot.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.jarviscopilot.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native JarvisCopilot tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "JarvisCopilot" is the secondary collaborator. If the
   contributor's commit shows "JarvisCopilot" as author (because they
   used JarvisCopilot to draft the skill), replace it with their actual name
...
Enable/disable per platform via `jarviscopilot tools` (the curses UI) or the
...
go to `~/.jarviscopilot/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `hermes_cli/curator.py` wires `jarviscopilot curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.jarviscopilot/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `jarviscopilot cron <verb>`
...
- File lock at `~/.jarviscopilot/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `jarviscopilot kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `hermes_cli/kanban.py` wires `jarviscopilot kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`jarviscopilot-kanban-dispatcher.service` for
...
JarvisCopilot supports **profiles** — multiple fully isolated instances, each with its own
...
2. **Use `display_hermes_home()` for user-facing messages.** Import from `jarviscopilot_constants`.
   ...
   print("Config saved to ~/.jarviscopilot/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_hermes_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_hermes_home()`,
   not `Path.home() / ".jarviscopilot"`.

4. **Tests that mock `Path.home()` must also set `HERMES_HOME`** — since code now uses
   `get_hermes_home()` (reads env var), not `Path.home() / ".jarviscopilot"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"HERMES_HOME": str(tmp_path / ".jarviscopilot")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not HERMES_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".jarviscopilot" / "profiles"`, NOT `get_hermes_home() / "profiles"`.
   This is intentional — it lets `jarviscopilot -p coder profile list` see all profiles regardless
...
### Tests must not write to `~/.jarviscopilot/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `HERMES_HOME` to a temp dir. Never hardcode `~/.jarviscopilot/` paths in tests.
...
| HOME / `~/.jarviscopilot/` | Your real config+auth.json | Temp dir per test |
