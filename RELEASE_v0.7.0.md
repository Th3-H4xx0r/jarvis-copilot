...
- **ACP: Client-Provided MCP Servers** — Editor integrations (VS Code, Zed, JetBrains) can now register their own MCP servers, which JarvisCopilot picks up as additional agent tools. Your editor's MCP ecosystem flows directly into the agent. ([#4705](https://github.com/NousResearch/hermes-agent/pull/4705))
...
- **MiniMax M2.7** added to jarviscopilot model picker and OpenCode ([#4208](https://github.com/NousResearch/hermes-agent/pull/4208))
...
- **Unified setup wizard provider selection** with `jarviscopilot model` — single code path for both flows ([#4200](https://github.com/NousResearch/hermes-agent/pull/4200))
...
- **`--max-turns` CLI flag** for `jarviscopilot chat` to limit agent iterations ([#4314](https://github.com/NousResearch/hermes-agent/pull/4314))
...
- **Claude Code credentials gated** behind explicit JarvisCopilot config in wizard trigger ([#4210](https://github.com/NousResearch/hermes-agent/pull/4210))
...
### Update System
- **Fork detection and upstream sync** in `jarviscopilot update` ([#4744](https://github.com/NousResearch/hermes-agent/pull/4744))
...
- **Handle conflicted git index** during jarviscopilot update ([#4735](https://github.com/NousResearch/hermes-agent/pull/4735))
...
### Browser
- **Camofox anti-detection browser backend** — local stealth browsing with auto-install via `jarviscopilot tools` ([#4008](https://github.com/NousResearch/hermes-agent/pull/4008))
...
- **Client-provided MCP servers** registered as agent tools — editors pass their MCP servers to JarvisCopilot ([#4705](https://github.com/NousResearch/hermes-agent/pull/4705))
...
- **MCP OAuth server** could block JarvisCopilot startup instead of degrading gracefully ([#4757](https://github.com/NousResearch/hermes-agent/pull/4757), closes [#4462](https://github.com/NousResearch/hermes-agent/issues/4462))
