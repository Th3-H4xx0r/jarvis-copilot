...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.jarviscopilot/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. JarvisCopilot upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "jarviscopilot",
                    "$device": "jarviscopilot",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.jarviscopilot/.update_response`` for the
          detached ``jarviscopilot update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``jarviscopilot update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `jarviscopilot setup gateway` / `jarviscopilot gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``jarviscopilot update --gateway`` watcher. Button
        ...
        ``~/.jarviscopilot/.update_response`` so the detached update process
        can read it.
        """
