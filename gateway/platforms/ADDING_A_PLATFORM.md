...
There are two ways to add a platform to the JarvisCopilot gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.jarviscopilot/plugins/` (or under `plugins/platforms/`
...
**zero changes to core JarvisCopilot code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `jarviscopilot gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the JarvisCopilot core.
