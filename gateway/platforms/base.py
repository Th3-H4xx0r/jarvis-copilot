...
# Audio file extensions JarvisCopilot recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through JarvisCopilot' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. JarvisCopilot-created Telegram private-chat
...
from jarviscopilot_constants import get_hermes_dir, get_hermes_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.jarviscopilot/.env manually."
)
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    existing regular files under JarvisCopilot-managed media caches, or roots the
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from jarviscopilot_cli.commands import should_bypass_active_session
