...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.jarviscopilot/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields JarvisCopilot manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your JarvisCopilot session")
        ...
        @tree.command(name="status", description="Show JarvisCopilot session status")
        ...
        @tree.command(name="stop", description="Stop the running JarvisCopilot agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.jarviscopilot/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update JarvisCopilot to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the JarvisCopilot gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a JarvisCopilot session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to JarvisCopilot in the thread",
        ...
        # jarviscopilot_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from jarviscopilot_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from jarviscopilot_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a JarvisCopilot skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from jarviscopilot_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new JarvisCopilot session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by JarvisCopilot: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "JarvisCopilot"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by JarvisCopilot: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "JarvisCopilot session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 JarvisCopilot handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ JarvisCopilot needs your input",
                description=body,
                color=discord.Color.orange(),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``jarviscopilot update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``jarviscopilot update`` prompts.
...
def interactive_setup() -> None:
    ...
    from jarviscopilot_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``jarviscopilot_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import jarviscopilot_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # jarviscopilot_cli/setup.py::_setup_discord function.  Same shape as Teams.
