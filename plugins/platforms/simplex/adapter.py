"""SimpleX Chat platform adapter (JarvisCopilot plugin).
...
This adapter ships as a JarvisCopilot platform plugin under
``plugins/platforms/simplex/``. The JarvisCopilot plugin loader scans the
...
discoverable and `jarviscopilot setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside JarvisCopilot — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``jarviscopilot setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.jarviscopilot/.env`` via ``jarviscopilot_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("jarviscopilot_cli.config not available; set SIMPLEX_* vars manually in ~/.jarviscopilot/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `jarviscopilot gateway status` without
