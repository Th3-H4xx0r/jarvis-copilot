...
# ``model_tools`` import time, so ``jarviscopilot status``, ``jarviscopilot chat``, etc.
...
#     fresh "JarvisCopilot is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a jarviscopilot MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _hermes_home = _Path.home() / ".jarviscopilot"
        ...
        # they don't sit in the chat forever as "JarvisCopilot is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HERMES_HOME", str(_Path.home() / ".jarviscopilot"))
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a jarviscopilot MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'JarvisCopilot is thinking…' marker message.
        ...
        "JarvisCopilot is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "JarvisCopilot is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "JarvisCopilot is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "JarvisCopilot is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "JarvisCopilot is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``jarviscopilot setup``.

    The setup wizard at ``jarviscopilot_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.jarviscopilot/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.jarviscopilot/.env")
    print_info("Restart the gateway: jarviscopilot gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'jarviscopilot[google_chat]'",
        ...
        # the "JarvisCopilot is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'JarvisCopilot is thinking…' marker message "
