...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated JarvisCopilot instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active JarvisCopilot profile.

    Resolution order:
      1. HERMES_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``jarviscopilot.<profile>``
      3. Fallback: ``"jarviscopilot"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.jarviscopilot/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    host: str = HOST
    workspace_id: str = "jarviscopilot"
    ...
    # Max chars of dialectic result to inject into JarvisCopilot system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.jarviscopilot
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active JarvisCopilot profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.jarviscopilot block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. JarvisCopilot session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — JarvisCopilot session_id ({timestamp}_{hex})
        ...
        # per-session: inherit JarvisCopilot session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'jarviscopilot honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run jarviscopilot honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `jarviscopilot honcho setup` to configure)."
        )
