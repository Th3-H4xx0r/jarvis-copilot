...
|-----|---------|-------------|
| `container_tag` | `jarviscopilot` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `jarviscopilot-{identity}` → `jarviscopilot-coder`). |
...
When enabled, JarvisCopilot can:
...
Use `{identity}` in the `container_tag` to scope memories per JarvisCopilot profile:

```json
{
  "container_tag": "jarviscopilot-{identity}"
}
```

For a profile named `coder`, this resolves to `jarviscopilot-coder`. The default profile resolves to `jarviscopilot-default`. Without `{identity}`, all profiles share the same container.
