"""langfuse — JarvisCopilot plugin for Langfuse observability.

Traces JarvisCopilot conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the JarvisCopilot plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``jarviscopilot plugins enable
observability/langfuse``, or by checking the box in the interactive
``jarviscopilot plugins`` UI). At runtime the plugin also requires the
``langfuse`` SDK and credentials; if either is missing the hooks are inert.

Required env vars (set in ~/.jarviscopilot/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the JarvisCopilot plugin system —
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="JarvisCopilot turn",
                tags=["jarviscopilot", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="JarvisCopilot turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older JarvisCopilot branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current JarvisCopilot also has a turn-scoped
    ...
    # messages is a list only for legacy JarvisCopilot branches that fired
    # pre_llm_call with API messages directly. Current JarvisCopilot fires
...
def register(ctx) -> None:
    ...
    # JarvisCopilot versions.  pre_api_request / post_api_request fire per API
