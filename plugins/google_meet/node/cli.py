"""`jarviscopilot meet node ...` subcommand tree.

Wired into the existing ``jarviscopilot meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``jarviscopilot meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``jarviscopilot meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             jarviscopilot meet node approve <name> ws://<host>:{args.port} {token}")
        try:
