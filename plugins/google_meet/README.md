...
Let the jarviscopilot agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where jarviscopilot runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `jarviscopilot meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `jarviscopilot meet` CLI |
...
| `cli.py` | `jarviscopilot meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `jarviscopilot meet node {run,list,approve,remove,status,ping}` |
...
jarviscopilot meet install                                      # pip + Chromium
jarviscopilot meet setup                                        # preflight
jarviscopilot meet auth                                         # optional
jarviscopilot meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
jarviscopilot meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.jarviscopilot/.env
jarviscopilot meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
jarviscopilot meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
jarviscopilot meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.jarviscopilot/.env
jarviscopilot meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, jarviscopilot will **not** switch your system audio input automatically — the
...
jarviscopilot plugins enable google_meet
jarviscopilot meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
jarviscopilot meet node approve my-mac ws://<mac-ip>:18789 <token>
jarviscopilot meet node ping my-mac
