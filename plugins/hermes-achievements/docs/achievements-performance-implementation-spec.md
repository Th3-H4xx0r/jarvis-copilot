# JarvisCopilot Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.jarviscopilot/plugins/hermes-achievements/state.json` (existing unlock state)
  - `~/.jarviscopilot/plugins/hermes-achievements/scan_snapshot.json` (new)
  - `~/.jarviscopilot/plugins/hermes-achievements/scan_checkpoint.json` (new)
