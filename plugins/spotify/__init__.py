...
``_check_spotify_available()`` — when the user has not run ``jarviscopilot auth
spotify``, the tools remain registered (so they appear in ``jarviscopilot
...
The Spotify auth flow (``jarviscopilot auth spotify``), CLI plumbing, and docs
