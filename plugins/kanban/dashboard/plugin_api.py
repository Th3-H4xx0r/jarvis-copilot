...
``jarviscopilot_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``jarviscopilot_cli/web_server.py``.

This means ``jarviscopilot dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from jarviscopilot_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``jarviscopilot_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from jarviscopilot_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from jarviscopilot_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``jarviscopilot kanban
...
# spawn failures, stuck-blocked). See jarviscopilot_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``jarviscopilot kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``jarviscopilot kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``jarviscopilot kanban specify <task_id>``.
    ...
    try:
        ...
        from jarviscopilot_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``jarviscopilot kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.jarviscopilot/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.jarviscopilot/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from jarviscopilot_constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``jarviscopilot profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from jarviscopilot_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``jarviscopilot kanban decompose <task_id>``.
    ...
    try:
        os.environ["HERMES_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from jarviscopilot_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.jarviscopilot/config.yaml.
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in jarviscopilot_cli/web_server.py.
