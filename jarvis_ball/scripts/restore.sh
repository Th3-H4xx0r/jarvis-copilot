#!/usr/bin/env bash
# Put a ball's factory firmware back: scripts/restore.sh ~/.jarvis_ball/backups/<mac>.bin
set -euo pipefail
...
[ -n "$port" ] || { echo "No ball on USB" >&2; exit 1; }
python -m esptool --chip esp32s3 --port "$port" -b 921600 write-flash 0 "$backup"
echo "restored $backup to the ball on $port"
