...
void WifiBoard::StartNetwork() {
    ...
    config.ssid_prefix = "Jarvis";  // JARVIS: DHCP hostname Jarvis-XXXX
...
void WifiBoard::OnWifiConnectTimeout(void* arg) {
    ...
    // JARVIS: keep the station retrying; setup is the app's QR flow, never an open hotspot.
    ESP_LOGW(TAG, "WiFi connection timeout, still retrying");
    board->StartWifiConfigMode();
}

void WifiBoard::StartWifiConfigMode() {
    // JARVIS: a paired ball with no reachable network says so and keeps trying.
    Application::GetInstance().OnWifiUnavailable();
    return;
    in_config_mode_ = true;
