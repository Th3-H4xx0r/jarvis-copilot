...
#include "jarvis/board_caps.h"  // JARVIS
...
private:
    ...
    int64_t boot_down_ms_ = 0;       // JARVIS
    static inline int last_x_ = 0;   // JARVIS: last touch position (the timer callback is static)
    static inline int last_y_ = 0;
    ...
    void InitializePowerSaveTimer() {
        ...
        // JARVIS: dim after 60 s but never auto power-off — that would kill the wake word.
    ...
    static void touchpad_timer_callback(void* arg) {
        ...
        // 检测触摸释放
            ...
            if (touch_duration < TOUCH_THRESHOLD_MS) {
                // JARVIS: taps go to the Jarvis UI (menu, back, orb, page onTap).
                Application::GetInstance().OnTouchTap(last_x_, last_y_);
            }
        }
        if (touch_point.num > 0) {  // JARVIS: remember where the finger was
            last_x_ = touch_point.x;
            last_y_ = touch_point.y;
        }
    }
    ...
    // JARVIS: single = voice / next menu item, double = menu, long = back / select,
    // held ≥ 5 s = factory reset.
    void InitializeButtons() {
        boot_button_.OnClick([]() { Application::GetInstance().OnButtonClick(); });
        boot_button_.OnDoubleClick([]() { Application::GetInstance().OnButtonDoubleClick(); });
        boot_button_.OnLongPress([]() { Application::GetInstance().OnButtonLongPress(); });
        boot_button_.OnPressDown([this]() { boot_down_ms_ = esp_timer_get_time() / 1000; });
        boot_button_.OnPressUp([this]() {
            if (boot_down_ms_ && esp_timer_get_time() / 1000 - boot_down_ms_ >= 5000) {
                Application::GetInstance().OnFactoryReset();
            }
            boot_down_ms_ = 0;
        });
    }
...
public:
    Spotpear_ESP32_S3_1_28_BOX() : boot_button_(BOOT_BUTTON_GPIO) {
        ...
        // 初始化音频I2C
        InitializeCodecI2c();
        // JARVIS: hardware check — the new revision has an ES8311 at 0x18; the old one doesn't.
        jarvis::Caps().codec_present = i2c_master_probe(codec_i2c_bus_, AUDIO_CODEC_ES8311_ADDR >> 1, 200) == ESP_OK ||
                                       i2c_master_probe(codec_i2c_bus_, AUDIO_CODEC_ES8311_ADDR, 200) == ESP_OK;
        jarvis::Caps().touch_present = cst816d_ != nullptr;
