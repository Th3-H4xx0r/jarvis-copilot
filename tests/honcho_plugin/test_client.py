...
from jarviscopilot_cli.profiles import _get_default_hermes_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "jarviscopilot"
        assert config.workspace_id == "jarviscopilot"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.jarviscopilot/honcho.json
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"jarviscopilot": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"jarviscopilot": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"jarviscopilot": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="jarviscopilot"
        ):
            result = config.resolve_session_name("/home/user/jarviscopilot/subdir")
        assert result == "jarviscopilot"
...
class TestResolveConfigPath:
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: HERMES_HOME points at ~/.jarviscopilot/profiles/<name>, so
        # _get_default_hermes_home() must resolve back to ~/.jarviscopilot — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".jarviscopilot"
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".jarviscopilot".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".jarviscopilot"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=True):
            ...
            os.environ.pop("HERMES_HOME", None)
            assert resolve_active_host() == "jarviscopilot"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HERMES_HONCHO_HOST": "jarviscopilot.coder"}):
            assert resolve_active_host() == "jarviscopilot.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("jarviscopilot_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "jarviscopilot.coder"

    def test_default_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("jarviscopilot_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "jarviscopilot"

    def test_custom_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("jarviscopilot_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "jarviscopilot"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            # Temporarily remove jarviscopilot_cli.profiles to simulate import failure
            saved = sys.modules.get("jarviscopilot_cli.profiles")
            sys.modules["jarviscopilot_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "jarviscopilot"
            finally:
                if saved is not None:
                    sys.modules["jarviscopilot_cli.profiles"] = saved
                else:
                    sys.modules.pop("jarviscopilot_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jarviscopilot.coder")
        assert config.host == "jarviscopilot.coder"
        assert config.workspace_id == "jarviscopilot"  # shared workspace
        assert config.ai_peer == "jarviscopilot.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jarviscopilot")
        assert config.host == "jarviscopilot"
        assert config.workspace_id == "jarviscopilot"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "jarviscopilot": {"aiPeer": "jarviscopilot", "peerName": "alice"},
                "jarviscopilot.coder": {
                    "aiPeer": "jarviscopilot.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="jarviscopilot.coder", config_path=config_file,
        )
        assert config.host == "jarviscopilot.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "jarviscopilot.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="jarviscopilot.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "jarviscopilot.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jarviscopilot": {"enabled": True, "aiPeer": "jarviscopilot"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jarviscopilot": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
    ...
    def test_explicit_unified_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            ...
            "hosts": {"jarviscopilot": {"enabled": True}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "unified"
...
class TestGetHonchoClient:
    ...
    def test_hermes_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("jarviscopilot_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_hermes_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("jarviscopilot_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"jarviscopilot": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
