"""Tests for ACP Registry metadata shipped with JarvisCopilot."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "jarviscopilot"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "JarvisCopilot"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/jarviscopilot"
    assert data["website"].startswith("https://jarviscopilot.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"jarviscopilot[acp]=={data['version']}"
    assert uvx["args"] == ["jarviscopilot-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"jarviscopilot[acp]=={_pyproject_version()}"
    )
