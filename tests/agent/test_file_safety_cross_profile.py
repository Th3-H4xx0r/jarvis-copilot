"""Tests for the cross-JarvisCopilot-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another JarvisCopilot profile's
...
accidentally edited skills under both ~/.jarviscopilot/profiles/hermes-security/skills/
AND ~/.jarviscopilot/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake JarvisCopilot root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    """Build a fake JarvisCopilot layout:
    ...
    monkeypatch.setattr(jarviscopilot_constants, "get_default_hermes_root", lambda: root)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_non_hermes_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any JarvisCopilot root
