...
def test_jarviscopilot_client_tag_includes_current_version():
    """The client tag must reflect jarviscopilot_cli.__version__ verbatim."""
    from jarviscopilot_cli import __version__
    from agent.portal_tags import jarviscopilot_client_tag

    assert jarviscopilot_client_tag() == f"client=hermes-client-v{__version__}"


def test_jarviscopilot_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import jarviscopilot_client_tag

    tag = jarviscopilot_client_tag()
