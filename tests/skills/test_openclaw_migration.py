...
"""
    ...
    assert "Tyler Williams > Active Projects: JarvisCopilot" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `jarviscopilot cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.jarviscopilot/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``JarvisCopilot``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "JarvisCopilot prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told JarvisCopilot to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "JarvisCopilot config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "JarvisCopilot uses tools well"
    # All-lowercase matches → lowercase ``jarviscopilot``; this preserves the
    # real filesystem path ``~/.jarviscopilot`` (JarvisCopilot home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "jarviscopilot should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "JarvisCopilot remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "jarviscopilot prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "JarvisCopilot was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "jarviscopilot uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "JarvisCopilot said to ask JarvisCopilot about JarvisCopilot settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.jarviscopilot`` (the real JarvisCopilot home), not the broken
    ``.JarvisCopilot``.
    ...
    rewritten to ``~/.JarvisCopilot/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.jarviscopilot/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .jarviscopilot directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.jarviscopilot'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "jarviscopilot config path: ~/.jarviscopilot/"
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    target_root = tmp_path / "jarviscopilot"
    target_root.mkdir()
    ...
    assert "JarvisCopilot" in result
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are JarvisCopilot" in result
