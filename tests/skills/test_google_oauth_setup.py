...
class TestHermesConstantsFallback:
    """Tests for _hermes_home.py fallback when jarviscopilot_constants is unavailable."""
    ...
    def _load_helper(self, monkeypatch):
        """Load _hermes_home.py with jarviscopilot_constants blocked."""
        monkeypatch.setitem(sys.modules, "jarviscopilot_constants", None)
    ...
    def test_fallback_uses_hermes_home_env_var(self, monkeypatch, tmp_path):
        """When jarviscopilot_constants is missing, HERMES_HOME comes from env var."""
    ...
    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When jarviscopilot_constants is missing and HERMES_HOME unset, default to ~/.jarviscopilot."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == Path.home() / ".jarviscopilot"
    ...
    def test_fallback_display_hermes_home_shortens_path(self, monkeypatch):
        ...
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.jarviscopilot"
    ...
    def test_fallback_display_hermes_home_profile_path(self, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(Path.home() / ".jarviscopilot/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.jarviscopilot/profiles/coder"
    ...
    def test_delegates_to_jarviscopilot_constants_when_available(self):
        """When jarviscopilot_constants IS importable, _hermes_home delegates to it."""
        ...
        assert module.get_hermes_home is jarviscopilot_constants.get_hermes_home
        assert module.display_hermes_home is jarviscopilot_constants.display_hermes_home
