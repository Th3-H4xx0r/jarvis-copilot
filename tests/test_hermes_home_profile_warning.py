...
Regression test for https://github.com/NousResearch/jarviscopilot/issues/18594.

When HERMES_HOME is unset but an active_profile file indicates a non-default
profile is active, get_hermes_home() should:
  1. STILL return ~/.jarviscopilot (raising would brick 30+ module-level callers)
...
def fresh_constants(monkeypatch, tmp_path):
    """Import jarviscopilot_constants fresh and reset the one-shot warn flag."""
    import importlib
    import jarviscopilot_constants
    importlib.reload(jarviscopilot_constants)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("HERMES_HOME", raising=False)
    return jarviscopilot_constants
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.jarviscopilot."""
    ...
    def test_default_active_profile_no_warning(
        ...
        """active_profile=default → still no warning, returns ~/.jarviscopilot."""
        hermes_dir = tmp_path / ".jarviscopilot"
        hermes_dir.mkdir()
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        (tmp_path / ".jarviscopilot" / "active_profile").write_text("coder\n")
