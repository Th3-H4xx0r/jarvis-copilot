...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"jarviscopilot[termux]"' in text
    assert '"jarviscopilot[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"jarviscopilot[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
