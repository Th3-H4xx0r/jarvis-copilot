"""
Tests for timezone support (jarviscopilot_time module + integration points).
...
import jarviscopilot_time


def _reset_jarviscopilot_time_cache():
    """Reset the jarviscopilot_time module cache (replacement for removed reset_cache)."""
    jarviscopilot_time._cached_tz = None
    jarviscopilot_time._cached_tz_name = None
    jarviscopilot_time._cache_resolved = False


# =========================================================================
# jarviscopilot_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def setup_method(self):
        _reset_jarviscopilot_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["jarviscopilot_timeZONE"] = "Asia/Kolkata"
        result = jarviscopilot_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["jarviscopilot_timeZONE"] = "UTC"
        result = jarviscopilot_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["jarviscopilot_timeZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="jarviscopilot_time"):
            result = jarviscopilot_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["jarviscopilot_timeZONE"] = "UTC"
        _reset_jarviscopilot_time_cache()
        r1 = jarviscopilot_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["jarviscopilot_timeZONE"] = "Asia/Kolkata"
        _reset_jarviscopilot_time_cache()
        r2 = jarviscopilot_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["jarviscopilot_timeZONE"] = "Europe/London"
        tz = jarviscopilot_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["jarviscopilot_timeZONE"] = "Not/A/Timezone"
        tz = jarviscopilot_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When jarviscopilot_timeZONE is set, child process sees TZ env var.
        ...
        #   2) jarviscopilot_timeZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("jarviscopilot_timeZONE=" + os.environ.get("jarviscopilot_timeZONE", "NOT_SET"))'
        )
        ...
        assert "jarviscopilot_timeZONE=NOT_SET" in result["output"], (
            "jarviscopilot_timeZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When jarviscopilot_timeZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("jarviscopilot_timeZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != JarvisCopilot tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to JarvisCopilot tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in JarvisCopilot tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of JarvisCopilot tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), JarvisCopilot is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when JarvisCopilot tz
        ...
        # Use a JarvisCopilot timezone far behind UTC so that the numeric wall time
        ...
        os.environ["jarviscopilot_timeZONE"] = "Pacific/Midway"  # UTC-11
        _reset_jarviscopilot_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of JarvisCopilot timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["jarviscopilot_timeZONE"] = "US/Eastern"
        _reset_jarviscopilot_time_cache()
