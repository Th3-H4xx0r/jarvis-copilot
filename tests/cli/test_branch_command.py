...
def session_db(tmp_path):
    ...
    os.environ["HERMES_HOME"] = str(tmp_path / ".jarviscopilot")
    os.makedirs(tmp_path / ".jarviscopilot", exist_ok=True)
    from jarviscopilot_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".jarviscopilot" / "test_sessions.db")
    yield db
    db.close()
