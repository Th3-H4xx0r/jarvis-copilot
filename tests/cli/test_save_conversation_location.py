...
file to be discoverable via ``jarviscopilot sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.jarviscopilot/sessions/saved/`` and prints
...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(jarviscopilot_constants, "_hermes_home_cache"):
        jarviscopilot_constants._hermes_home_cache = None
    return home
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.jarviscopilot/sessions/saved/, not CWD."""
    ...
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "jarviscopilot_constants"]:
    ...
    # File MUST be under ~/.jarviscopilot/sessions/saved/
    ...
    assert "jarviscopilot --resume 20260101_120000_abc123" in out, out
