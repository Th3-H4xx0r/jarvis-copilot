...
Run from the PR worktree:
    ...
    cd ~/.jarviscopilot/jarviscopilot/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.jarviscopilot/jarviscopilot``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.jarviscopilot/jarviscopilot
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a jarviscopilot checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a jarviscopilot checkout"
)
