...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_hermes_config(self, monkeypatch):
        ...
        # Drop any cached import of jarviscopilot_cli.config.
        sys.modules.pop("jarviscopilot_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "jarviscopilot_cli.config" not in sys.modules, (
            "langfuse plugin imported jarviscopilot_cli.config — regression toward "
...
# JarvisCopilot logs.  The fix in `_get_langfuse()` validates the documented
