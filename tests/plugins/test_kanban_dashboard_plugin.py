...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (False, "No gateway is running — start `jarviscopilot gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
# tests/jarviscopilot_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_board_endpoint_survives_task_age_exception(client, monkeypatch):
    ...
    monkeypatch.setattr("jarviscopilot_cli.kanban_db.task_age", _boom)
