"""Tests for `jarviscopilot update --yes / -y` — assume yes for interactive prompts.
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "status" in joined and "--porcelain" in joined:
            out = " M jarviscopilot_cli/main.py\n" if dirty else ""
...
class TestUpdateYesConfigMigration:
    ...
    @patch("jarviscopilot_cli.config.migrate_config")
    @patch("jarviscopilot_cli.config.check_config_version", return_value=(1, 2))
    @patch("jarviscopilot_cli.config.get_missing_config_fields", return_value=[])
    @patch("jarviscopilot_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``jarviscopilot_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``jarviscopilot_cli.main`` first could leave
