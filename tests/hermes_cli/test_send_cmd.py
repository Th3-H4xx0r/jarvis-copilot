"""Tests for the ``jarviscopilot send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``jarviscopilot_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from jarviscopilot_cli import send_cmd
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``jarviscopilot send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``jarviscopilot config set``) lives
    ...
    import jarviscopilot_cli.config as _hc_config
    reload(_hc_config)
