"""Tests for the update check mechanism in jarviscopilot_cli.banner."""
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    with patch("jarviscopilot_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import jarviscopilot_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "jarviscopilot_cli" / "banner.py"
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    with patch("jarviscopilot_cli.banner.subprocess.run") as mock_run:
        with patch("jarviscopilot_cli.banner.check_via_pypi", return_value=0):
