"""Tests for --ignore-user-config and --ignore-rules flags on `jarviscopilot chat`.
...
files. In JarvisCopilot the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.jarviscopilot/config.yaml`` in ``load_cli_config()``
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``jarviscopilot_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real jarviscopilot parser accepts both flags."""
        ...
        import inspect
        import jarviscopilot_cli.main as hm
