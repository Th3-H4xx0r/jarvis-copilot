...
``jarviscopilot_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``jarviscopilot --help``, ``jarviscopilot version``,
``jarviscopilot logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from jarviscopilot_cli.main import (
...
def _live_subcommand_names() -> set[str]:
    """Run ``jarviscopilot --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from jarviscopilot_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["jarviscopilot", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        (["jarviscopilot"], None),
        (["jarviscopilot", "--help"], None),
        (["jarviscopilot", "-h"], None),
        (["jarviscopilot", "--version"], None),
        (["jarviscopilot", "-w"], None),
        ...
        (["jarviscopilot", "-w", "--tui"], None),
        (["jarviscopilot", "version"], "version"),
        (["jarviscopilot", "--tui", "chat"], "chat"),
        (["jarviscopilot", "-w", "logs"], "logs"),
        (["jarviscopilot", "chat", "hello world"], "chat"),
        (["jarviscopilot", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["jarviscopilot", "-m", "gpt5", "chat"], "chat"),
        (["jarviscopilot", "--model", "gpt5", "chat", "hi"], "chat"),
        (["jarviscopilot", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["jarviscopilot", "-z", "hello world"], None),
        (["jarviscopilot", "-z", "hello", "chat"], "chat"),
        (["jarviscopilot", "--model=gpt5", "chat"], "chat"),     # inline form
        (["jarviscopilot", "--", "chat"], "chat"),               # -- terminator
        (["jarviscopilot", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["jarviscopilot", "some-plugin-cmd"], "some-plugin-cmd"),
        (["jarviscopilot", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["jarviscopilot"],                          # bare → chat
        ["jarviscopilot", "--help"],                # top-level help
        ["jarviscopilot", "-h"],
        ["jarviscopilot", "version"],               # known built-in
        ["jarviscopilot", "logs"],
        ["jarviscopilot", "gateway", "run"],
        ["jarviscopilot", "--tui"],
        ["jarviscopilot", "-w", "--tui"],
        ["jarviscopilot", "chat", "hi"],
        ["jarviscopilot", "help"],                  # accepted built-in-ish
        ["jarviscopilot", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["jarviscopilot", "meet", "join"],          # potential google_meet plugin
        ["jarviscopilot", "honcho", "status"],      # potential memory plugin
        ["jarviscopilot", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``jarviscopilot help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"jarviscopilot_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
