"""Comprehensive tests for jarviscopilot_cli.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.jarviscopilot/profiles)
    * HERMES_HOME  -> tmp_path/.jarviscopilot  (so get_hermes_home() agrees)
    * Creates the bare-minimum ~/.jarviscopilot directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["jarviscopilot", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the JarvisCopilot install itself or with
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.jarviscopilot must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".jarviscopilot"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes jarviscopilot, .worktrees,
        ...
        (default_home / "jarviscopilot" / ".git").mkdir(parents=True)
        (default_home / "jarviscopilot" / "venv" / "bin").mkdir(parents=True)
        (default_home / "jarviscopilot" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "jarviscopilot").exists()
...
class TestNoSkillsOptOut:
    """Tests for `jarviscopilot profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `jarviscopilot update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("jarviscopilot_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".jarviscopilot" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_hermes_home_returns_default(self, profile_env):
        # HERMES_HOME points to tmp_path/.jarviscopilot which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".jarviscopilot" / "profiles" / "coder"
...
class TestResolveProfileEnv:
    ...
    def test_existing_profile_returns_path(self, profile_env):
        ...
        assert result == str(tmp_path / ".jarviscopilot" / "profiles" / "coder")

    def test_default_returns_default_home(self, profile_env):
        tmp_path = profile_env
        result = resolve_profile_env("default")
        assert result == str(tmp_path / ".jarviscopilot")
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("jarviscopilot")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".jarviscopilot" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("jarviscopilot_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".jarviscopilot" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".jarviscopilot" / "honcho.json"
        honcho_path.write_text(json.dumps({
            "hosts": {
                "jarviscopilot.ssi_health": {
                    "recallMode": "hybrid",
        ...
        assert "jarviscopilot.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["jarviscopilot.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jarviscopilot.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "jarviscopilot.ssi_health": {"workspace": "jarviscopilot", "enabled": True}
            }
        }))
        ...
        assert "jarviscopilot.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["jarviscopilot.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jarviscopilot.heimdall"]["workspace"] == "jarviscopilot"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "jarviscopilot.ssi_health": {"aiPeer": "ssi_health"},
                "jarviscopilot.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["jarviscopilot.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jarviscopilot.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("jarviscopilot", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/jarviscopilot", "default/.worktrees", "default/profiles",
...
class TestInternalHelpers:
    ...
    def test_profiles_root_under_home(self, profile_env):
        ...
        assert root == tmp_path / ".jarviscopilot" / "profiles"

    def test_default_hermes_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_hermes_home()
        assert home == tmp_path / ".jarviscopilot"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (HERMES_HOME outside ~/.jarviscopilot), profiles go under HERMES_HOME."""
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (HERMES_HOME under ~/.jarviscopilot), profiles root is still ~/.jarviscopilot/profiles."""
        native = tmp_path / ".jarviscopilot"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        ...
        from jarviscopilot_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".jarviscopilot" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from jarviscopilot_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".jarviscopilot"
