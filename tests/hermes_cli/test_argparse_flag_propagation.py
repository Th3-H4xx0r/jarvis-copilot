...
subparser, placing the flag BEFORE the subcommand (e.g. 'jarviscopilot --yolo chat')
...
def _build_parser():
    """Build the jarviscopilot argument parser from the real code.
    ...
    """
    parser = argparse.ArgumentParser(prog="jarviscopilot")
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("jarviscopilot_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "jarviscopilot_cli.banner", fake_banner)
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `jarviscopilot gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `jarviscopilot <argv>` must exit 0 (help) rather than
