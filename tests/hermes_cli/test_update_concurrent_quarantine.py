...
from jarviscopilot_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_hermes_instances
# helper (and need the autouse stub in tests/jarviscopilot_cli/conftest.py disabled),
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    ...
    assert "JarvisCopilot Desktop" in msg
    assert "--force" in msg
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "JarvisCopilot Desktop" in captured or "gateway" in captured.lower()
