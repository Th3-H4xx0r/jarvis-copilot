...
def main_mod(monkeypatch):
    import jarviscopilot_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["jarviscopilot", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    assert "JarvisCopilot Agent v" in out
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["jarviscopilot", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "jarviscopilot_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from jarviscopilot_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "jarviscopilot -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """jarviscopilot -z bypasses HermesCLI, but recall still needs SessionDB."""
    from jarviscopilot_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "jarviscopilot_state", mod("jarviscopilot_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "jarviscopilot_cli.config",
        mod("jarviscopilot_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "jarviscopilot_cli.models",
        mod("jarviscopilot_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "jarviscopilot_cli.runtime_provider",
        mod(
            "jarviscopilot_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "jarviscopilot_cli.tools_config",
        mod("jarviscopilot_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("jarviscopilot_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "jarviscopilot_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "jarviscopilot --tui --resume 20260409_000001_abc123" in out
    assert 'jarviscopilot --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "jarviscopilot --tui --resume actual_session" in out
