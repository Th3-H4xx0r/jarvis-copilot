"""Tests for jarviscopilot_cli.doctor."""
...
import jarviscopilot_cli.doctor as doctor
import jarviscopilot_cli.gateway as gateway_cli
from jarviscopilot_cli import doctor as doctor_mod
from jarviscopilot_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `jarviscopilot doctor` crashed on Windows
...
class TestGitHubTokenCheck:
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from jarviscopilot_cli.doctor import run_doctor, _DHH
        import io, contextlib
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from jarviscopilot_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("jarviscopilot_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "jarviscopilot_cli.doctor", raising=False)
        from jarviscopilot_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
