"""Tests for jarviscopilot_cli configuration management."""
...
class TestGetHermesHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".jarviscopilot"
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``jarviscopilot logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from jarviscopilot_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="jarviscopilot_cli.config"):
                config = load_config()
            ...
            # stands out at jarviscopilot startup before logging is configured)
            captured = capsys.readouterr()
            assert "jarviscopilot config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            assert "jarviscopilot config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            assert "jarviscopilot config:" in after_edit, "edited file should re-warn"
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from jarviscopilot_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
