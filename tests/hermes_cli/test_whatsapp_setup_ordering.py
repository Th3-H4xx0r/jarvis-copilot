...
Before the fix, ``jarviscopilot whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``jarviscopilot gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    jarviscopilot = home / ".jarviscopilot"
    jarviscopilot.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("HERMES_HOME", str(jarviscopilot))
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("jarviscopilot_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``jarviscopilot whatsapp`` with an existing paired session and
