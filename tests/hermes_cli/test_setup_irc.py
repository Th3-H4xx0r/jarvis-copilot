"""Tests for IRC gateway configuration via `jarviscopilot setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh JarvisCopilot install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new JarvisCopilot install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#jarviscopilot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `jarviscopilot setup gateway` experience with IRC present."""
