...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.jarviscopilot/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.jarviscopilot            (has its own auth.json fixture)
    * Profile     -> tmp_path/.jarviscopilot/profiles/coder   (active, HERMES_HOME points here)
    ...
    global_root = tmp_path / ".jarviscopilot"
    global_root.mkdir()
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    ...
    # sees tmp_path/home/.jarviscopilot as the "real home" — which is NOT equal
    ...
    from jarviscopilot_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: HERMES_HOME is set to a custom path that is NOT under
    # ~/.jarviscopilot/profiles/ — get_default_hermes_root() returns HERMES_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from jarviscopilot_cli.auth import read_credential_pool, write_credential_pool
