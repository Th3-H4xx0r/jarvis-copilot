"""Tests for SIGHUP protection and stdout mirroring in ``jarviscopilot update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``jarviscopilot_cli/main.py``.  These exist so
that ``jarviscopilot update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "jarviscopilot update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "jarviscopilot_cli.config.get_hermes_home", _boom, raising=True
        )
