...
Complements tests/jarviscopilot_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/jarviscopilot_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def test_per_task_max_retries_overrides_dispatcher_limit(kanban_home, all_assignees_spawnable):
    ...
    Three-tier resolution order:
      1. ``task.max_retries`` (set via ``create_task(max_retries=N)`` /
         ``jarviscopilot kanban create --max-retries N``)
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.jarviscopilot/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("HERMES_HOME", raising=False)
    profiles = tmp_path / ".jarviscopilot" / "profiles"
...
def test_parse_duration_accepts_formats():
    from jarviscopilot_cli.kanban import _parse_duration
...
def test_cli_create_max_runtime_via_duration(kanban_home):
    """`jarviscopilot kanban create --max-runtime 2h` should persist 7200 seconds."""
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: jarviscopilot_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "jarviscopilot_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    ...
    r = _sp.run(
        [_sys.executable, "-m", "jarviscopilot_cli.main", "kanban",
...
def test_cli_show_json_carries_runs(kanban_home):
    """jarviscopilot kanban show --json must include runs[] so scripts that
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    jarviscopilot subprocess (which would hang trying to call an LLM).
    """
...
def test_cli_create_skill_flag_repeatable(kanban_home):
    """`jarviscopilot kanban create --skill a --skill b` persists the list."""
    out = run_slash(
...
def test_cli_show_renders_skills(kanban_home):
    """`jarviscopilot kanban show <id>` prints a skills row when present."""
    out = run_slash(
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from jarviscopilot_cli import kanban as kb_cli
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "jarviscopilot gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "jarviscopilot gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "jarviscopilot gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "jarviscopilot gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    """`jarviscopilot kanban daemon` (no --force) is a deprecation stub."""
    from jarviscopilot_cli import kanban as kb_cli
    ns = argparse.Namespace(
    ...
    assert "jarviscopilot gateway start" in err
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `jarviscopilot kanban --help` output"
    )
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import jarviscopilot_cli.config as _cfg_mod
