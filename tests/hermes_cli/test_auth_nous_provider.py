...
from jarviscopilot_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from jarviscopilot_cli.auth import _resolve_verify
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="jarviscopilot_cli.auth")
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jarviscopilot_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jarviscopilot_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("jarviscopilot_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import jarviscopilot_cli.auth as auth_mod
        import jarviscopilot_cli.models as models_mod
        import jarviscopilot_cli.nous_subscription as ns
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    """
    from jarviscopilot_cli.auth import _refresh_access_token
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    ...
    from jarviscopilot_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from jarviscopilot_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from jarviscopilot_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from jarviscopilot_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from jarviscopilot_cli.auth import _try_import_shared_nous_state
