...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`jarviscopilot_cli.skills_config.save_disabled_skills` stores.
"""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `jarviscopilot skills config` writes).
    monkeypatch.setattr(
    ...
    assert "jarviscopilot skills config" in msg
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "jarviscopilot_constants.get_optional_skills_dir",
