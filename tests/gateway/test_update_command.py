...
class TestHandleUpdateCommand:
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and jarviscopilot_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m jarviscopilot_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
