...
def adapter(tmp_path):
    ...
    ~/.jarviscopilot/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.jarviscopilot/...)
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the JarvisCopilot host. The relay flattens Chat
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'JarvisCopilot is thinking…' stuck in chat. Race fix via
    ...
    async def test_orphan_typing_cards_reaped_on_completion(self, adapter):
        ...
        'JarvisCopilot is thinking…' messages."""
        from plugins.platforms.google_chat.adapter import _TYPING_CONSUMED_SENTINEL
...
class TestGoogleChatInteractiveSetup:
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        monkeypatch.setattr("jarviscopilot_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("jarviscopilot_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("jarviscopilot_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "jarviscopilot_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "jarviscopilot_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "jarviscopilot_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "jarviscopilot_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
