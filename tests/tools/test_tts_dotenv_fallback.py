...
TTS provider tools must resolve API keys from ``~/.jarviscopilot/.env`` (via
``jarviscopilot_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``jarviscopilot_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.jarviscopilot/.env`` carries the
    ...
    ``jarviscopilot_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``jarviscopilot_cli.config.load_env`` to simulate ``~/.jarviscopilot/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while jarviscopilot_cli.config.get_env_value is patched must
        ...
        try:
            ...
            with patch(
                "jarviscopilot_cli.config.load_env",
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.jarviscopilot/.env carrying the key (load_env returns the dict
        ...
        with patch(
            ...
            # os.environ is empty.
            from jarviscopilot_cli.config import get_env_value as live_get
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.jarviscopilot/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
