...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        ...
        # ``~/.jarviscopilot``) must be write-denied. The hermetic test conftest
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.jarviscopilot/.env`` (whose credentials are inherited by every profile)
        ...
        from jarviscopilot_constants import get_hermes_home, get_default_hermes_root
...
class TestWriteAllowed:
    ...
    def test_hermes_config_not_env(self):
        path = os.path.join(str(Path.home()), ".jarviscopilot", "config.yaml")
