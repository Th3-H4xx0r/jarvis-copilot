...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `jarviscopilot chat` sessions (no HERMES_KANBAN_TASK) must have
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["jarviscopilot_cli/kanban.py"],
...
# restarting JarvisCopilot. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated JarvisCopilot home with two distinct kanban boards seeded.
