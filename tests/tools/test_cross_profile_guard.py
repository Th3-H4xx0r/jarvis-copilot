...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile JarvisCopilot layout and point HERMES_HOME at
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.jarviscopilot/profiles/<name>/."""
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        ...
        assert "Active JarvisCopilot profile" in src
        assert "cross_profile=True" in src
        assert "~/.jarviscopilot/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active JarvisCopilot profile: default" in src
        assert "Active JarvisCopilot profile: {active_profile}" in src
