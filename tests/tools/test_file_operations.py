...
class TestIsWriteDenied:
    ...
    def test_hermes_control_files_and_mcp_tokens_denied(self, path):
        """JarvisCopilot control files and mcp-tokens entries must be write-denied."""
    ...
    def test_control_files_protected_in_profile_mode(self, tmp_path, monkeypatch, name):
        ...
        global ~/.jarviscopilot/{auth.json,config.yaml,webhook_subscriptions.json}
...
class TestSearchFilesFallbackHiddenPaths:
    ...
    def test_hidden_root_with_hidden_ancestor_includes_files(self, tmp_path, monkeypatch):
        ...
        root = tmp_path / ".jarviscopilot" / "logs"
