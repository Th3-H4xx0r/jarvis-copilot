...
- Signal forwarding runs through the init so ``docker stop`` triggers
  jarviscopilot's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, jarviscopilot accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, jarviscopilot accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into jarviscopilot,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, jarviscopilot "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    # actually carries the package source so ``await import('@jarviscopilot/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@jarviscopilot/ink``)
    ...
    assert "ui-tui/packages/hermes-ink/" in dockerfile_text, (
        ...
        "so ``await import('@jarviscopilot/ink')`` resolves at runtime."
    )
