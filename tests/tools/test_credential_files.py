...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.jarviscopilot/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.jarviscopilot/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.jarviscopilot")

        assert mounts[0]["container_path"] == "/home/user/.jarviscopilot/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.jarviscopilot/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.jarviscopilot/skills/cat/myskill/scripts/run.sh" in paths
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.jarviscopilot/cache/documents" in paths
        assert "/root/.jarviscopilot/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.jarviscopilot/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.jarviscopilot/cache/documents" in container_paths
        assert "/root/.jarviscopilot/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        assert entries[0]["container_path"] == "/root/.jarviscopilot/cache/screenshots/session_abc/screen1.png"
