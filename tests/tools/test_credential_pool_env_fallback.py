...
- _seed_from_env reads API keys from ~/.jarviscopilot/.env when not in os.environ
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.jarviscopilot/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.jarviscopilot/.env, not just os.environ.
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.jarviscopilot/.env."""
