"""contract test: dockerfile chowns runtime node_modules trees to jarviscopilot

regression guard for #18800. the container drops privileges to the jarviscopilot
...
the runtime node_modules trees are owned by jarviscopilot.
"""
...
def test_dockerfile_chowns_runtime_node_modules_to_hermes_user() -> None:
    ...
    chown_lines = [
        ...
        if "chown" in line and "jarviscopilot:jarviscopilot" in line
    ]
    assert chown_lines, (
        "Dockerfile must contain a chown -R jarviscopilot:jarviscopilot for the runtime "
    ...
    # because HERMES_WEB_DIST points at jarviscopilot_cli/web_dist for runtime.
    for required_path in ("/opt/hermes/ui-tui", "/opt/hermes/node_modules"):
        assert required_path in chown_block, (
            f"{required_path} must be passed to a chown -R jarviscopilot:jarviscopilot "
