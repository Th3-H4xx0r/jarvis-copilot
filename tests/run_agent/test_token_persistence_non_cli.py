...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    jarviscopilot_state = ModuleType("jarviscopilot_state")
    jarviscopilot_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "jarviscopilot_state", jarviscopilot_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "JarvisCopilot"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "JarvisCopilot"
