...
def _make_agent(*tool_names: str, max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("jarviscopilot_cli.config.load_config", return_value=config or {}),
...
def test_plugin_pre_tool_block_wins_without_counting_as_toolguard_block():
    ...
    with (
        patch("jarviscopilot_cli.plugins.get_pre_tool_call_block_message", return_value="plugin policy"),
