#!/usr/bin/env ruby
# Register the JarvisWatchWidget WidgetKit extension as an Xcode target and embed it
# in the WATCH app, so the watch face complication ("JARVIS — tap to talk") builds
# and can be added to a face. Mirrors add-watch-widget-target.rb, watchOS settings.
#
...
#
#   ruby scripts/add-watch-widget-target.rb
require 'xcodeproj'
require 'fileutils'
...
APP = 'JarvisWatch'
WIDGET = 'JarvisWatchWidget'
BUNDLE = 'com.jarviscopilot.jarviscopilotMobileAndIOS.watchkitapp.JarvisWatchWidget'
TEAM = 'VY5CNF8734'
DEPLOYMENT = '26.5'
...
def widget_settings
  {
    ...
    'INFOPLIST_FILE' => 'JarvisWatchWidget/Info.plist',
  }
end
...
abort 'add-watch-widget-target: could not acquire lock' unless acquired
...
begin
  ...
  unless widget
    ...
    widget.build_configuration_list = Xcodeproj::Project::ProjectHelper
      .configuration_list(project, :watchos, DEPLOYMENT, widget, :framework)
    ...
    created = true
    warn "add-watch-widget-target: created target #{WIDGET}"
  end
  ...
  ['JarvisWatchWidget/Info.plist'].each do |path|
  ...
  project.save
  puts "add-watch-widget-target: #{created ? 'created' : 'ok'} (#{WIDGET} embedded in #{APP})"
ensure
