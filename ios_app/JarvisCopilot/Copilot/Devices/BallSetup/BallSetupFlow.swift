...
/// Drives pairing a Jarvis Ball: connect to its hotspot → choose Wi-Fi → send credentials →
/// pair → done. Everything external (the ball, iOS's hotspot API, the Jarvis server, time)
/// is injected so the whole flow runs in tests.
@Observable
@MainActor
final class BallSetupFlow {
    enum Step: Int, CaseIterable, Identifiable {
        case connect, wifi, send, pair, done
        var id: Int { rawValue }
        var title: String {
            switch self {
            case .connect: return "Connect to Jarvis Ball"
    ...
    private(set) var info: BallInfo?
    private(set) var networks: [BallNetwork] = []
    ...
    private let client: BallSetupTalking
    private let joiner: HotspotJoining
    private let server: BallServerTalking
    private let sleep: @Sendable (Double) async -> Void
    private let now: @Sendable () -> Date
    private var pairing: BallPairing?
    private var phoneSSID: String?
    private var startedAt = Date.distantPast
    private var cancelled = false
    private var freshCodeTries = 0

    init(code: BallSetupCode,
         client: BallSetupTalking = BallSetupHTTP(),
         joiner: HotspotJoining = SystemHotspotJoiner(),
         server: BallServerTalking = JarvisBallServer(),
    ...
    func start() async {
        ...
        do {
            ...
            return fail(.connect, "Couldn't join \(code.ssid). Is the ball showing its QR code?")
        }
        guard let info = await retrying(seconds: 10, { try await self.client.info() }) else {
            return fail(.connect, "Joined \(code.ssid) but the ball didn't answer.")
        }
    ...
    func loadNetworks() async {
        ...
        if networks.isEmpty { detail[.wifi] = "The ball can't see any networks. Move it closer and refresh." }
    }
    ...
    func submitWifi() async {
        ...
        } catch BallSetupError.rejected(let message) {
            return fail(.send, message)
        } catch {
            return fail(.send, "Lost the ball's hotspot. Retry.")
        }
        statuses[.send] = .done
    ...
    private func pair(ssid: String) async {
        ...
        while now() < deadline {
            ...
            do {
                ...
                // Normal for a moment: the hotspot follows the ball onto the home network's channel.
                misses += 1
        ...
        fail(.pair, "The ball didn't finish pairing. Check its screen.")
    }

    private func handle(failure s: BallSetupStatus, ssid: String) async {
        switch s.error {
        ...
        case "code_rejected" where freshCodeTries < 1:
            ...
            await submitWifi()
        default:
            fail(.pair, s.message.isEmpty ? "The ball reached Wi-Fi but not Jarvis." : s.message)
        }
    }

    private func serverFallback() async {
        detail[.pair] = "Waiting for the ball to reach Jarvis…"
        await joiner.leave(ssid: code.ssid)
        for _ in 0..<30 {
            guard !cancelled else { return }
            if await server.ballOnline(name: code.ballName, since: startedAt) { return await finish(confirmed: true) }
            await sleep(2)
        }
        fail(.pair, "The ball didn't come online. Check its screen.")
    }
    ...
    private func confirmOnline() async {
        statuses[.done] = .active
        detail[.done] = "Waiting for the ball to come online…"
        // The ball reboots after pairing; give it time to rejoin Wi-Fi and open its bridge.
        for _ in 0..<30 {
            guard !cancelled else { return }
            if await server.ballOnline(name: code.ballName, since: startedAt) { return complete() }
            await sleep(2)
        }
        fail(.done, "Paired, but Jarvis hasn't seen the ball online yet.")
    }

    private func complete() {
        statuses[.done] = .done
        detail[.done] = "\(code.ballName) is paired"
        finished = true
    }
    ...
    private func fetchPairing(step: Step = .connect) async -> Bool {
        do {
            pairing = try await server.pairStart(label: code.ballName)
            return true
        } catch {
    ...
    static func setupBody(ssid: String, password: String, server: String, pairing: BallPairing,
        ...
        let body: [String: Any] = [
            ...
            "theme": JarvisBallLook.theme,
            "timezone": timeZone.identifier,
            "tz_posix": JarvisBallLook.posixTZ(timeZone, now: now),
            "clock_24h": JarvisBallLook.clock24h,
        ]
