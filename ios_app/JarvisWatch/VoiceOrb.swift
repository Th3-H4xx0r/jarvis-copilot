...
struct VoiceOrb: View {
    ...
    /// The phone's glass-sphere palette (`OrbShader.metal`): a near-black
    /// interior, deep-blue lit liquid, a cool white-blue fresnel rim, and a
    /// warm contact glow where the liquid meets the glass. Same values, so the
    /// watch reads as the same object rather than a different orb.
    private static let rimBlue = RGB(r: 0.80, g: 0.88, b: 1.0)
    private static let liquidBlue = RGB(r: 0.14, g: 0.38, b: 0.92)
    private static let domeBlue = RGB(r: 0.50, g: 0.60, b: 0.80)
    private static let warm = RGB(r: 1.0, g: 0.47, b: 0.16)
    private static let glassBody = RGB(r: 0.012, g: 0.020, b: 0.038)

    private static func palette(_ m: Mode) -> (RGB, RGB, RGB) {
        switch m {
        case .idle:
            return (rimBlue, liquidBlue, domeBlue)
        case .thinking:
            // A touch warmer, so "working" is legible at a glance.
            return (rimBlue, RGB.mix(liquidBlue, warm, 0.18), RGB.mix(domeBlue, warm, 0.25))
        case .speaking:
            return (rimBlue, liquidBlue, RGB.mix(domeBlue, warm, 0.40))
        case .error:
            return (RGB(r: 1.0, g: 0.82, b: 0.84), RGB(r: 0.90, g: 0.26, b: 0.32),
                    RGB(r: 1.0, g: 0.50, b: 0.42))
        }
    }
    ...
    private static func draw(_ ctx: inout GraphicsContext, size sz: CGSize, mode: Mode, t: Double) {
        ...
        // The glass body first, in NORMAL blend: a dark globe the liquid then
        // lights from inside. Without it the watch showed bare ribbons floating
        // on black instead of the phone's solid dark sphere.
        let bodyRect = CGRect(x: c.x - rs, y: c.y - rs, width: rs * 2, height: rs * 2)
        ctx.fill(
            Path(ellipseIn: bodyRect),
            with: .radialGradient(
                Gradient(stops: [
                    .init(color: glassBody.opacity(0.98), location: 0),
                    .init(color: glassBody.opacity(0.94), location: 0.72),
                    .init(color: RGB.mix(glassBody, domeBlue, 0.22).opacity(0.90), location: 1),
                ]),
                // Offset up-left: one light source, same as the phone's dome term.
                center: CGPoint(x: c.x - rs * 0.30, y: c.y - rs * 0.34),
                startRadius: 0, endRadius: rs * 1.5))
        ...
        // a wide faint halo-ring under a bright thin ring. Cool and bright, the
        // way the phone's glass catches the light.
        let rimCol = RGB.mix(hi, white, 0.35)
