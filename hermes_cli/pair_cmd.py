...
# ── webui import shim ───────────────────────────────────────────────────────
# pair_cmd lives in hermes_cli/, the pairing module lives in webui/api/.
...
def _ensure_pairing_import():
    ...
    The webui directory sits next to hermes_cli/ in the JarvisCopilot
    repo. Falls back to walking up from the venv-installed location.
    """
    candidates = []
    here = Path(__file__).resolve()
    # repo layout: <repo>/hermes_cli/pair_cmd.py + <repo>/webui/
