...
- **Git Worktree Isolation** — `jarviscopilot -w` launches isolated agent sessions in git worktrees for safe parallel work on the same repo. ([#654](https://github.com/NousResearch/hermes-agent/pull/654))
...
- Fix provider selection not persisting when switching via jarviscopilot model ([#881](https://github.com/NousResearch/hermes-agent/pull/881))
...
- Fix install.sh creating ~/.jarviscopilot before moving Node.js directory ([#53](https://github.com/NousResearch/hermes-agent/pull/53)) — @JoshuaMart
