...
def _parse_target(target: str) -> tuple[str, str, str]:
    """Split a route target into (scheme, host, port).

    Accepts an optional scheme prefix so HTTPS upstreams work:
      127.0.0.1:8787            → http
      https://127.0.0.1:8787    → https (self-signed ok; verify disabled)
      http://127.0.0.1:8788     → http
    The JarvisCopilot WebUI serves its own self-signed TLS, so a route pointing at
    it MUST be https:// or nginx (speaking plain HTTP to a TLS port) gets a 502.
    """
    t = (target or "").strip()
    scheme = "http"
...
def _worker_user_directive(settings: Dict[str, Any]) -> str:
    """nginx ``user`` line for the main context — empty unless we run as root.

    nginx started as root drops its WORKER processes to a built-in unprivileged
    user (commonly ``www-data``/``nobody``). The edge dir lives under the root
    home (``~/.jarviscopilot/edge`` → ``/root/...``, mode 700), which that user
    cannot even traverse — so nginx fails to create request-body temp files
    (``client_body_temp``) with ``EACCES`` and returns **500** on any POST whose
    body is large enough to spill to disk (e.g. the voice audio upload) *before*
    the request ever reaches the app. Pin workers to the user that owns the edge
    dir (whoever runs this process) so they can write those temp files.

    Only emitted when running as root: otherwise nginx ignores ``user`` with a
    startup warning, and the workers already run as the (owning) launching user.
    An explicit ``settings['nginx_worker_user']`` overrides the auto-detection.
    """
    user = str(settings.get("nginx_worker_user") or "").strip()
    if not user:
        ...
        try:
            import pwd
            user = pwd.getpwuid(os.geteuid()).pw_name or "root"
        except Exception:
            user = "root"
    return f"user {user};\n"
...
def render_nginx(settings: Dict[str, Any]) -> str:
    ...
    for sub, target in routes.items():
        ...
        scheme, t_host, t_port = _parse_target(target)
        upstream = f"{scheme}://{t_host}:{t_port}"
        # For an HTTPS upstream (the WebUI serves its own self-signed cert),
        # disable cert verification and send SNI so the TLS handshake succeeds.
        proxy_ssl = ("""
            proxy_ssl_verify off;
            proxy_ssl_server_name on;""" if scheme == "https" else "")
    ...
    blocks = "\n\n".join(server_blocks)
    worker_user = _worker_user_directive(settings)
...
{worker_user}worker_processes 1;
# Log to stderr so the supervisor's Popen captures it into the same log file the
# UI "View logs" tails — otherwise upstream errors (e.g. "connect() failed (111:
# Connection refused)" that cause a 502) land in a separate file you can't see.
