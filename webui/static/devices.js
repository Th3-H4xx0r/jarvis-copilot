...
function _renderDevicesSidebar() {
  ...
  const rows = _devicesData.map(d => {
    ...
    const skillsBadge = (d.skills && d.skills.length)
      ...
      : '';
    return `<div class="device-row" data-id="${_devEsc(d.id)}" onclick="selectDevice('${_devEsc(d.id)}')" style="display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:8px;cursor:pointer;font-size:13px">
      ${dot}
...
function _renderDevicesMain() {
  ...
  const cards = _devicesData.map(d => {
    ...
    return `<div class="device-card" style="background:var(--card-bg,rgba(255,255,255,.03));border:1px solid var(--border,rgba(255,255,255,.08));border-radius:12px;padding:14px 16px;margin-bottom:12px">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
        <div style="font-size:14px;font-weight:600">${_devEsc(d.name || 'device')}</div>
        ${onlineBadge}
        <div style="margin-left:auto;display:flex;gap:6px">
          <button class="btn secondary" style="padding:4px 10px;font-size:11px" onclick="logoutDevice('${_devEsc(d.id)}','${_devEsc(d.name)}')">Log out</button>
          <button class="btn" style="padding:4px 10px;font-size:11px;background:rgba(233,69,96,.15);color:#ff7a8a;border:1px solid rgba(233,69,96,.3)" onclick="revokeDevice('${_devEsc(d.id)}','${_devEsc(d.name)}')">Revoke</button>
        </div>
      </div>
