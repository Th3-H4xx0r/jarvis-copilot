...
const APP_TITLEBAR_KEYS = {
  chat: 'tab_chat', voice: 'tab_voice', tasks: 'tab_integrations', skills: 'tab_skills',
...
function _cronPanelExpandKey(jobId, suffix){
  return `jarviscopilot-webui-cron-${suffix}-expanded-${encodeURIComponent(String(jobId||''))}`;
}
...
async function runKanbanDispatcher(){
  ...
  // (one `jarviscopilot -p <assignee>` per claimed row, up to max=8 per call).
...
//
// The assignee field auto-completes against the union of (a) live JarvisCopilot
...
async function _kanbanPopulateAssigneeSelect(currentValue){
  ...
  let html = '';
  if (profiles.length) {
    html += `<optgroup label="${esc(t('kanban_assignee_profiles_label') || 'JarvisCopilot profiles')}">`;
...
function _renderLlmWikiStatus(d) {
  ...
  const rawDocsUrl = status.docs_url || 'https://jarviscopilot.nousresearch.com/docs/user-guide/skills/bundled/research/research-llm-wiki';
  ...
  const toggleNote = status.toggle_available
    ? 'Toggle available from configured JarvisCopilot Agent setting.'
...
function switchSettingsSection(name){
  const section=(name==='appearance'||name==='preferences'||name==='providers'||name==='plugins'||name==='integrations'||name==='system')?name:'conversation';
  _settingsSection=section;
  _currentSettingsSection=section;
  const map={conversation:'Conversation',appearance:'Appearance',preferences:'Preferences',providers:'Providers',plugins:'Plugins',integrations:'Integrations',system:'System'};
  // Sidebar menu items
  document.querySelectorAll('#settingsMenu .side-menu-item').forEach(it=>{
    it.classList.toggle('active', it.dataset.settingsSection===section);
  });
  // Panes in main
  ['conversation','appearance','preferences','providers','plugins','integrations','system'].forEach(key=>{
...
// ── Integrations: Photon (iMessage) provider setup ─────────────────────────
// A small <span> describing the running sidecar's state (probed by the backend
// on GET/POST /api/integrations/photon) so a still-in-mock-mode sidecar after a
// save is visible instead of silently swallowing messages.
function _photonSidecarStatusHtml(sc){
  if(!sc) return '';
  if(!sc.reachable) return `<span style="color:#d29922">● sidecar not reachable${sc.error?` (${esc(sc.error)})`:''}</span>`;
  if(!sc.ok) return `<span style="color:#f85149">● sidecar error: ${esc(sc.error||'')}</span>`;
  if(sc.mock) return `<span style="color:#d29922">● sidecar running in MOCK mode — set PROJECT_ID/SECRET and restart it</span>`;
  return `<span style="color:#3fb950">● sidecar live${sc.connected?'' :' (connecting…)'}</span>`;
}

function _photonCardHtml(p){
  p=p||{};
  const v=(k)=>(p[k]==null?'':String(p[k]));
  const secretHint=(f)=>f?'<span style="opacity:.6;font-size:11px">saved — leave blank to keep</span>':'';
  const status=p.configured?'<span style="color:#3fb950">● configured</span>':'<span style="opacity:.6">not configured</span>';
  const row=(label,key,type,ph,hint)=>`
    <label style="display:block;margin:.55em 0">
      ...
      <input type="${type}" id="ph_${key}" value="${esc(v(key))}" placeholder="${ph||''}"
        ...
        style="width:100%;max-width:440px;padding:8px 10px;border-radius:8px;
        border:1px solid var(--border,#33363d);background:var(--input-bg,rgba(255,255,255,.05));
        color:inherit;font-size:13px"></label>`;
  return `
    <div class="settings-card" style="padding:14px 16px;border:1px solid var(--border,#33363d);border-radius:12px">
      ...
      ${row('Sidecar URL','sidecar_url','text','http://127.0.0.1:8799')}
      ${row('Sidecar token','sidecar_token','password',p.sidecar_token_set?'••••••••':'auto-generated by install.sh',secretHint(p.sidecar_token_set))}
      ${row('Allowed inbound handles','allowed_users','text','comma-separated (optional)')}
      <label style="display:flex;align-items:center;gap:8px;margin:.6em 0;font-size:13px;cursor:pointer">
        <input type="checkbox" id="ph_allow_all" ${p.allow_all?'checked':''}>
        Allow all inbound senders (recommended when it's just you texting Jarvis)
      </label>
      <div style="margin-top:.4em;display:flex;align-items:center;gap:10px">
        <button type="button" class="btn-primary" id="phSaveBtn" onclick="savePhotonProvider()"
          style="padding:7px 14px;border-radius:8px;font-size:13px">Save</button>
        <span id="phSaveMsg" style="font-size:12px;opacity:.9"></span>
      </div>
      <p style="opacity:.55;font-size:11px;margin:.5em 0 0">Restart the gateway to apply inbound-auth changes (allowlist).</p>
    </div>`;
}

async function loadIntegrationsPanel(){
  const host=$('integrationsList');
  if(!host) return;
  host.innerHTML='<div style="opacity:.6;font-size:13px;padding:8px 0">Loading…</div>';
  let p={};
  try{ p=(await api('/api/integrations/photon'))||{}; }catch(e){ p={}; }
  host.innerHTML=_photonCardHtml(p);
}

async function savePhotonProvider(){
  const btn=$('phSaveBtn'), msg=$('phSaveMsg');
  ...
  const allowAll=$('ph_allow_all'); if(allowAll) payload.allow_all=allowAll.checked;
...
window.savePhotonProvider=savePhotonProvider;
...
async function loadSettingsPanel(){
  ...
  try{
    ...
    // Uses a separate key (jarviscopilot-webui-workspace-panel-pref) so that
    // closing the panel via toolbar X does not clear the user's preference.
    const wsPanelCb=$('settingsWorkspacePanelOpen');
    if(wsPanelCb){
      wsPanelCb.checked=localStorage.getItem('jarviscopilot-webui-workspace-panel-pref')==='open';
      wsPanelCb.onchange=function(){
        const open=this.checked;
        localStorage.setItem('jarviscopilot-webui-workspace-panel-pref',open?'open':'closed');
        // Also sync the runtime key so the current session reflects the change
        localStorage.setItem('jarviscopilot-webui-workspace-panel',open?'open':'closed');
    ...
    if(modelSel){
      ...
      // (what the CLI's `jarviscopilot model` command writes) still selects the matching
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=settings.bot_name||'JarvisCopilot';
...
function _buildProviderCard(p){
  ...
  // key_source can be 'oauth' (jarviscopilot auth), 'config_yaml' (token in config.yaml), or 'none'.
  ...
  if(isOauth){
    ...
    if(p.key_source==='config_yaml'){
      hint.textContent=t('providers_oauth_config_yaml_hint')||'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run jarviscopilot auth.';
      ...
      hint.textContent=t('providers_oauth_not_configured_hint')||'Not authenticated. Run jarviscopilot auth in the terminal to configure this provider.';
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=body.bot_name||'JarvisCopilot';
  if(typeof applyBotName==='function') applyBotName();
...
async function saveSettings(andClose){
  ...
  body.bot_name=botName||'JarvisCopilot';
