...
def test_session_db_close_is_idempotent():
    ...
    Skipped when jarviscopilot_state is not on the import path (e.g. on the GH
    ...
    if importlib.util.find_spec("jarviscopilot_state") is None:
        pytest.skip("jarviscopilot_state not on import path (CI-only — agent repo not present)")
