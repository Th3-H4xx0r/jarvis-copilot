...
Covers:
  1. _provider_api_key_present returns True for minimax-cn when
     MINIMAX_CN_API_KEY is in env (via jarviscopilot_cli.auth.get_auth_status)
  ...
  4. The jarviscopilot_cli import failure path is safe (falls back gracefully)
"""
...
def _inject_jarviscopilot_cli_auth(get_auth_status_return):
    """Inject a minimal jarviscopilot_cli.auth stub into sys.modules.

    CI doesn't install jarviscopilot_cli (it's a separate package).  Tests that
    exercise the jarviscopilot_cli fallback path must inject the module themselves
    rather than relying on mock.patch('jarviscopilot_cli.auth.get_auth_status')
    which fails with ModuleNotFoundError when the module isn't installed.
    """
    mock_auth = types.ModuleType("jarviscopilot_cli.auth")
    mock_auth.get_auth_status = mock.MagicMock(return_value=get_auth_status_return)
    mock_jarviscopilot_cli = types.ModuleType("jarviscopilot_cli")

    return mock.patch.dict(sys.modules, {
        "jarviscopilot_cli": mock_jarviscopilot_cli,
        "jarviscopilot_cli.auth": mock_auth,
    })
...
# ---------------------------------------------------------------------------
# 1. _provider_api_key_present via jarviscopilot_cli fallback
# ---------------------------------------------------------------------------

class TestProviderApiKeyPresentFallback:

    def test_minimax_cn_logged_in_returns_true(self):
        """minimax-cn: if jarviscopilot_cli.auth.get_auth_status returns logged_in, must be True."""
        with mock.patch("api.onboarding._SUPPORTED_PROVIDER_SETUPS", {
            "openrouter": {}, "anthropic": {}, "openai": {}, "custom": {}
        }):
            with _inject_jarviscopilot_cli_auth({"logged_in": True}):
    ...
    def test_unsupported_provider_logged_out_returns_false(self):
        ...
        with mock.patch("api.onboarding._SUPPORTED_PROVIDER_SETUPS", {
            ...
            with _inject_jarviscopilot_cli_auth({"logged_in": False}):
                result = _call_provider_api_key_present("deepseek")
                assert result is False

    def test_jarviscopilot_cli_import_failure_is_safe(self):
        """If jarviscopilot_cli is unavailable, falls back silently to False."""
        import builtins
        real_import = builtins.__import__

        def _block_jarviscopilot_cli(name, *args, **kwargs):
            if name.startswith("jarviscopilot_cli"):
                raise ImportError("jarviscopilot_cli not available")
            return real_import(name, *args, **kwargs)

        with mock.patch("api.onboarding._SUPPORTED_PROVIDER_SETUPS", {
            "openrouter": {}, "anthropic": {}, "openai": {}, "custom": {}
        }):
            with mock.patch("builtins.__import__", side_effect=_block_jarviscopilot_cli):
                result = _call_provider_api_key_present("minimax-cn")
