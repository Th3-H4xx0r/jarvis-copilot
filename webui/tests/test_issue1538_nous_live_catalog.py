...
``jarviscopilot_cli.models.provider_model_ids("nous")``, prefix every id with ``@nous:``
to match the existing routing convention, fall back to the curated static
list when ``jarviscopilot_cli`` is unavailable.
"""
...
# by jarviscopilot_cli.models.provider_model_ids("nous") (see #1538 issue body).
...
def _install_fake_jarviscopilot_cli(monkeypatch, *, nous_ids=None, raise_on_lookup=False):
    """Install fake ``jarviscopilot_cli`` modules so detection sees Nous as authenticated
    and ``provider_model_ids("nous")`` returns the desired catalog.

    Mirrors :func:`tests.test_issue1420_lmstudio_provider_env_var._install_fake_jarviscopilot_cli`
    but specialised for Nous detection (Nous is OAuth so the env-var path
    is not used — we drive detection via ``jarviscopilot_cli.auth.list_auth_providers``).
    """
...
def _scrub_provider_env(monkeypatch):
    ...
    via the fake jarviscopilot_cli stubs (not unrelated keys leaked from the runner)."""
    for var in (
...
class TestNousLiveCatalog:
    ...
    def test_nous_models_live_fetch_when_jarviscopilot_cli_available(self, monkeypatch, tmp_path):
        _scrub_provider_env(monkeypatch)
        _install_fake_jarviscopilot_cli(monkeypatch, nous_ids=SAMPLE_NOUS_LIVE_IDS)
        ...
        try:
            ...
            for required in (
                ...
                assert required in ids, (
                    ...
                    f"the jarviscopilot_cli dispatch is broken or the @nous: prefix is "
                    f"missing."
                )
        finally:
            restore()
...
class TestNousStaticFallback:
    """When ``jarviscopilot_cli`` is not importable or its lookup raises, we fall back
    to the curated four-entry static list — never empty."""

    def test_static_fallback_when_jarviscopilot_cli_raises(self, monkeypatch, tmp_path):
        _scrub_provider_env(monkeypatch)
        _install_fake_jarviscopilot_cli(monkeypatch, raise_on_lookup=True)
        monkeypatch.setattr(profiles, "get_active_hermes_home", lambda: tmp_path)
        ...
        try:
            ...
            nous_groups = [g for g in data.get("groups", []) if g.get("provider_id") == "nous"]
            assert nous_groups, (
                "Nous group must still appear when jarviscopilot_cli fails — the "
...
class TestStaticListPreservedAsFallback:
    ...
    def test_static_list_present(self):
        ...
        assert _PROVIDER_MODELS.get("nous"), (
            ...
            "a fallback for environments where jarviscopilot_cli is unavailable."
        )
