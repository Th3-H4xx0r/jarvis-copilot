...
def test_onboarding_uses_i18n_helpers():
    ...
    assert "onboarding_title: 'Welcome to JarvisCopilot Web UI'" in i18n
    assert "onboarding_title: 'Bienvenido a JarvisCopilot Web UI'" in i18n


def test_bootstrap_script_contains_official_installer_and_windows_guard():
    src = read("bootstrap.py")
    assert (
        "https://raw.githubusercontent.com/NousResearch/jarviscopilot/main/scripts/install.sh"
        in src
    )
