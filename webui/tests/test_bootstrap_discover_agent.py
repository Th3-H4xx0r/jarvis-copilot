...
When the standard candidate paths (`~/.jarviscopilot/jarviscopilot`, `~/jarviscopilot`,
`<webui-parent>/jarviscopilot`, `HERMES_WEBUI_AGENT_DIR`) don't match, bootstrap
should fall back to introspecting the `jarviscopilot` console-script's shebang —
...
def _make_agent_install(tmp_path, *, with_run_agent: bool = True):
    """Build a fake jarviscopilot install with venv/bin/python3 + run_agent.py."""
...
def _make_jarviscopilot_cli(tmp_path, shebang_target: str | None):
    """Write a `jarviscopilot` console-script with the given shebang interpreter."""
    bin_dir = tmp_path / "user-bin"
    bin_dir.mkdir()
    jarviscopilot = bin_dir / "jarviscopilot"
    if shebang_target is None:
        jarviscopilot.write_text("not a script", encoding="utf-8")
    else:
        jarviscopilot.write_text(
            textwrap.dedent(
                f"""\
                #!{shebang_target}
    ...
    return jarviscopilot


def _isolate_discover_agent_dir(monkeypatch, tmp_path, hermes_path):
    """Point `which("jarviscopilot")` at our fake CLI and clear all standard candidates."""
    monkeypatch.setattr(bootstrap.shutil, "which", lambda name: str(hermes_path) if name == "jarviscopilot" else None)
    ...
    # `jarviscopilot` sibling on the dev machine running these tests.
    monkeypatch.setattr(bootstrap, "REPO_ROOT", tmp_path / "isolated-repo-root")
    # Pin Path.home() to a directory with no `.jarviscopilot/jarviscopilot` or
    # `jarviscopilot` so the hard-coded `Path.home() / ".jarviscopilot" / "jarviscopilot"`
    # / `Path.home() / "jarviscopilot"` candidates in `discover_agent_dir()`
...
def test_discovers_agent_dir_from_hermes_shebang(monkeypatch, tmp_path):
    """Happy path: jarviscopilot shebang → walk up parents → find run_agent.py → return install."""
    install, venv_python = _make_agent_install(tmp_path)
    jarviscopilot = _make_jarviscopilot_cli(tmp_path, str(venv_python))
    _isolate_discover_agent_dir(monkeypatch, tmp_path, jarviscopilot)
...
def test_returns_none_when_hermes_not_on_path(monkeypatch, tmp_path):
    _make_agent_install(tmp_path)  # install exists, but no `jarviscopilot` CLI to point at it
...
def test_returns_none_when_hermes_has_no_shebang(monkeypatch, tmp_path):
    """A `jarviscopilot` file without a #! line gives us nothing to introspect."""
    _make_agent_install(tmp_path)
    jarviscopilot = _make_jarviscopilot_cli(tmp_path, shebang_target=None)
    _isolate_discover_agent_dir(monkeypatch, tmp_path, jarviscopilot)
...
def test_returns_none_when_shebang_interpreter_does_not_walk_to_run_agent(monkeypatch, tmp_path):
    ...
    jarviscopilot = _make_jarviscopilot_cli(tmp_path, "/usr/bin/python3")
    _isolate_discover_agent_dir(monkeypatch, tmp_path, jarviscopilot)
