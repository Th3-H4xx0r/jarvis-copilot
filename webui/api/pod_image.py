"""GET /api/devices/pod/image?url=…&size=… — a web image, made safe for the Jarvis Pod.

The pod's JPEG decoder handles baseline JPEG only, has no WebP/GIF, and a 240 px round
...
def fetch_for_pod(url: str, size: int) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (JarvisPod image fetch)"})
...
def handle_pod_image(handler, parsed) -> bool:
