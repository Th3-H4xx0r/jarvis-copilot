"""WebUI bridge for JarvisCopilot persistent session goals."""
...
try:  # Exposed as a module attribute so tests can monkeypatch it directly.
    from jarviscopilot_cli.goals import (  # type: ignore
...
except Exception:  # pragma: no cover - depends on installed jarviscopilot
...
def _default_max_turns() -> int:
    """Return the configured /goal turn budget, defaulting to JarvisCopilot' 20 turns."""
    try:
...
def _profile_db(profile_home: str | Path):
    ...
    The upstream JarvisCopilot GoalManager persists through jarviscopilot_cli.goals.load_goal(),
    ...
    if cached is not None:
        return cached
    try:
        from jarviscopilot_state import SessionDB  # type: ignore
...
class _ProfileGoalManager:
    ...
    def __init__(self, session_id: str, *, profile_home: str | Path, default_max_turns: int = 20):
        if GoalState is None:
            raise RuntimeError("JarvisCopilot goal state unavailable")
...
def restore_goal_state(session_id: str, snapshot: Any, *, profile_home: str | Path | None = None) -> None:
    ...
    if isinstance(mgr, _ProfileGoalManager):
        ...
        return
    try:
        from jarviscopilot_cli.goals import save_goal  # type: ignore
