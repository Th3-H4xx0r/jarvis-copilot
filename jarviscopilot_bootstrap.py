"""Windows UTF-8 bootstrap for JarvisCopilot entry points.
...
should be imported at the very top of every JarvisCopilot entry point
(``jarviscopilot``, ``jarviscopilot``, ``jarviscopilot-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import jarviscopilot_bootstrap``
# (or ``from jarviscopilot_bootstrap import apply_windows_utf8_bootstrap``) at
