...
SSE streaming. By exposing this endpoint, jarviscopilot becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  jarviscopilot   │
│  NextChat        │  │                                    │  gateway        │
...
4. Chat with jarviscopilot through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns jarviscopilot as a model) |
...
But jarviscopilot sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "jarviscopilot"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured jarviscopilot model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of jarviscopilot's core prompt
...
Once implemented, jarviscopilot works as a drop-in backend for:
