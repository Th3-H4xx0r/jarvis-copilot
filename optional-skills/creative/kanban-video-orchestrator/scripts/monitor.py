#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `jarviscopilot kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed jarviscopilot CLI."""
    try:
        out = subprocess.run(
            ["jarviscopilot", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `jarviscopilot kanban list`
    out = subprocess.run(
        ["jarviscopilot", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["jarviscopilot", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not hermes_available():
        print("ERROR: 'jarviscopilot' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
