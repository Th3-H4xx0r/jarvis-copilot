...
jarviscopilot kanban watch --tenant <project-slug>

# Snapshot of the board
jarviscopilot kanban list --tenant <project-slug>
jarviscopilot kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
jarviscopilot kanban stats --tenant <project-slug>

# Visual dashboard (browser)
jarviscopilot dashboard

# Inspect a specific task (includes comments + events)
jarviscopilot kanban show <task-id>

# Follow a single task's event stream
jarviscopilot kanban tail <task-id>
```

Verify available subcommands with `jarviscopilot kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `jarviscopilot kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `jarviscopilot kanban block <id>` then `jarviscopilot kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `jarviscopilot kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `jarviscopilot kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `jarviscopilot profile list` (profile exists?), `jarviscopilot status` (gateway/dispatcher up?), and verify tenant. |
...
jarviscopilot kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
jarviscopilot kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(jarviscopilot kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
jarviscopilot kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
jarviscopilot kanban block <task-id>
jarviscopilot kanban archive <task-id>

# Diagnose what happened
jarviscopilot kanban show <task-id>      # task body, comments, recent events
jarviscopilot kanban tail <task-id>      # follow the live event stream
jarviscopilot kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `jarviscopilot kanban create` for the director
...
while true; do
    clear
    jarviscopilot kanban list --tenant <slug>
    echo "---"
    jarviscopilot kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `jarviscopilot kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `jarviscopilot profile list` and start
