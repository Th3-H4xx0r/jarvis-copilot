...
Maps each role archetype to the JarvisCopilot skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public jarviscopilot
...
## JarvisCopilot skills relevant to video production

### Visual / rendering skills (`jarviscopilot/skills/creative/`)
...
### Design / pre-production skills (`jarviscopilot/skills/creative/`)
...
### Audio / media skills (`jarviscopilot/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`jarviscopilot/skills/devops/`)
...
These are **not** JarvisCopilot skills but external CLIs / APIs that profiles invoke.
...
## Built-in JarvisCopilot tools for media review

These are native JarvisCopilot tools — not invoked via terminal but through their own
...
key is present in `~/.jarviscopilot/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every JarvisCopilot profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.jarviscopilot/.env` → environment variable.
