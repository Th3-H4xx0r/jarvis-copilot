"""ACP auth helpers — detect and advertise JarvisCopilot authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active JarvisCopilot runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if JarvisCopilot can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for JarvisCopilot.
    ...
    not have JarvisCopilot provider credentials configured yet, so JarvisCopilot always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate JarvisCopilot using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure JarvisCopilot provider",
            description=(
                "Open JarvisCopilot' interactive model/provider setup in a terminal. "
                "Use this when JarvisCopilot has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
