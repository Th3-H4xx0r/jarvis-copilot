"""ACP session manager — maps ACP sessions to JarvisCopilot AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.jarviscopilot/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when JarvisCopilot itself is running in WSL.

    Windows ACP clients can launch ``jarviscopilot acp`` inside WSL while still sending
    ...
    """
    from jarviscopilot_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch JarvisCopilot from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["jarviscopilot-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed JarvisCopilot agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by JarvisCopilot AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current JarvisCopilot runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.jarviscopilot/state.db``) is lazily created.
        """
