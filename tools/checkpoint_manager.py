...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (JarvisCopilot convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal JarvisCopilot infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``jarviscopilot checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `jarviscopilot checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "jarviscopilot@local"], store, cfg_wd)
    _run_git(["config", "user.name", "JarvisCopilot Checkpoint"], store, cfg_wd)
...
# ---------------------------------------------------------------------------
# Public helpers for `jarviscopilot checkpoints` CLI
# ---------------------------------------------------------------------------
