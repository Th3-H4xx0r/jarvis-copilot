...
Lets the LLM write a Python script that calls JarvisCopilot tools via RPC,
...
# available on every platform JarvisCopilot itself runs on.
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-JarvisCopilot-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-JarvisCopilot-profile soft guard."""',
...
"""Auto-generated JarvisCopilot tools RPC stubs."""
...
"""Auto-generated JarvisCopilot tools RPC stubs (file-based transport)."""
...
def _execute_remote(
    ...
    try:
        ...
        tz = os.getenv("jarviscopilot_timeZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    to a subset of JarvisCopilot tools.
    ...
    try:
        ...
        # Ensure the jarviscopilot root is importable in the sandbox so
        ...
        # jarviscopilot_timeZONE is an internal JarvisCopilot setting and must not leak
        # into child processes.
        _tz_name = os.getenv("jarviscopilot_timeZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("jarviscopilot_timeZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.jarviscopilot/.env')).
...
def _load_config() -> dict:
    ...
    ``jarviscopilot --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (jarviscopilot's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``jarviscopilot tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and jarviscopilot's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.jarviscopilot/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call JarvisCopilot tools programmatically. "
