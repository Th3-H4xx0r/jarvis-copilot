...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.jarviscopilot/config.yaml``. JarvisCopilot
...
Configuration is loaded from ~/.jarviscopilot/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``jarviscopilot_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from jarviscopilot_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.jarviscopilot/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.jarviscopilot/config.yaml.
    ...
    try:
        ...
        logger.debug("jarviscopilot_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into JarvisCopilot without any Python code
...
#           output_format: wav
#
# JarvisCopilot writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `jarviscopilot model` or set XAI_API_KEY.")
    ...
    # output_format when JarvisCopilot actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where JarvisCopilot caches Piper voice models.

    Resolves to ``~/.jarviscopilot/cache/piper-voices/`` under the active
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.jarviscopilot/config.yaml (tts: section).
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run jarviscopilot setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'jarviscopilot setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'jarviscopilot tools' and select Piper under TTS, or install manually: "
