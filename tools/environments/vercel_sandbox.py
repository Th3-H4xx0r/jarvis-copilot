...
Uses the Vercel Python SDK to run commands in cloud sandboxes through JarvisCopilot'
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.jarviscopilot"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.jarviscopilot"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.jarviscopilot"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.jarviscopilot"
        )
