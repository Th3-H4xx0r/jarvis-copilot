...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local JarvisCopilot home override into subprocess env."""
    try:
        from jarviscopilot_constants import get_hermes_home_override
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\jarviscopilot\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\jarviscopilot\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hermes_portable_git = os.path.join(_local_appdata, "jarviscopilot", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. JarvisCopilot requires Git for Windows on Windows.\n"
...
def _make_run_env(env: dict) -> dict:
    ...
    # JarvisCopilot is driving via _find_bash (Git Bash), and Git Bash itself
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, JarvisCopilot trusts them.
    """
