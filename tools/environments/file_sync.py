...
def iter_sync_files(container_base: str = "/root/.jarviscopilot") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.jarviscopilot to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.jarviscopilot", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.jarviscopilot/`` directory as a tar archive,
        ...
        # against an uninitialized remote .jarviscopilot/ directory.
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.jarviscopilot/skills/a.md`` →
        ``~/.jarviscopilot/skills/a.md``, a new remote file at
        ``/root/.jarviscopilot/skills/b.md`` maps to ``~/.jarviscopilot/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
