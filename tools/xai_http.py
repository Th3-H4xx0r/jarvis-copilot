...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.jarviscopilot/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    ...
    """
    try:
        from jarviscopilot_cli.config import get_env_value as _hermes_get_env_value
...
def hermes_xai_user_agent() -> str:
    """Return a stable JarvisCopilot-specific User-Agent for xAI HTTP calls."""
    try:
        from jarviscopilot_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"JarvisCopilot-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers JarvisCopilot-managed xAI OAuth credentials when available, then falls back
