...
# Must match jarviscopilot_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.jarviscopilot/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'jarviscopilot auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (jarviscopilot_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (JarvisCopilot subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
