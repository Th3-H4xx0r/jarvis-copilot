...
Backend is selected during ``jarviscopilot tools`` setup (web.backend in config.yaml).
When available, JarvisCopilot can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.jarviscopilot/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``jarviscopilot tools``).
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `jarviscopilot tools` to set one up."
                ),
            }
        else:
            logger.info(
