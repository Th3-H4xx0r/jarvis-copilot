...
def managed_nous_tools_enabled() -> bool:
    ...
    try:
        ...
        from jarviscopilot_cli.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.jarviscopilot/.env`` (via
    ``jarviscopilot_cli.config.get_env_value`` when available) so tool-side
