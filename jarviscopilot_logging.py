"""Centralized logging setup for JarvisCopilot.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.jarviscopilot/logs/`` (profile-aware via ``get_hermes_home()``).
...
from jarviscopilot_constants import get_config_path, get_hermes_home
...
# Used by _ComponentFilter and exposed for ``jarviscopilot logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "hermes_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("jarviscopilot_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the JarvisCopilot logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the JarvisCopilot home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the jarviscopilot group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from jarviscopilot_cli.config import is_managed
