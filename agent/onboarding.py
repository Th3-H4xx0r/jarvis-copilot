...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time JarvisCopilot starts and finds ``~/.openclaw/``.

    Points users at ``jarviscopilot claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``jarviscopilot claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to JarvisCopilot, run "
        "`jarviscopilot claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `jarviscopilot claw cleanup` (renames it to ~/.openclaw.pre-migration — "
