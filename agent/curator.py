...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.jarviscopilot/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``jarviscopilot update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``jarviscopilot curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `jarviscopilot curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "`jarviscopilot curator run` (no flag).\n"
    "\n"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as JarvisCopilot' background skill CURATOR. This is an "
    ...
    "into ~/.jarviscopilot/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.jarviscopilot/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.jarviscopilot/logs/curator/``)
    ...
    authored skill data in ``~/.jarviscopilot/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every jarviscopilot install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        keep an umbrella stable: jarviscopilot curator pin document-tools
    ...
    lines.append("full report: jarviscopilot curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: jarviscopilot curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.jarviscopilot/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.jarviscopilot/skills/.archive/` for "
            "safety and can be restored via `jarviscopilot curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.jarviscopilot/skills/.archive/`. "
            "Restore any via `jarviscopilot curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `jarviscopilot curator restore <name>`")
    lines.append("- All archives live under `~/.jarviscopilot/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `jarviscopilot curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `jarviscopilot curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``jarviscopilot model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `jarviscopilot model` → auxiliary
