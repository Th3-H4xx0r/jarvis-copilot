"""Projects codex app-server events into JarvisCopilot' messages list.

The translator that lets JarvisCopilot' memory/skill review keep working under the
...
JarvisCopilot' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape JarvisCopilot
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way JarvisCopilot' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but JarvisCopilot stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way JarvisCopilot already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — JarvisCopilot' messages store text only.
        text_parts: list[str] = []
