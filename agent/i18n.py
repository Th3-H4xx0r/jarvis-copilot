"""Lightweight internationalization (i18n) for JarvisCopilot static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by JarvisCopilot itself -- approval prompts, a handful of gateway slash
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a jarviscopilot dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`jarviscopilot_cli.config.save_config` if a running process
