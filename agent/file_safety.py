...
def _hermes_root_path() -> Path:
    """Resolve the JarvisCopilot root dir (always the parent of any profile, never per-profile)."""
    try:
        from jarviscopilot_constants import get_default_hermes_root  # local import to avoid cycles
...
def is_write_denied(path: str) -> bool:
    ...
    # JarvisCopilot control-plane files: block both the ACTIVE profile's view
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Two categories are blocked:
      ...
      * Internal JarvisCopilot cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global JarvisCopilot
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.jarviscopilot/.env`` and exfiltrate the file. The read-deny exists
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a JarvisCopilot credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the JarvisCopilot MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a JarvisCopilot MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# JarvisCopilot profiles are separate HERMES_HOME dirs under
...
# edited skills under both ``~/.jarviscopilot/profiles/hermes-security/skills/``
# AND ``~/.jarviscopilot/skills/`` (the default profile's skills) without realizing
...
def _resolve_active_profile_name() -> str:
    ...
    ``~/.jarviscopilot``              -> ``"default"``
    ``~/.jarviscopilot/profiles/X``  -> ``"X"``
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside JarvisCopilot scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    JarvisCopilot entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to JarvisCopilot profile {info['target_profile']!r}, but the "
