...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``jarviscopilot lsp
  status``.
...
def hermes_lsp_bin_dir() -> Path:
    ...
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".jarviscopilot")
    p = Path(home) / "lsp" / "bin"
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``jarviscopilot lsp status`` CLI to give users a quick
