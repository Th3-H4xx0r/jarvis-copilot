...
JarvisCopilot keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for JarvisCopilot's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with JarvisCopilot)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). JarvisCopilot typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with JarvisCopilot retry/error classification."""
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.jarviscopilot/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `jarviscopilot setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "jarviscopilot (gemini-native)",
        }
