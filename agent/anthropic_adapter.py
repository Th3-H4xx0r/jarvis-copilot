"""Anthropic Messages API adapter for JarvisCopilot.

Translates between JarvisCopilot's internal OpenAI-style message format and
...
# JarvisCopilot effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    JarvisCopilot historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by JarvisCopilot)
    ...
    # This remains as a compatibility fallback for pre-migration JarvisCopilot configs.
...
# Stores credentials in ~/.jarviscopilot/.anthropic_oauth.json (our own file).
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize JarvisCopilot with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Hermes-managed OAuth credentials from ~/.jarviscopilot/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read JarvisCopilot OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See jarviscopilot#13848.
            #
    ...
    # are preserved.  See jarviscopilot#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("JarvisCopilot", "Claude Code")
                text = text.replace("JarvisCopilot agent", "Claude Code")
                text = text.replace("jarviscopilot", "claude-code")
    ...
    # silently hides reasoning text that JarvisCopilot surfaces in its CLI. We
