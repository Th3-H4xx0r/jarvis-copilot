...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.jarviscopilot/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before JarvisCopilot triggers summarization.  Higher values delay
...
# in lockstep with jarviscopilot_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``jarviscopilot_cli.__version__`` is
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``jarviscopilot_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.jarviscopilot/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    try:
        from jarviscopilot_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from JarvisCopilot auth store.
    ...
    try:
        from jarviscopilot_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from jarviscopilot_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from jarviscopilot_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from jarviscopilot_cli.models import get_nous_recommended_aux_model
...
def _try_azure_foundry(
    *,
    ...
    :func:`jarviscopilot_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from jarviscopilot_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def resolve_provider_client(
    ...
    try:
        from jarviscopilot_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("jarviscopilot_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from jarviscopilot_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "claude-code":
            # Claude Code CLI provider. No api_key/base_url required from creds
            # (login lives in the CLI itself); the base_url is a marker scheme.
            command = str(creds.get("command", "")).strip() or None
            if not final_model:
                logger.warning(
            ...
            client = ClaudeCodeClient(
                base_url=str(creds.get("base_url", "")).strip() or "claude-cli://local",
                command=command,
            )
            logger.debug("resolve_provider_client: %s (%s)", provider, final_model)
            return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
