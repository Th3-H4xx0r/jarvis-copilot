...
Every JarvisCopilot request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to JarvisCopilot and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=jarviscopilot",
        "client=hermes-client-v<__version__>",
    ]

The version is sourced live from ``jarviscopilot_cli.__version__`` so it auto-aligns
...
``jarviscopilot_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current JarvisCopilot release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``jarviscopilot_cli`` cannot be imported (should
...
def jarviscopilot_client_tag() -> str:
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=jarviscopilot", jarviscopilot_client_tag()]
