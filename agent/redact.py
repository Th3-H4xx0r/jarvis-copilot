...
# (bridged to this env var in jarviscopilot_cli/main.py, gateway/run.py, and
# cli.py) or `HERMES_REDACT_SECRETS=false` in ~/.jarviscopilot/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across JarvisCopilot — used by
    ``jarviscopilot config``, ``jarviscopilot status``, ``jarviscopilot dump``, and anywhere
