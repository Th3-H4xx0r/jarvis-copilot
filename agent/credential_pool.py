...
from jarviscopilot_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``jarviscopilot model`` / ``jarviscopilot auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another JarvisCopilot process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``jarviscopilot`` invocation that defaults to the active provider
        (e.g. setup wizard, ``jarviscopilot auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another JarvisCopilot
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another JarvisCopilot process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other JarvisCopilot profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `jarviscopilot model` / `jarviscopilot auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `jarviscopilot model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `jarviscopilot auth remove <provider> <N>` is stable across all source types.
    try:
        from jarviscopilot_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, JarvisCopilot PKCE)
        # when the user has explicitly configured anthropic as their provider.
        # Without this gate, auxiliary client fallback chains silently read
        # ~/.claude/.credentials.json without user consent.  See PR #4210.
        try:
            from jarviscopilot_cli.auth import is_provider_explicitly_configured
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `jarviscopilot auth add nous --label <name>`).  Fall back to the
        ...
        # JarvisCopilot auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from jarviscopilot_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.jarviscopilot/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `jarviscopilot auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `jarviscopilot auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the JarvisCopilot auth store.  Without this gate the removal is instantly
        ...
        # JarvisCopilot owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `jarviscopilot auth openai-codex`.
        ...
        # When the user logs in via ``jarviscopilot model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``jarviscopilot auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.jarviscopilot/.env over os.environ — the user's config file is the
    # authoritative source for JarvisCopilot credentials. Stale env vars from parent
    ...
    # Honour user suppression — `jarviscopilot auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.jarviscopilot/.env.
    ...
    try:
        from jarviscopilot_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.jarviscopilot/.env over os.environ
