...
Plugins built on JarvisCopilot often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing JarvisCopilot
...
:class:`~jarviscopilot_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors JarvisCopilot' main config
...
handles every provider, fallback chain, and per-task override JarvisCopilot
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`jarviscopilot_cli.plugins.PluginContext`
