"""Unified removal contract for every credential source JarvisCopilot reads from.

JarvisCopilot seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.jarviscopilot/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.jarviscopilot/.anthropic_oauth.json
    ...
    manual        — user ran `jarviscopilot auth add`
...
unify here is **removal**:
    ...
    ``jarviscopilot auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.jarviscopilot/.env  → clear it
    ...
    """
    from jarviscopilot_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.jarviscopilot/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to JarvisCopilot.",
            f"  The pool entry is now suppressed — JarvisCopilot will ignore "
            f"{env_var} until you run `jarviscopilot auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so JarvisCopilot stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `jarviscopilot auth add anthropic` to re-enable if needed.",
    ])


def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.jarviscopilot/.anthropic_oauth.json is ours — delete it outright."""
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared JarvisCopilot Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `jarviscopilot login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `jarviscopilot auth add nous` to re-engage, which is the documented re-add
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``jarviscopilot auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``jarviscopilot auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `jarviscopilot model` → xAI Grok OAuth (SuperGrok Subscription) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the JarvisCopilot auth store is not enough — _seed_from_singletons() would
    ...
    via ``jarviscopilot auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `jarviscopilot auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `jarviscopilot auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    JarvisCopilot stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `jarviscopilot auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.jarviscopilot/.anthropic_oauth.json",
    ))
    register(RemovalStep(
