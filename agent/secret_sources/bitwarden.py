...
JarvisCopilot pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.jarviscopilot/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  JarvisCopilot pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.jarviscopilot/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block JarvisCopilot startup.  Missing binary, no network,
...
def _hermes_bin_dir() -> Path:
    """Where JarvisCopilot stores its managed binaries.  Profile-aware."""
...
# ---------------------------------------------------------------------------
# Public entry point — called from jarviscopilot_cli.env_loader
# ---------------------------------------------------------------------------
