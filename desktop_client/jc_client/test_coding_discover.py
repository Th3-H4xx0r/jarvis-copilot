...
def _put_frames(req_id, csid, cwd, raw, *, chunk_b64=None):
    """Build coding_transcript_put frames carrying ``raw`` (gzip+base64,
    seq-ordered, final eof) — the wire shape the SERVER pushes."""
...
def test_transcript_put_multichunk_reassembles(tmp_path):
    ...
    assert len(frames) > 1  # genuinely multi-chunk
    for fr in frames:
        agent.handle_frame(fr)
    assert _read_put(home, cwd, "sess-big") == body


def test_transcript_put_overwrites_existing(tmp_path):
    home = str(tmp_path)
    cwd = _real("putover")
    _write_transcript_at(home, cwd, "sess-ov", b"OLD\n")
    agent = _transcript_agent([], home)
    for fr in _put_frames("po", "sess-ov", cwd, b"NEW LONGER CONTENT\n"):
        agent.handle_frame(fr)
    assert _read_put(home, cwd, "sess-ov") == b"NEW LONGER CONTENT\n"
...
def test_transcript_put_rejects_bad_session_id(tmp_path):
    ...
    cwd = _real("putbad")
    agent = _transcript_agent([], home)
    # path-traversal id must never touch the filesystem
...
def test_transcript_put_ignores_error_frame(tmp_path):
    home = str(tmp_path)
    cwd = _real("puterr")
    agent = _transcript_agent([], home)
    agent.handle_frame({"type": "coding_transcript_put", "req_id": "pe",
                        "claude_session_id": "sess-e", "cwd": cwd,
                        "ok": False, "error": "boom"})
    from jc_client.coding_discover import (_claude_projects_dir,
                                           _encode_project_dir)
    path = os.path.join(_claude_projects_dir(home),
                        _encode_project_dir(cwd), "sess-e.jsonl")
    assert not os.path.exists(path)


def test_transcript_put_write_failure_does_not_raise(tmp_path, monkeypatch):
    home = str(tmp_path)
    cwd = _real("putwf")
    agent = _transcript_agent([], home)
    import jc_client.coding_discover as cd
    ...
    # must swallow the I/O error, not bubble into the pump
    for fr in _put_frames("pwf", "sess-wf", cwd, b"data\n"):
        agent.handle_frame(fr)


def test_transcript_put_rejects_decompression_bomb(tmp_path, monkeypatch):
    # A small gzip that inflates past the raw cap must be dropped, NOT written
    # (and must not OOM by inflating the whole thing).
    import jc_client.coding_discover as cd
    monkeypatch.setattr(cd, "_TRANSCRIPT_PUT_MAX_RAW_BYTES", 1024)
    home = str(tmp_path)
    cwd = _real("putbomb")
    bomb = b"A" * (50 * 1024)  # 50 KiB of zeros-ish -> tiny gzip, > 1 KiB raw cap
    agent = _transcript_agent([], home)
...
def test_transcript_put_overflow_evicts_only_oldest(tmp_path):
    # Filling past _TRANSCRIPT_PUT_MAX_STREAMS evicts ONE oldest partial stream,
    # not all of them — a later-completing legit stream still writes.
    import jc_client.coding_discover as cd
    home = str(tmp_path)
    cwd = _real("puteviction")
    agent = _transcript_agent([], home)
    n = cd._TRANSCRIPT_PUT_MAX_STREAMS
    # Open N partial streams (first chunk only, no eof) — keep them in-flight.
    for i in range(n):
        fr = _put_frames(f"r{i}", f"sess-{i}", cwd, b"x" * 10, chunk_b64=8)[0]
        fr["eof"] = False
        agent.handle_frame(fr)
    assert len(agent._put_buffers) == n
    # A brand-new stream triggers eviction of exactly one (the oldest, r0).
    fr_new = _put_frames("rNEW", "sess-new", cwd, b"y" * 4)[0]
    fr_new["eof"] = False
    agent.handle_frame(fr_new)
    assert "r0" not in agent._put_buffers           # oldest evicted
    assert "r1" in agent._put_buffers               # the rest survive
    assert "rNEW" in agent._put_buffers
    assert len(agent._put_buffers) == n
