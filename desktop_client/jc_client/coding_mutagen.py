...
DEFAULT_IGNORES = [
    ...
    ".symlinks",
    # Flutter/Dart machine-specific GENERATED state. The ios/macos `ephemeral`
    # tree holds ABSOLUTE symlinks + machine-path files; .dart_tool/.packages/
    # .flutter-plugins*/Generated.xcconfig/flutter_export_environment.sh all encode
    # absolute paths and are regenerated by `flutter pub get`/build. Syncing them
    # between two machines yields permanent, EVER-REGENERATING conflicts that wedge
    # a sync in "auto-resolving…" forever (same class as .symlinks above). They are
    # safe to drop — Flutter regenerates them locally.
    "ephemeral", ".dart_tool", ".packages",
