...
## Token Lifecycle
- Store at `~/.jarviscopilot/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
- jarviscopilot Copilot OAuth: `hermes_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
