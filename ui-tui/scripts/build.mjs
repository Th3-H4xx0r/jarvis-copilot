...
await build({
  ...
  // Skip the prebuilt @jarviscopilot/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@jarviscopilot/ink': resolve(root, 'packages/hermes-ink/src/entry-exports.ts') },
