import { forceRedraw, type MouseTrackingMode } from '@jarviscopilot/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update JarvisCopilot Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `JarvisCopilot #${i + 1}`
