import type { RunExternalProcess } from '@jarviscopilot/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`jarviscopilot ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching jarviscopilot: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`jarviscopilot ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
