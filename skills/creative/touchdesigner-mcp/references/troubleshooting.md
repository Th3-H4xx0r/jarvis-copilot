...
## 5. JarvisCopilot Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.jarviscopilot/config.yaml` when `HERMES_HOME` is unset)
...
Restart the JarvisCopilot session for changes to take effect. The MCP connection is
