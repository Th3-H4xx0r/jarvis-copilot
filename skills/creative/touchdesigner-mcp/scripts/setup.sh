...
HERMES_HOME_DIR="${HERMES_HOME:-$HOME/.jarviscopilot}"
...
# ── 3. Ensure JarvisCopilot config has twozero_td MCP entry ──
if [[ ! -f "$HERMES_CFG" ]]; then
    echo -e " ${FAIL} JarvisCopilot config not found at ${HERMES_CFG}"
    manual_steps+=("Create ${HERMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HERMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in JarvisCopilot config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to JarvisCopilot config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart JarvisCopilot session to pick up config change")
fi
