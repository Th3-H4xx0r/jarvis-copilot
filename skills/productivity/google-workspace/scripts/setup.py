#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for JarvisCopilot.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same JarvisCopilot profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'jarviscopilot[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to JarvisCopilot home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for JarvisCopilot")
